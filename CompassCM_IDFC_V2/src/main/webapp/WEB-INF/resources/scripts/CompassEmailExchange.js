var compassEmailExchange = compassEmailExchange || (function(){
	...
	return {
		...
		prepareEmailSending : function(context, elm, caseNo, isDraft, emailNo){
			_completeCount = 0;
				...
				success : function(resData){
					if(uploadingFiles != undefined && uploadingFiles.length > 0){
						for (var i = 0; i < uploadingFiles.length; i++){
						    ...
						    compassEmailExchange.emailAttachments(context, elm, caseNo, fd, resData, isDraft, emailNo);
						}
					}else{
						compassEmailExchange.sendEmail(context, elm, caseNo, resData, isDraft, emailNo);
					}
				}
			});
		},
		prepareEmailAsDraft : function(context, elm, caseNo, isDraft, emailNo){
			_completeCount = 0;
			//alert("sending email for draft");
			$(elm).attr("disabled", true);
			$(elm).html("Preparing to save as draft...");
			/*alert("emailNo = '"+emailNo+"'");
			alert("length = "+uploadingFiles.length);*/
			$.ajax({
				url : context+"/common/getAttachmentFolder",
				cache : false,
				type : 'POST',
		...
		deleteDraftEmail : function(context, elm, caseNo, isDraft, emailNo){
			$(elm).attr("disabled", true);
			$(elm).html("Deleting draft email...");
			/*alert("emailNo = '"+emailNo+"'");
			alert("length = "+uploadingFiles.length);*/
			var folder = "DRAFTS";
			$.ajax({
				url : context+"/common/deleteDraftEmail",
				data : {caseNo:caseNo,isDraft:isDraft,emailNo:emailNo},
				cache : false,
				type : 'POST',
				success : function(resData){
					compassEmailExchange.showAllMessage(context, caseNo, folder);
				},
		...
		emailAttachments : function (context, elm, caseNo, formData, attachmentFolder, isDraft, emailNo){
			...
			$.ajax({
				...
				success : function(resData){
					...
					if((_completeCount == uploadingFiles.length)){
		        		setTimeout(compassEmailExchange.sendEmail(context, elm, caseNo, attachmentFolder, isDraft, emailNo), 2000);
		        	}
				}
			});
		},
		sendEmail : function (context, elm, caseNo, attachmentFolder, isDraft, emailNo){
			//alert('send email');
			...
			var fullData = "caseNo="+caseNo+"&to="+to+"&cc="+cc+"&subject="+subject+"&content="+content+"&attachmentFolder="+attachmentFolder+"&isDraft="+isDraft;
			...
			$.ajax({
				...
				cache : false,
				data : {caseNo:caseNo,to:to,cc:cc,subject:subject,content:content,attachmentFolder:attachmentFolder,isDraft:isDraft,emailNo:emailNo},//fullData,
				type : 'POST',
				success : function(resData){
