var compassTopFrameFromPortal = compassTopFrameFromPortal || (function(){
	var _dateFormat;
	...
	return {
		...
		fromDate : function(compassDateFormat){
			var date = new Date();
			return compassTopFrameFromPortal.getDate(date,"D","15");
		},
		toDate : function(compassDateFormat){
			return $.datepicker.formatDate(compassDateFormat, new Date());
		},
		init : function(unqId, searchTable, compassDateFormat,isDefaultDateRequired){
			_dateFormat = compassDateFormat;
			
			//console.log("date required = "+isDefaultDateRequired);
			...
			$("table."+searchTable).find("input[id^='FROMDATE']").val(compassTopFrameFromPortal.fromDate(compassDateFormat));
			$("table."+searchTable).find("input[id^='TODATE']").val(compassTopFrameFromPortal.toDate(compassDateFormat));
			...
			$("table."+searchTable).find("input[id^='FROMDATE']").blur(function(){
				...
				if(arrLen == 3){
					...
					if(isDefaultDateRequired == undefined || isDefaultDateRequired == true){
						$("table."+searchTable).find("input[id^='FROMDATE']").val(compassTopFrameFromPortal.fromDate(compassDateFormat));
					}else if(isDefaultDateRequired == false){
						$("table."+searchTable).find("input[id^='FROMDATE']").val();
					}else{
						alert("Error While defining default from date after focus out ");
					}
			...
			$("table."+searchTable).find("input[id^='TODATE']").blur(function(){
				...
				if(arrLen == 3){
					...
					if(isDefaultDateRequired == undefined || isDefaultDateRequired == true){
						...
						$("table."+searchTable).find("input[id^='TODATE']").val();
					}else{
						alert("Error While defining default to date after focus out ");
					}
				}
			});
		},
		...
		submitForm : function(unqId, event, submitButton, moduleSearchResultPanel, moduleSearchResult, searchUrl, preloaderUrl){
			if($(submitButton).parents("div.panelSearchForm").find("input[id^='FROMDATE']").val() != undefined && 
				...
				$(submitButton).parents("div.panelSearchForm").find("input[id^='FROMDATE']").val(compassTopFrameFromPortal.fromDate(compassDateFormat));
			...
			if($(submitButton).parents("div.panelSearchForm").find("input[id^='TODATE']").val() != undefined && 
				...
				$(submitButton).parents("div.panelSearchForm").find("input[id^='TODATE']").val(compassTopFrameFromPortal.toDate(compassDateFormat));
		...
		moduleSearch : function(searchFieldId, serachFor, viewName, isMultipleSelect, contextPath){
			alert(searchFieldId+", "+serachFor+", "+viewName+", "+isMultipleSelect+", "+contextPath);
			...
			$.ajax({
				url : contextPath+"/commonFromPortal/genericModuleFieldsSearch",
				cache : false,
