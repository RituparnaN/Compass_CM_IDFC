var rfiCaseWorkFlowActionsNew = rfiCaseWorkFlowActionsNew || (function () {
	...
	return {
		...
		handleViewComments: function (elm) {
			...
			$.ajax({
				url: ctx+"/common/riskAssessmentPendingCasesNew/getCaseWorkflowModuleDetails",
				cache: false,
				type: "GET",
				data: "moduleValue=" + qId +"&compassRefNo="+crn+ "&moduleCode=" + "riskAssessmentViewCommentsNew" + "&detailPage=" + escape("RiskAssessmentPendingCasesNew/AddViewCommentsWithHistory")+"&actionType="+"viewComments",
		...
		addRows: function () {
			...
			if (rowsData.length === JSON.parse(document.getElementById("makerList").value).length) {
				...
				$("#compassGenericModal-body").html(rfiCaseWorkFlowActionsNew.getRows());
			...
			if (!isFieldEmpty) {
				if (document.getElementById("bulk").value === "bulk") {
					...
					selectedQIds.forEach(qId => {
						...
						var ctx = window.location.href;
						ctx = ctx.substring(0, ctx.split("/", 4).join("/").length);
						var url = ctx + '/common/saveRaiseToRFINew'
						var loaderUrl = ctx + '/includes/images/qde-loadder.gif';
						$("#compassGenericModal-body").html("<br/><center><img src='" + loaderUrl + "'/></center><br/>");
						...
						$.ajax({
							...
							success: function (response) {
								...
								rfiCaseWorkFlowActionsNew.checkCheck('Successfully Raised for RFI.', testArr, selectedQIds.length, testCounter,response,qId);
								
							},
							error: function (a, b, c) {
								testCounter += 1;
								rfiCaseWorkFlowActionsNew.checkCheck("Something went wrong" + a + b + c, testArr, selectedQIds.length, testCounter);
							}
						})
					})
				}
		...
		handleRaiseForRFIPage: function (elm) {
			...
			var ctx = window.location.href;
			ctx = ctx.substring(0, ctx.split("/", 4).join("/").length);
			var url = ctx + '/common/getMakerCheckerListNew'
		...
		getRows: function () {
			...
			var endHtmlString = "</tbody>" +
				...
				"				<div class='pull'>" +
				"					<input type='button' id='save' class='btn btn-success btn-sm'style='float:right' name='save' value='Save' onClick = 'rfiCaseWorkFlowActionsNew.handleRowsSave()'/>" +
				"					<input type='button' id='addRowButton' class='btn btn-primary btn-sm'style='float:right;margin-right:10px' name='addRow' value='Add Row' onClick='rfiCaseWorkFlowActionsNew.addRows()'/>" +
				"				</div>" +
				"			</div>" +
				"			</form>" +
				"			</div>" +
			...
			rowsData.forEach((element, i) => {
				var makerListString = rfiCaseWorkFlowActionsNew.getMakerList(element.id)
				var checkerListString = rfiCaseWorkFlowActionsNew.getCheckerList(element.id)
				rowsString = rowsString + "									<tr>" +
					...
					"                                        </td>" +
					"										<td width='30%'>" +
					"										    <input type='text'  " + element.editable + "  class='form-control input-sm datepicker' value = '" + element.makerDueDate + "' name='makerDueDate||" + element.id + "' onChange='rfiCaseWorkFlowActionsNew.handleInputChange(this)'/>" +
					"											" +
					"                                        </td>" +
					"                                        <td width='10%'></td>" +
					"                                        <td width='15%'>" +
					...
					"                                        </td>" +
					"										<td width='30%'>" +
					"										    <input type='text' " + element.editable + "  class='form-control input-sm datepicker'  value = '" + element.checkerDueDate + "' type='date' name='checkerDueDate||" + element.id + "' onChange='rfiCaseWorkFlowActionsNew.handleInputChange(this)'/>" +
					"											" +
					"                                        </td></tr>" +
					"                                        <tr><td width='15%'>" +
					"                                            Maker code" +
					"                                        </td>" +
					"										<td width='30%'>" +
					"                                            <select  " + element.editable + " class='form-control selectpicker' name='makerName||" + element.id + "' value = '" + element.makerName + "' onChange='rfiCaseWorkFlowActionsNew.handleInputChange(this)'>" +
					...
					"                                        </td>" +
					"										<td width='30%'>" +
					"                                            <select  " + element.editable + " id = 'checkerNames||" + element.id + "' class='form-control selectpicker' multiple='multiple' data-live-search='true' name='checkerNames||" + element.id + "'  onChange='rfiCaseWorkFlowActionsNew.handleInputChange(this)'>" +
					"                                            " + checkerListString +
					"                                            </select><br>" +
					"                                        </td></tr>" +
					"										<tr>" +
					...
					"											</td>" +
					"											<td colspan = '4'>" +
					"												<textarea  style='margin-bottom:10px'" + element.editable + "  class='form-control' placeholder = '" + element.comments + "' onKeyUp = 'rfiCaseWorkFlowActionsNew.handleInputChange(this)' name = 'comments||" + element.id + "'></textarea>" +
					"                                            	<input type='button' " + element.editable + " id='removeRowButton"+i+"' class='btn btn-danger btn-sm' style='float:right' name='removeRow' value='Remove Row' onClick='rfiCaseWorkFlowActionsNew.removeRows(" + element.id + ")' />" +
					"											</td>" +
					"										</tr>" +
					"											</tbody>" +
					"                                          </table></td>" +
