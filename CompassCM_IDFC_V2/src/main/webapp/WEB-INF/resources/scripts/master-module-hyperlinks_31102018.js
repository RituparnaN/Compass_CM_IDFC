...
function resetColumns(elm){
	...
	var hideColumn = true;
	if($("."+tableClass+"ResetCols").children("ul").children("li#"+colName).attr('index')){
		var resetColIndex = parseInt($("."+tableClass+"ResetCols").children("ul").children("li#"+colName).attr('index'));
	}else if($("."+tableClass+"ResetCols").children("ul").children("li#"+colName).attr('noindex')){
		hideColumn = false;
	}else{
		var resetColIndex = $("."+tableClass+"ResetCols").children("ul").children("li#"+colName).index();
	...
	if ($(".searchResultGenericTable:visible > thead > tr:first > th").length > 6){
		if($("."+tableClass).closest(".DTFC_LeftHeadWrapper").children().eq(0).hasClass("dataTables_wrapper")){
			$("."+tableClass).closest(".DTFC_LeftHeadWrapper").children().eq(0).remove();
		}
		if($("."+tableClass).closest(".DTFC_LeftBodyLiner").children().eq(0).hasClass("dataTables_wrapper")){
			$("."+tableClass).closest(".DTFC_LeftBodyLiner").children().eq(0).remove();
		}
		$("."+tableClass).closest('.dataTables_scrollBody thead tr').css({visibility:'collapse'});
		$("."+tableClass).closest(".dataTables_scrollBody:visible").children().eq(0).width("100%");
	}
	else{
		if($("."+tableClass).closest(".DTFC_ScrollWrapper").children().eq(2).hasClass("DTFC_LeftWrapper")){
			$("."+tableClass).closest(".DTFC_ScrollWrapper").children().eq(2).hasClass("DTFC_LeftWrapper").remove();
			$("."+tableClass).closest('.dataTables_scrollBody thead tr').css({visibility:'collapse'});
		}
		else{
		}
	...
	if($("."+tableClass).closest(".dataTable").children("tbody").children("tr").children("td").eq(0).hasClass("dataTables_empty")){
		$("."+tableClass).closest(".DTFC_LeftWrapper").children().eq(0).remove();
	}
