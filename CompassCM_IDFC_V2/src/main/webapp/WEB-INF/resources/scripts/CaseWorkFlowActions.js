var caseWorkFlowActions = caseWorkFlowActions || (function(){
	...
	return {
		...
		escalateCaseByBranchUser : function(tableParentDivId, divId){
			...
			var caseStatus = "3";
			var flagType = "Y";
			caseWorkFlowActions.hasEddQuestions(moduleValue, function(){
				var url = "/amlCaseWorkFlow/getCaseWorkflowModuleDetails?moduleCode="+moduleCode+"&moduleHeader="+moduleHeader+"&moduleValue="+moduleValue+"&detailPage="+detailPage+"&actionType="+actionType+"&caseStatus="+caseStatus+"&flagType="+flagType+"&formId="+formId;
				//var url = "/amlCaseWorkFlow/addViewComments?CaseNos="+selectedCase+"&CaseStatus=3&FlagType=Y&formId="+formId;
				caseWorkFlowActions.openModalForCaseWorkFlow("Add Comment For Case Escalation To AMLUser", url);
			});
		...
		hasEddQuestions: function(caseNo, callback){
			var hasEddQuestions = true;
			var ctx = window.location.href;
			ctx = ctx.substring(0, ctx.split("/", 4).join("/").length);
			var url = ctx+"/common/getEddQuestionCount";
			$.ajax({
				url: url,
				cache: false,
				type: "POST",
				data: {caseNo: caseNo},
				success: function(res) {
					//console.log("eddQuestionCount = "+res);
					if(res == 0){
						alert("Please add EDD Questions for case no: "+caseNo);
						hasEddQuestions = false;
					}
				},
				complete: function(){
					if(hasEddQuestions == true){
						callback();
					}
				},
				error: function(a,b,c) {
		...
		assignToBranchUserByAMLuser : function(tableParentDivId, divId){
			...
			caseWorkFlowActions.openModalForCaseWorkFlow("Add Comment For Assignment To Branch", url);
		},
		...
		viewIndianSTR : function(tableParentDivId, divId){
			...
			var selectedCase = selectedCases.substr(0,selectedCases.length-1);
			//window.open(ctx + "/common/getINDSTRReport?caseNo="+selectedCase+'&canUpdated=Y&canExported=N');
			window.open(ctx + "/common/getINDSTRReport?caseNo="+selectedCase, "Indian_STR", "");
		},
		viewIndianSTRTRF : function(tableParentDivId, divId){
			var selectedCases = "";
			$("#"+tableParentDivId).find("table").children("tbody").children("tr").each(function(){
    			var row = $(this).children("td").children("input[type='checkbox']");
    			if($(row).prop("checked")){
			...
			var selectedCase = selectedCases.substr(0,selectedCases.length-1);
			//window.open(ctx + "/common/getINDSTRTRFReport?caseNo="+selectedCase+"&canUpdated=Y&canExported=N","Indian_STR_TRF","");
			window.open(ctx + "/common/getINDSTRTRFReport?caseNo="+selectedCase,"Indian_STR_TRF","");
		},
		...
		okayByAMLO : function(tableParentDivId, divId){
			var selectedCases = "";
			$("#"+tableParentDivId).find("table").children("tbody").children("tr").each(function(){
    			var row = $(this).children("td").children("input[type='checkbox']");
    			if($(row).prop("checked")){
    		...
    		var url = "/amlCaseWorkFlow/casesToBeReviewedByAMLO?CaseNos="+selectedCase+"&CaseStatus=2000&FlagType=Y&formId="+formId+
				      "&fromDate="+CM_FROMDATE+"&toDate="+CM_TODATE+"&alertCode="+CM_ALERTCODE+"&branchCode="+CM_BRANCHCODE+"&accountNo="+CM_ACCOUNTNO+
				      "&customerId="+CM_CUSTOMERID+"&hasAnyOldCases="+CM_HASANYOLDCASES+"&caseRating="+CM_CASERATING+
				      "&fromCaseNo="+CM_FROMCASENO+"&toCaseNo="+CM_TOCASENO+"&action="+action;
			
			caseWorkFlowActions.openModalForCaseWorkFlow("Comments for Case(s) Found Okay", url);
		},
		
		toBeReviewedByAMLO : function(tableParentDivId, divId){
			var selectedCases = "";
			$("#"+tableParentDivId).find("table").children("tbody").children("tr").each(function(){
    			var row = $(this).children("td").children("input[type='checkbox']");
    			if($(row).prop("checked")){
    		...
    		var CM_FROMCASENO = $("#FROMCASENO_"+divId).val();
    		var CM_TOCASENO = $("#TOCASENO_"+divId).val();
    		var url = "/amlCaseWorkFlow/casesToBeReviewedByAMLO?CaseNos="+selectedCase+"&CaseStatus=2001&FlagType=Y&formId="+formId+
				      "&fromDate="+CM_FROMDATE+"&toDate="+CM_TODATE+"&alertCode="+CM_ALERTCODE+"&branchCode="+CM_BRANCHCODE+"&accountNo="+CM_ACCOUNTNO+
				      "&customerId="+CM_CUSTOMERID+"&hasAnyOldCases="+CM_HASANYOLDCASES+"&caseRating="+CM_CASERATING+
				      "&fromCaseNo="+CM_FROMCASENO+"&toCaseNo="+CM_TOCASENO+"&action="+action;
			
			caseWorkFlowActions.openModalForCaseWorkFlow("Comments for Case(s) To Be Reopened", url);
		},
		bonafideOkayByAMLO : function(tableParentDivId, divId){
			var selectedCases = "";
			var selectedCustIds = "";
			...
			var moduleValue = selectedCase;
			var detailPage = "AMLCaseWorkFlow/AddViewCommentsWithHistory";
			var actionType = "bonafideOkayByAMLO";
			var caseStatus = "61";
			var flagType = "Y";
			var CM_FROMDATE = $("#FROMDATE_"+divId).val();
    		var CM_TODATE = $("#TODATE_"+divId).val();
		...
		bonafideOkayByMLRO : function(tableParentDivId, divId){
			var selectedCases = "";
			var selectedCustIds = "";
			...
			var moduleValue = selectedCase;
			var detailPage = "AMLCaseWorkFlow/AddViewCommentsWithHistory";
			var actionType = "bonafideOkayByMLRO";
			var caseStatus = "62";
			var flagType = "Y";
			var CM_FROMDATE = $("#FROMDATE_"+divId).val();
    		var CM_TODATE = $("#TODATE_"+divId).val();
