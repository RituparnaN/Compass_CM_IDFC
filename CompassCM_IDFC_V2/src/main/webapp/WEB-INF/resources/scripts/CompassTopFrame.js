var compassTopFrame = compassTopFrame || (function(){
	...
	return {
		...
		init : function(unqId, searchTable, compassDateFormat,isDefaultDateRequired){
			...
			//Validation for FromDate to be greater than ToDate 
		...
		submitForm : function(unqId, event, submitButton, moduleSearchResultPanel, moduleSearchResult, searchUrl, preloaderUrl){
			...
			var fromDate = $(submitButton).parents("div.panelSearchForm").find("input[id^='FROMDATE']").val();
			var toDate = $(submitButton).parents("div.panelSearchForm").find("input[id^='TODATE']").val();
		...
		moduleSearch : function(searchFieldId, serachFor, viewName, isMultipleSelect, contextPath){
			//console.log(searchFieldId+", "+serachFor+", "+viewName+", "+isMultipleSelect+", "+contextPath);
