...
/*ACCOUNT PROFILING*/
function showProfilingDetailsInBottomTable(elm, tdValue, detailPage){
	var searchBtnId = $("div#accountProfilingDetailsDiv").find(".btn-primary").attr("id");
	var unqId = searchBtnId.split(/([0-9]+)/)[1];
	var ap_fromDate = $("#ap_fromDate"+unqId).val();
	var ap_toDate = $("#ap_toDate"+unqId).val();
	var ap_accountNo = $("#ap_accountNo"+unqId).val();
	var ap_custId = $("#ap_custId"+unqId).val();
	/*var activeTab = $("table.accountProfiling").parents("div.card-primary").find("ul.modalNav").find("li.active").children("a").html();
	if(activeTab == "Non Cash Profile Details"){
		$(elm).parents("div#accountProfilingDetailsDiv").find("div.profilingDetailsDiv3").css("display", "block");
		var mainDiv = $(elm).parents("div#accountProfilingDetailsDiv").find("div.profilingDetailsDiv3").children("div").children("div");
	}*/
	$(elm).parents("div#accountProfilingDetailsDiv").find("div.profilingDetailsDiv3").css("display", "block");
	var mainDiv = $(elm).parents("div#accountProfilingDetailsDiv").find("div.profilingDetailsDiv3").children("div").children("div");
	var ctx = window.location.href;
	...
	var loaderUrl = ctx + '/includes/images/qde-loadder.gif';
	$(mainDiv).html("<br/><center><img src='"+loaderUrl+"'/></center><br/>");
	var url = ctx + '/common/getNonCashChannelDetails';
	var formData = 'channelType='+tdValue+'&detailPage='+detailPage+'&ap_fromDate='+ap_fromDate
					+'&ap_toDate='+ap_toDate+'&ap_accountNo='+ap_accountNo+'&ap_custId='+ap_custId;
	$.ajax({
		url :  url,
		data: formData,
		cache : false,
...
function showNonCashChannelDetailsBelowTable(elm, tdValue, detailPage){
	var searchBtnId = $("div#accountProfilingDetailsDiv").find(".btn-primary").attr("id");
	var unqId = searchBtnId.split(/([0-9]+)/)[1];
	var ap_fromDate = $("#ap_fromDate"+unqId).val();
	var ap_toDate = $("#ap_toDate"+unqId).val();
	var ap_accountNo = $("#ap_accountNo"+unqId).val();
	var ap_custId = $("#ap_custId"+unqId).val();
	var channelType = $(elm).parents("div.modalBottomDetails").children("div.card-header").children("h6").html();
	
	$(elm).parents("div.modalBottomDetails").find("div.nonCashBottomDetailsDiv").css("display", "block");
	var belowDiv = $(elm).parents("div.modalBottomDetails").find("div.nonCashBottomDetailsDiv").children("div").children("div");
	var ctx = window.location.href;
	ctx = ctx.substring(0, ctx.split("/", 4).join("/").length);
	var loaderUrl = ctx + '/includes/images/qde-loadder.gif';
	$(belowDiv).html("<br/><center><img src='"+loaderUrl+"'/></center><br/>");
	var url = ctx + '/common/getNonCashTransactionDetails';
	var formData = 'counterPartyName='+tdValue+'&channelType='+channelType+'&detailPage='+detailPage+'&ap_fromDate='+ap_fromDate
					+'&ap_toDate='+ap_toDate+'&ap_accountNo='+ap_accountNo+'&ap_custId='+ap_custId;
	$.ajax({
		url :  url,
		data: formData,
...
function eventFired(_tableClass){
	...
	detailPageUrl = "KYCModules/CustomerMaster/CustomerDetails";
	var customerIdColIndex = $(table).children("thead").children("tr").children(" th#SOURCECUSTOMERID").index()+1;
	var customerIdCols = $(table).children("tbody").children("tr").children("td:nth-child("+customerIdColIndex+")");
	$(customerIdCols).each(function(a,b){
		var tdValue = $(this).html();
		
		if($(this).closest('table').find('th#SOURCECUSTOMERID').length)
		{
			$(this).addClass("hyperlinkCols");
			$(this).attr("onclick", "openDetails(this, 'Customer Details', '"+tdValue+"','customerMaster', '"+detailPageUrl+"')");
		}
	...
	detailPageUrl = "KYCModules/CustomerMaster/CustomerDetails";
	var customerIdColIndex = $(table).children("thead").children("tr").children("th#DESTINATIONCUSTOMERID").index()+1;
	var customerIdCols = $(table).children("tbody").children("tr").children("td:nth-child("+customerIdColIndex+")");
	$(customerIdCols).each(function(a,b){
		var tdValue = $(this).html();
		
		if($(this).closest('table').find('th#DESTINATIONCUSTOMERID').length)
		{
			$(this).addClass("hyperlinkCols");
			$(this).attr("onclick", "openDetails(this, 'Customer Details', '"+tdValue+"','customerMaster', '"+detailPageUrl+"')");
		}
	...
	detailPageUrl = "MasterModules/AccountsMaster/AccountDetails";
	var accountNoColIndex = $(table).children("thead").children("tr").children("th#SOURCEACCOUNTNO").index()+1;
	var accountNoCols = $(table).children("tbody").children("tr").children("td:nth-child("+accountNoColIndex+")");
	$(accountNoCols).each(function(a,b){
		var tdValue = $(this).html();
		/*$(this).addClass("hyperlinkCols");
		$(this).attr("onclick", "openDetails(this, 'Account Details', '"+tdValue+"','accountsMaster', '"+detailPageUrl+"')");*/
		if($(this).closest('table').find('th#SOURCEACCOUNTNO').length)
		{
			$(this).addClass("hyperlinkCols");
			$(this).attr("onclick", "openDetails(this, 'Account Details', '"+tdValue+"','accountsMaster', '"+detailPageUrl+"')");
		}
	...
	detailPageUrl = "MasterModules/AccountsMaster/AccountDetails";
	var accountNoColIndex = $(table).children("thead").children("tr").children("th#DESTINATIONACCOUNTNO").index()+1;
	var accountNoCols = $(table).children("tbody").children("tr").children("td:nth-child("+accountNoColIndex+")");
	$(accountNoCols).each(function(a,b){
		var tdValue = $(this).html();
		/*$(this).addClass("hyperlinkCols");
		$(this).attr("onclick", "openDetails(this, 'Account Details', '"+tdValue+"','accountsMaster', '"+detailPageUrl+"')");*/
		if($(this).closest('table').find('th#DESTINATIONACCOUNTNO').length)
		{
			$(this).addClass("hyperlinkCols");
			$(this).attr("onclick", "openDetails(this, 'Account Details', '"+tdValue+"','accountsMaster', '"+detailPageUrl+"')");
		}
	...
	$(alertSerialIdCols).each(function(a,b){
		var tdValue = $(this).html();
		var tdValue1 = $(this).closest("tr").find('td:eq(1)').text();
		var parameterType = $(this).closest("tr").find('td:eq(2)').text();
		//alert(tdValue+' & '+tdValue1+' & '+parameterType);
		$(this).addClass('alertSerialIdHyperlink');
		$(this).attr("onclick", "openAlertSerialIdDetails(this, '"+tdValue+"', '"+tdValue1+"', '"+parameterType+"')");
	});
	...
	detailPageUrl = "AlertEngines/AlertLinkedTransactions";
	var transactionsColIndex = $(table).children("thead").children("tr").children("th#TRANSACTIONS").index()+1;
	var transactionsCols = $(table).children("tbody").children("tr").children("td:nth-child("+transactionsColIndex+")");
	$(transactionsCols).each(function(a,b){
		var alertNoRow = "";
		if($(table).children("thead").children("tr").children("th#ALERTNUMBER").length){
			alertNoRow = $(table).children("thead").children("tr").children("th#ALERTNUMBER").index()+1;
		}else if($(table).children("thead").children("tr").children("th#ALERTNO").length){
			alertNoRow = $(table).children("thead").children("tr").children("th#ALERTNO").index()+1;
		}
		var alertNo = $(this).parent("tr").children("td:nth-child("+alertNoRow+")").html();//$(this).html();
		$(this).addClass("hyperlinkCols");
		$(this).attr("onclick", "openDetails(this, 'Alert Link Transaction', '"+alertNo+"','AlertLinkedTransactions', '"+detailPageUrl+"')");
	});
	...
	$(fileNameCols).each(function(a,b){
		var tdValue = $(this).html();
		var fileUrl = $(this).closest('td').next().text();
		var fileSeqNo = $(this).closest('td').prev().text();
		$(this).addClass("hyperlinkCols");
		$(this).attr("onclick", "downloadGeneratedXMLFile('"+fileSeqNo+"','"+tdValue+"','"+fileUrl+"')");
	});
	...
	detailPageUrl = "AMLCaseWorkFlow/CaseDetails";
	var mergedCaseColIndex = $(table).children("thead").children("tr").children("th#MERGEDCASE_FLAG").index()+1;
	var caseMergedCols = $(table).children("tbody").children("tr").children("td:nth-child("+mergedCaseColIndex+")");
	$(caseMergedCols).each(function(a,b){
		var tdValue = $(this).html();
		$(this).addClass("hyperlinkCols");
		$(this).attr("onclick", "openDetails(this, 'List Of Merged Cases', '"+tdValue+"','mergedCasesHistoryDetails', '"+detailPageUrl+"')");
	});
	
	detailPageUrl = "";
	var emailsQuestionnIDColIndex = $(table).children("thead").children("tr").children("th#EMAILQUESTIONID").index()+1;
	var emailQuestionsCols = $(table).children("tbody").children("tr").children("td:nth-child("+emailsQuestionnIDColIndex+")");
	$(emailQuestionsCols).each(function(a,b){
		var tdValue = $(this).html();
		$(this).addClass("hyperlinkCols");
		$(this).attr("onclick", "getEmailQuestion(this)");
	});
	
	//25/03/2019
	detailPageUrl = "";
	var seqNoColIndex = $(table).children("thead").children("tr").children("th#SEQNO").index()+1;
	var seqNoCols = $(table).children("tbody").children("tr").children("td:nth-child("+seqNoColIndex+")");
	$(seqNoCols).each(function(a,b){
		var tdValue = $(this).html();
		$(this).addClass("hyperlinkCols");
		$(this).attr("onclick", "getSeqNoDetails("+tdValue+")");
	});

	/*Removing tooltip/title from every table td*/
	//$('[data-toggle="tooltip"]').tooltip({container: 'body'});
	$(table).tooltip("disable");
	$(table).find("td").removeAttr("title");
	$(table).find("td").addClass("tdCellCustomTooltipClass");
	
	/*Account Profiling*/
	detailPageUrl = "AccountProfiling/AccountProfileDetailsBottom";
	exactTable = getExactTable(table, "NONCASHCHANNELTYPE");
	var channelTypeColIndex = $(exactTable).children("thead").children("tr").children("th#NONCASHCHANNELTYPE").index()+1;
	var channelTypeCols = $(exactTable).children("tbody").children("tr").children("td:nth-child("+channelTypeColIndex+")");
	$(channelTypeCols).each(function(a,b){
		var tdValue = $(this).html().trim();
		$(this).addClass("hyperlinkCols");
		$(this).attr("onclick", "showProfilingDetailsInBottomTable(this, '"+tdValue+"', '"+detailPageUrl+"')");
	});
	
	detailPageUrl = "AccountProfiling/NonCashChannelDetailsBottom";
	exactTable = getExactTable(table, "COUNTERPARTYNAME");
	var channelTypeColIndex = $(exactTable).children("thead").children("tr").children("th#COUNTERPARTYNAME").index()+1;
	var channelTypeCols = $(exactTable).children("tbody").children("tr").children("td:nth-child("+channelTypeColIndex+")");
	$(channelTypeCols).each(function(a,b){
		var tdValue = $(this).html().trim();
		$(this).addClass("hyperlinkCols");
		$(this).attr("onclick", "showNonCashChannelDetailsBelowTable(this, '"+tdValue+"', '"+detailPageUrl+"')");
	});
	
  //FOR RISK ASSESSMENT QUESTION DETAILS
	var questionIdColIndex = $(table).children("thead").children("tr").children("th#ASSESSMENTQUESTIONID").index()+1;
	var questionsIdCols = $(table).children("tbody").children("tr").children("td:nth-child("+questionIdColIndex+")");
	$(questionsIdCols).each(function(a,b){
		var tdValue = $(this).html();
		var statusIdRow = $(table).children("thead").children("tr").children("th#STATUS").index()+1;
		var versionIdRow = $(table).children("thead").children("tr").children("th#VERSION_SEQNO").index()+1;
		var assessmentSectionRow = $(table).children("thead").children("tr").children("th#ASSESSMENTSECTIONCODE").index()+1;
		var status = $(this).parent("tr").children("td:nth-child("+statusIdRow+")").html();
		var version = $(this).parent("tr").children("td:nth-child("+versionIdRow+")").html();
		var assessmentSection = $(this).parent("tr").children("td:nth-child("+assessmentSectionRow+")").html();
		$(this).addClass("hyperlinkCols");
		$(this).attr("onclick", "getRiskAssessmentQuestionDetails('"+tdValue+"', '"+status+"', '"+version+"', '"+assessmentSection+"')");
	});
  
	function getExactTable(table, columnId){
		return $(table).children("thead").children("tr").children("th#"+columnId).closest("table");
	}
}