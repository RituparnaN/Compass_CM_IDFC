...
<script type="text/javascript">
	$(document).ready(function() {
		...
		/*$('input[type=radio]').parent("label").on('mouseover', function(event) {
			...
			var isDisabled = $(radio).prop("checked", false);
			if(isChecked){
				if(!isDisabled){
		...
		*/
		...
		$(".saveCDDForm").click(function(){
			...
			if(CURRENTROLE == 'ROLE_BPAMAKER' && cddFormStatus == 'BPA-P')
			if($("#CUSTOMERPREVNAME"+id).val() == ''){
				alert('Formerly known as is a mandatory field.');
				return false;	
			}
	...
	*/
	...
	function screen(elm, nameType, lineNo){
		...
		if(nameType == "CUSTOMERNAME"){
			...
			var CUSTOMERALIASNAMETOSCREEN = $("#CUSTOMERALIASNAMETOSCREEN"+id).val();
			var CUSTOMERFORMARNAMETOSCREEN = $("#CUSTOMERFORMARNAMETOSCREEN"+id).val();
			fullData = "CDDFORMTYPE=CORPORATENEW&COMPASSREFERENCENO="+COMPASSREFERENCENO+"&CDDNAMETYPE="+nameType+"&CDDNAMELINENO="+lineNo+"&NAME1="+CUSTOMERNAMETOSCREEN+"&NAME2="+CUSTOMERALIASNAMETOSCREEN+"&NAME3="+CUSTOMERFORMARNAMETOSCREEN+"&NAME4=&NAME5=&DATEOFBIRTH=&PASSPORTNO=&PANNO=&ACCOUNTNO=&CUSTOMERID=&userCode=NA&BlackListCheck=Y&SelectedBlackListCheck=Y"+
			   "&CustomerDataBaseCheck=Y&RejectedListCheck=Y&EmployeeDataBaseCheck=N";
		}
		else {
			fullData = "CDDFORMTYPE=CORPORATENEW&COMPASSREFERENCENO="+COMPASSREFERENCENO+"&CDDNAMETYPE="+nameType+"&CDDNAMELINENO="+lineNo+"&NAME1="+name+"&NAME2=&NAME3=&NAME4=&NAME5=&DATEOFBIRTH=&PASSPORTNO=&PANNO=&ACCOUNTNO=&CUSTOMERID=&userCode=NA&BlackListCheck=Y&SelectedBlackListCheck=Y"+
			   "&CustomerDataBaseCheck=Y&RejectedListCheck=Y&EmployeeDataBaseCheck=N";
		}
		// alert(fullData);
		screenName(fullData);
	...
	function loadIdentificationForm(COMPASSREFERENCENO, LINENO){
		...
		$.ajax({
			...
			type : "POST",
			cache : false,
			data : "COMPASSREFERENCENO="+COMPASSREFERENCENO+"&LINENO="+LINENO+"&TYPE=CORPNEW",
			success : function(res){
	...
	function openIDF(elm){
		...
		$.ajax({
			...
			type : "POST",
			cache : false,
			data : "COMPASSREFERENCENO="+compassRefNo+"&LINENO="+lineNo+"&IDFORMTYPE="+entityType+"&FORMTYPE=CORPNEW&FORMLINENO="+formLineNo,
			success : function(res){
				$("#compassMediumGenericModal-body").html(res);
			},
			error : function(){
...
<div class="col-sm-12">
	<div class="card card-primary panel_CDDForm">
		<div class="card-header panelSlidingCDDForm${UNQID} clearfix">
			<h6 class="card-title pull-${dirL}">CDD Form for New Corporate Customer For Compass ReferenceNo : ${COMPASSREFERENCENO}</h6>
		</div>
					...
					<div class="tab-content">
						<div role="tabpanel" class="tab-pane active" id="cdd">
							...
							<table class="table table-bordered table-striped" style="margin-bottom: 0px;">
								<tbody>
									<tr>
										...
										<td width="4%">&nbsp;</td>
										<td width="15%">
											Formerly Known As<font size = "4" color = "red"><b> * </b></font>
										</td>
										<td width="33%">
									...
									<tr>
										<td colspan="5" style="text-align: left;">
											...
											<table class="table" style="margin-bottom: 0px;">
												...
												<tr>
													<td colspan="2">
														Risk Rating<font size = "4" color = "red"><b> * </b></font>
													</td>
													<td colspan="2">
							...
							<table class="table table-bordered table-striped" style="margin-bottom: 0px;" id="GeneralInformationTable">
								<tbody>
									<tr>
										<td colspan="5"> <strong>General Information</strong><font size = "4" color = "red"><b> * </b></font> </td>
									</tr>
									<tr>
										<td colspan="2" rowspan="2" style="vertical-align: middle;">
											Is Customer a Government entity or State Owned Entity (SOE) or 50% or more owned by a state or local Government?
											<br/>If Yes, please state the country
										</td>
										<td colspan="3">
											<label class="btn btn-outline btn-primary btn-sm radio-inline" for="GOVERNMENTENTITYY${UNQID}">
												<input type="radio" id="GOVERNMENTENTITYY${UNQID}" name="GOVERNMENTENTITY" <% if(CDDDisabledFieldsMap.isFieldDisabled(prefix+"GOVERNMENTENTITY")){ %> disabled <% } %> value="YES"
												<c:if test="${FORMDATA['GOVERNMENTENTITY'] eq 'YES'}">checked="checked"</c:if>
												/>
												Yes
											</label>
											
											<label class="btn btn-outline btn-primary btn-sm radio-inline" for="GOVERNMENTENTITYN${UNQID}">
												<input type="radio" id="GOVERNMENTENTITYN${UNQID}"  name="GOVERNMENTENTITY" <% if(CDDDisabledFieldsMap.isFieldDisabled(prefix+"GOVERNMENTENTITY")){ %> disabled <% } %> value="NO"
												<c:if test="${FORMDATA['GOVERNMENTENTITY'] eq 'NO'}">checked="checked"</c:if>
												/>
												No
											</label>
										</td>
									...
									<tr>
										<td colspan="3">
											<select class="form-control input-sm selectpicker" name="GOVERNMENTENTITYCOUNTRY" id="GOVERNMENTENTITYCOUNTRY${UNQID}" <% if(CDDDisabledFieldsMap.isFieldDisabled(prefix+"GOVERNMENTENTITYCOUNTRY")){ %> disabled <% } %>>
												<option value="">Select One</option>
												<c:forEach var="COUNTRYMASTER" items="${COUNTRYMASTER}">
													<option value="${COUNTRYMASTER['OPTIONVALUE']}" <c:if test="${FORMDATA['GOVERNMENTENTITYCOUNTRY'] eq COUNTRYMASTER['OPTIONVALUE']}">selected="selected"</c:if>>${COUNTRYMASTER['OPTIONNAME']}</option>
												</c:forEach>
											</select>
										</td>
									</tr>
									<tr>
										<td colspan="2" rowspan="2" style="vertical-align: middle;">
											Is Customer majority  owned by an entity that is listed on a recognized stock exchange?<br/> 
											If yes, select the country of the Stock Exchange: 
										</td>
										<td colspan="3">
											<label class="btn btn-outline btn-primary btn-sm radio-inline" for="ENTITYSTOCKEXCHANGEY${UNQID}">
												<input type="radio" id="ENTITYSTOCKEXCHANGEY${UNQID}" name="ENTITYSTOCKEXCHANGE"  <% if(CDDDisabledFieldsMap.isFieldDisabled(prefix+"ENTITYSTOCKEXCHANGE")){ %> disabled <% } %>value="YES"
												<c:if test="${FORMDATA['ENTITYSTOCKEXCHANGE'] eq 'YES'}">checked="checked"</c:if>
												/>
												Yes
											</label>
											
											<label class="btn btn-outline btn-primary btn-sm radio-inline" for="ENTITYSTOCKEXCHANGEN${UNQID}">
												<input type="radio" id="ENTITYSTOCKEXCHANGEN${UNQID}"  name="ENTITYSTOCKEXCHANGE"  <% if(CDDDisabledFieldsMap.isFieldDisabled(prefix+"ENTITYSTOCKEXCHANGE")){ %> disabled <% } %>value="NO"
												<c:if test="${FORMDATA['ENTITYSTOCKEXCHANGE'] eq 'NO'}">checked="checked"</c:if>
												/>
												No
											</label>
										</td>
									...
									<tr>
										<td colspan="3">
											<select class="form-control input-sm selectpicker" name="ENTITYSTKEXCHNGCOUNTRY" id="ENTITYSTKEXCHNGCOUNTRY${UNQID}" <% if(CDDDisabledFieldsMap.isFieldDisabled(prefix+"ENTITYSTKEXCHNGCOUNTRY")){ %> disabled <% } %>>
												<option value="">Select One</option>
												<c:forEach var="STOCKEXCHANGEMASTER" items="${STOCKEXCHANGEMASTER}">
													<option value="${STOCKEXCHANGEMASTER['OPTIONVALUE']}" <c:if test="${FORMDATA['ENTITYSTKEXCHNGCOUNTRY'] eq STOCKEXCHANGEMASTER['OPTIONVALUE']}">selected="selected"</c:if>>${STOCKEXCHANGEMASTER['OPTIONNAME']}</option>
												</c:forEach>
											</select>
										</td>
									</tr>
									<tr>
										<td colspan="2" rowspan="2" style="vertical-align: middle;">
											Is the Customer listed on a recognized stock exchange? <br/>
											If yes, select the country of the Stock Exchanges:
										</td>
										<td colspan="3">
											<label class="btn btn-outline btn-primary btn-sm radio-inline" for="CUSTOMERSTOCKEXCHANGEY${UNQID}">
												<input type="radio" id="CUSTOMERSTOCKEXCHANGEY${UNQID}" name="CUSTOMERSTOCKEXCHANGE" <% if(CDDDisabledFieldsMap.isFieldDisabled(prefix+"CUSTOMERSTOCKEXCHANGE")){ %> disabled <% } %> value="YES"
												<c:if test="${FORMDATA['CUSTOMERSTOCKEXCHANGE'] eq 'YES'}">checked="checked"</c:if>
												/>
												Yes
											</label>
											
											<label class="btn btn-outline btn-primary btn-sm radio-inline" for="CUSTOMERSTOCKEXCHANGEN${UNQID}">
												<input type="radio" id="CUSTOMERSTOCKEXCHANGEN${UNQID}"  name="CUSTOMERSTOCKEXCHANGE" <% if(CDDDisabledFieldsMap.isFieldDisabled(prefix+"CUSTOMERSTOCKEXCHANGE")){ %> disabled <% } %> value="NO"
												<c:if test="${FORMDATA['CUSTOMERSTOCKEXCHANGE'] eq 'NO'}">checked="checked"</c:if>
												/>
												No
											</label>
										</td>
									...
									<tr>
										<td colspan="3">
											<select class="form-control input-sm selectpicker" name="CUSTOMERSTKEXCHNGCOUNTRY" id="CUSTOMERSTKEXCHNGCOUNTRY${UNQID}" <% if(CDDDisabledFieldsMap.isFieldDisabled(prefix+"CUSTOMERSTKEXCHNGCOUNTRY")){ %> disabled <% } %>>
												<option value="">Select One</option>
												<c:forEach var="STOCKEXCHANGEMASTER" items="${STOCKEXCHANGEMASTER}">
													<option value="${STOCKEXCHANGEMASTER['OPTIONVALUE']}" <c:if test="${FORMDATA['CUSTOMERSTKEXCHNGCOUNTRY'] eq STOCKEXCHANGEMASTER['OPTIONVALUE']}">selected="selected"</c:if>>${STOCKEXCHANGEMASTER['OPTIONNAME']}</option>
												</c:forEach>
											</select>
										</td>
									</tr>
									<tr>
										<td colspan="2" rowspan="2" style="vertical-align: middle;">
											Is Customer a financial institution other than bank that is regulated by a regulatory authority in a country?<br/> 
											If yes, please state the country
										</td>
										<td colspan="3">
											<label class="btn btn-outline btn-primary btn-sm radio-inline" for="OTHERREGULATORYY${UNQID}">
												<input type="radio" id="OTHERREGULATORYY${UNQID}" name="OTHERREGULATORY" <% if(CDDDisabledFieldsMap.isFieldDisabled(prefix+"OTHERREGULATORY")){ %> disabled <% } %> value="YES"
												<c:if test="${FORMDATA['OTHERREGULATORY'] eq 'YES'}">checked="checked"</c:if>
												/>
												Yes
											</label>
											
											<label class="btn btn-outline btn-primary btn-sm radio-inline" for="OTHERREGULATORYN${UNQID}">
												<input type="radio" id="OTHERREGULATORYN${UNQID}"  name="OTHERREGULATORY" <% if(CDDDisabledFieldsMap.isFieldDisabled(prefix+"OTHERREGULATORY")){ %> disabled <% } %> value="NO"
												<c:if test="${FORMDATA['OTHERREGULATORY'] eq 'NO'}">checked="checked"</c:if>
												/>
												No
											</label>
										</td>
									...
									<tr>
										<td colspan="3">
											<select class="form-control input-sm selectpicker" name="OTHERREGULATORYCOUNTRY" id="OTHERREGULATORYCOUNTRY${UNQID}" <% if(CDDDisabledFieldsMap.isFieldDisabled(prefix+"OTHERREGULATORYCOUNTRY")){ %> disabled <% } %>>
												<option value="">Select One</option>
												<c:forEach var="COUNTRYMASTER" items="${COUNTRYMASTER}">
													<option value="${COUNTRYMASTER['OPTIONVALUE']}" <c:if test="${FORMDATA['OTHERREGULATORYCOUNTRY'] eq COUNTRYMASTER['OPTIONVALUE']}">selected="selected"</c:if>>${COUNTRYMASTER['OPTIONNAME']}</option>
												</c:forEach>
											</select>
										</td>
									</tr>
									<tr>
										<td colspan="2"> Was there face-to-face meeting with Customer? (To attach evidence, Branch Compliance Team, to determine type of evidences required) </td>
										<td colspan="3">
									...
									<tr>
										...
										<td colspan="3">
											<label class="btn btn-outline btn-primary btn-sm radio-inline" for="FRGNCRSPNDNTBNKCSTOMRHRY${UNQID}">
												<input type="radio" id="FRGNCRSPNDNTBNKCSTOMRHRY${UNQID}" name="FRGNCRSPNDNTBNKCSTOMRHR" <% if(CDDDisabledFieldsMap.isFieldDisabled(prefix+"FRGNCRSPNDNTBNKCSTOMRHR")){ %> disabled <% } %> value="YES"
											...
											<label class="btn btn-outline btn-primary btn-sm radio-inline" for="FRGNCRSPNDNTBNKCSTOMRHRN${UNQID}">
												<input type="radio" id="FRGNCRSPNDNTBNKCSTOMRHRN${UNQID}"  name="FRGNCRSPNDNTBNKCSTOMRHR" <% if(CDDDisabledFieldsMap.isFieldDisabled(prefix+"FRGNCRSPNDNTBNKCSTOMRHR")){ %> disabled <% } %> value="NO"
								...
								<tbody>
									<tr style="background-color: #BBB;">
										...
										<td colspan="1" style="text-align: center;">
											...
											<button type="button" class="btn btn-success btn-xs" id="uploadAlertsRatingMapping${UNQID}">Upload / View Beneficial Owners Structure</button>
									...
									<tr style="background-color: #BBB;">
										...
										<td colspan="1" style="text-align: center;">
											<c:if test="${(FORMDATA['STATUS'] eq 'BPA-P' && CURRENTROLE eq 'ROLE_BPAMAKER') || (FORMDATA['STATUS'] eq 'BPD-P' && CURRENTROLE eq 'ROLE_BPDMAKER')}">
												...
												<% } else { %>
												<button type="button" class="btn btn-primary btn-xs cddModifyButton" id="addIntermediaries${UNQID}" disabled>Add Intermediary</button>
												<% } %>
											</c:if>
										</td>
									</tr>
						...
						<jsp:include page="corpNew_Part2.jsp" />