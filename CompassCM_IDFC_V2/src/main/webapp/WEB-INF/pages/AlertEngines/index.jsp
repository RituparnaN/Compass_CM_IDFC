...
<script type="text/javascript">
	$(document).ready(function() {
		...
		compassTopFrame.init(id, 'alertEnginesTable'+id, 'dd/mm/yy',isDefaultDateRequired);	

		$('#FROMDATE_'+id).val('');
		$('#TODATE_'+id).val('');
		
		$('.panelSlidingAlertEngines'+id).on("click", function (e) {
			var mainRow = $(this).parents(".compassrow"+id);
			compassTopFrame.searchPanelSliding(id, mainRow, 'alertEnginesSerachResultPanel');
	    });
		
		$("#searchMasterForm"+id).submit(function(e){
			var submitButton = $("#searchAlertEngine"+id);
			
			if(($('#FROMDATE_'+id).val() == '' || $('#TODATE_'+id).val() == '') && $('#CUSTOMERID_'+id).val() == '' &&
			   $('#ACCOUNTNO_'+id).val() == '' && $('#ALERTNO_'+id).val() == ''){
				alert('Select any of the following FromDate/ToDate/AccountNo/AlertNo/CustomerID to search.');
			}else{	
			if($('#FROMDATE_'+id).val() == '' || $('#TODATE_'+id).val() == ''){
				//console.log("date null");
				if($('#CUSTOMERID_'+id).val().length == '0' && $('#ACCOUNTNO_'+id).val().length == '0' 
					&& $('#ALERTNO_'+id).val().length == '0'){
					alert("Please enter any of AccountNo/AlertNo/CustomerID to search.");
					$("#alertEnginesSerachResultPanel"+id).css("display","none");
				}else{
					compassTopFrame.submitForm(id, e, submitButton, 'alertEnginesSerachResultPanel', 
					'alertEnginesSerachResult', '${pageContext.request.contextPath}/common/searchGenericMaster',
...
<div class="row compassrow${UNQID}">
	<div class="col-sm-12">
		<div class="card card-primary panel_alertEngines">
			<div class="card-header panelSlidingAlertEngines${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}"><spring:message code="app.common.alertSearchHeader"/></h6>
			...
			<div class="card-search-card" >
				<table class="table table-striped formSearchTable alertEnginesTable${UNQID}" style="margin-bottom: 0px;">
					<tbody>
			...
			<div class="card-footer clearfix">
				<div class="pull-${dirR}">
					<button  type="submit" id="searchAlertEngine${UNQID}" class="btn btn-success btn-sm"><spring:message code="app.common.searchButton"/></button>
		...
		<div class="card card-primary" id="alertEnginesSerachResultPanel${UNQID}" style="display: none;">
			<div class="card-header panelSlidingAlertEngines${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}"><spring:message code="app.common.alertResultHeader"/></h6>
				<div class="btn-group pull-${dirR} clearfix">
					<span class="pull-right"><i class="collapsable fa fa-chevron-up"></i></span>
				</div>
			</div>
			<div id="alertEnginesSerachResult${UNQID}"></div>
		</div>
	</div>
</div>