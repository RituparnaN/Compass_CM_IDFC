...
<script type="text/javascript">
	$(document).ready(function(){
		...
		compassDatatable.construct(tableClass, "AlertedTransactionDetails", true);
		...
		$("#transactionButton").click(function(){
			var accountNo = $("#compassGenericModal-body").find("div.card-body").find("input#moduleValue"+id).val();
			var url = "${pageContext.request.contextPath}/common/transactionDetails?moduleType=transactionDetailsMaster&accountNo="+accountNo+"&callFromAccountsMaster=Yes";
			window.open(url,'TransactionDashBoard','height=600px,width=1000px');
					...
					<div class="col-sm-6">
						<div class="card-body" style="text-align: center; padding: 5px 0px;">
							...
							<table class="table compassModuleDetailsSearchTable" style="margin-bottom: 0px;">
								<tr>
									<td width="30%">Alert Number</td>
									<td width="40%"><input type="text" class="form-control input-sm" name="moduleValue" id="moduleValue${UNQID}" value="${moduleValue}" disabled/></td>
	...
	<ul class="nav nav-pills modalNav" role="tablist">
		<c:forEach var="tab" items="${TABNAMES}" varStatus="tabIndex">
			<li role="presentation">
				<a class="subTab nav-link <c:if test='${tabIndex.index == 0}'>active</c:if>" aria-controls="tab" role="tab" data-toggle="tab">${tab}</a>
			</li>
		</c:forEach>
	</ul>
