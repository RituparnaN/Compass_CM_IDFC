...
<script type="text/javascript">
	$(document).ready(function(){
		...
		compassDatatable.construct(tableClass, "AlertDetails", true);
		...
		if($("#alertDetailsSearchFrame"+id).parents("div#caseAlertDetailsDiv").attr("class") != undefined){
			$("#alertDetailsSearchFrame"+id).css("display", "none");
		}
	});
	
	function showAlertAttachment(){
		var id = '${UNQID}';
		var caseNo = $("#ASSIGNEDCASENO"+id).val();
		compassFileUpload.init("attachEvedence","${pageContext.request.contextPath}","5678","0","N","N",caseNo);
	}
</script>
<div class="container" style="width: 100%;">
	<div class="row" id="alertDetailsSearchFrame${UNQID}">
		<div class="col-sm-12">
					...
					<div class="col-sm-6">
						<div class="card-body" style="text-align: center; padding: 5px 0px;">
							...
							<table class="table compassModuleDetailsSearchTable" style="margin-bottom: 0px;">
								<tr>
									<td width="30%"><spring:message code="app.common.ALERTNO"/></td>
										...
										<c:choose>
											<c:when test="${LABELSITRCOUNT % 2 == 0}">
												<tr>
													<td width="15%">
														<c:choose>
															<c:when test="${ALLLABELSMAP.key eq 'ATTACHEDFILESCOUNT'}">
																<fmt:parseNumber var="attachCount" type="number" value="${ALLLABELSMAP.value}"/>
																<c:choose>
																	<c:when test="${attachCount > -1}">
																		<a href="javascript:void(0)" onclick="showAlertAttachment()"><spring:message code="${ALLLABELSMAP.key}"/></a>
																	</c:when>
																	<c:otherwise>
																	    <spring:message code="${ALLLABELSMAP.key}"/>
																	</c:otherwise>
																</c:choose>
													...
													<%-- 
													<td width="30%">
														<input type="text" id="${ALLLABELSMAP.key}${UNQID}" class="form-control input-sm" value="${ALLLABELSMAP.value}" readonly="readonly"/>
													</td>
													 --%>
													<c:choose>
														<c:when test="${f:contains(ALLLABELSMAP.key,'COMMENTS')}">
															<td width="30%">
																<textarea class="form-control input-sm viewCommentsTextarea" id="${ALLLABELSMAP.key}${UNQID}" readonly="readonly">${ALLLABELSMAP.value}</textarea>
															</td>
														</c:when>
														<c:otherwise>
															<td width="30%">
																<input type="text" class="form-control input-sm" id="${ALLLABELSMAP.key}${UNQID}" value="${ALLLABELSMAP.value}" readonly="readonly"/>
															</td>
														</c:otherwise>
														</c:choose>
													<td width="10%">&nbsp;</td>
