	...
	$("#saveMapping"+id).click(function(){
		...
		$("#reportingUsersSerachResult"+id).find("table").children("tbody").children("tr").each(function(){
			...
			if($(row).prop("checked")){
				...
				fullData = fullData + "checkbox="+isChecked+"--USERCODE="+userCode+"--REPORTINGUSERCODE="+reportingUserCode+"--REVIEWERSCODE="+reviewersCode+";";
				selectedCount++;
			}
		});
		//alert("fullData="+fullData);
		 if (selectedCount == 0){
			...
			if(statusSelected == "P"){
				...
				if(!makerComments == ""){
					//alert("fullData="+fullData+"makerComments="+makerComments);
					$.ajax({
