...
<script type="text/javascript">
	$(document).ready(function() {
		var id = '${UNQID}';
		compassTopFrame.init(id, 'customerMasterTable', 'dd/mm/yy');
		
		$('.panelSlidingCustomerMaster'+id).on("click", function (e) {
			var mainRow = $(this).parents(".compassrow"+id);
			compassTopFrame.searchPanelSliding(id, mainRow, 'customerMasterSearchResultPanel');
	    });
		
		$("#searchMasterForm"+id).submit(function(e){
			var submitButton = $("#searchCustomerMaster"+id);
			if($('#CUSTOMERID_'+id).val() == ''  && $('#ACCOUNTNO_'+id).val() == ''  && $('#CUSTOMERNAME_'+id).val() == ''
			   && $('#PANNO_'+id).val() == '' && $('#MOBILENO_'+id).val() == ''  && $('#EMAILID_'+id).val() == ''){ 
			  /*  && $('#BRANCHCODE_'+id).val() == 'ALL' && $('#RISKRATING_'+id).val() == 'ALL' && $('#CUSTOMERTYPE_'+id).val() == 'ALL'
			   && $('#USINDICIA_'+id).val() == 'NONE' */
				alert('Please select atleast one of the parameters to search.');
			}else{
				compassTopFrame.submitForm(id, e, submitButton, 'customerMasterSearchResultPanel', 
					'customerMasterSearchResult', '${pageContext.request.contextPath}/common/searchGenericMaster',
					'${pageContext.request.contextPath}/includes/images/qde-loadder.gif');
			}
		});
	});
	
	function uploadManualRiskFile(elm){
		compassFileUpload.init("uploadFile","${pageContext.request.contextPath}","uploadManualRiskFile","0","Y","N","");
	}

</script>
<div class="row compassrow${UNQID}">
	<div class="col-sm-12">
		<div class="card card-primary panel_customerMaster">
			<div class="card-header panelSlidingCustomerMaster${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}"><spring:message code="app.common.customerSearchHeader"/></h6>
			...
			<input type="hidden" name="bottomPageUrl" value="KYCModules/CustomerMaster/SearchBottomPage">
			<div class="card-search-card" >
				<table class="table table-striped formSearchTable customerMasterTable" style="margin-bottom: 0px;">
					<tbody>
			...
			<div class="card-footer clearfix">
				<div class="pull-${dirR}">
					<button type="submit" id="searchCustomerMaster${UNQID}" class="btn btn-success btn-sm"><spring:message code="app.common.searchButton"/></button>
					<c:if test="${f:containsIgnoreCase(ROLE,'AMLO') || f:containsIgnoreCase(ROLE,'MLROL1')}">
						<button type="button" id="uploadManualRiskFile${UNQID}" class="btn btn-primary btn-sm"  onclick="uploadManualRiskFile(this)">Upload ManualRisk File</button>
				</c:if>
				</div>
			</div>
			</form>
		...
		<div class="card card-primary" id="customerMasterSearchResultPanel${UNQID}" style="display: none;">
			<div class="card-header panelSlidingCustomerMaster${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}"><spring:message code="app.common.customerResultHeader"/></h6>
				<div class="btn-group pull-${dirR} clearfix">
					<span class="pull-right"><i class="collapsable fa fa-chevron-up"></i></span>
				</div>
			</div>
			<div id="customerMasterSearchResult${UNQID}"></div>
		</div>
	</div>
</div>
