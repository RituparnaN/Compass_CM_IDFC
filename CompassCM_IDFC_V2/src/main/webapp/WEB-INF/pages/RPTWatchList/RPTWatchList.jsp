...
<script type="text/javascript">
	$(document).ready(function() {
		var id = '${UNQID}';
		compassTopFrame.init(id, 'rptWatchList'+id, 'dd/mm/yy');
		
		$('.panelSlidingRPTWatchList'+id).on("click", function (e) {
			var mainRow = $(this).parents(".compassrow"+id);
			compassTopFrame.searchPanelSliding(id, mainRow, 'panelSlidingRPTWatchList');
	    });
		
		$("#searchMasterForm"+id).submit(function(e){
			var submitButton = $("#searchRPTWatchList"+id);
			compassTopFrame.submitForm(id, e, submitButton, 'panelSlidingRPTWatchList', 
					'RPTWatchListSerachResult', '${pageContext.request.contextPath}/common/searchGenericMaster',
					'${pageContext.request.contextPath}/includes/images/qde-loadder.gif');
		});
		
		$("#addRPTWatchList"+id).click(function(){
			var LISTCODE = $("#RPTLISTCODE_"+id).val();
			var LISTNAME = $("#LISTNAME_"+id).val();
			var LISTTYPE = $("#LISTTYPE_"+id).val();
			...
			$.ajax({
				url: "${pageContext.request.contextPath}/rptmaker/createNewRPTWatchList",
				cache: false,
				type: "POST",
				data : "LISTCODE="+LISTCODE+"&LISTNAME="+LISTNAME+"&LISTTYPE="+LISTTYPE+"&DESCRIPTION="+DESCRIPTION,
				success: function(res){
					alert(res);
					$("#searchMasterForm"+id).submit();
				},
	...
	function openRPTWatchListDetails(elm){
		var listCode = $(elm).html();
		$("#compassMediumGenericModal").modal("show");
		$("#compassMediumGenericModal-title").html("RPT WatchList Details");
		$.ajax({
...
<div class="row compassrow${UNQID}">
	<div class="col-sm-12">
		<div class="card card-primary panel_RPTWatchList">
			<div class="card-header panelSlidingRPTWatchList${UNQID} clearfix">
			...
			<input type="hidden" name="bottomPageUrl" value="RPTWatchList/SearchBottomPage">
			<div class="card-search-card" >
				<table class="table table-striped formSearchTable rptWatchListTable${UNQID}" style="margin-bottom: 0px;">
					<tbody>
			...
			<div class="card-footer clearfix">
				<div class="pull-${dirR}">
					<button  type="submit" id="searchRPTWatchList${UNQID}" class="btn btn-success btn-sm"><spring:message code="app.common.searchButton"/></button>
					<security:authorize access="hasRole('ROLE_RPTMAKER')">
						<button  type="button" id="addRPTWatchList${UNQID}" class="btn btn-primary btn-sm"><spring:message code="app.common.createButton"/></button>
					</security:authorize>
				</div>
			</div>
			</form>
		...
		<div class="card card-primary" id="panelSlidingRPTWatchList${UNQID}" style="display: none;">
			<div class="card-header panelSlidingRPTWatchList${UNQID} clearfix">
			...
			<div id="RPTWatchListSerachResult${UNQID}"></div>
		</div>
	</div>
</div>