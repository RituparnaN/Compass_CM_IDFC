...
<body style="margin:5px; overflow-y: scroll;">
	<div class="card card-default card-primary">
		<div class="col-sm-12 card-header" id="createRulePanelHeader" style="cursor:pointer;">
			<p style=" float: left;margin:0px;">Create Risk Rating Rule</p>
			<p id="ruleCountRows" style=" float:right;margin:0px;">1</p>
		</div>
		<div class="card-body" id="setRulePanelDiv" style="max-height:80%; overflow-y:auto; ">
			<table class="table customTable" id="ConditionDetailsTable" border="1px" >
				<tr class="collapseRow" style="height: 10px; background-color: #337ab7; cursor: pointer;"><td colspan="6"></td></tr>
				<tr>
					<td width="10%">
						<select class="form-control input-sm conOperatorKeyword" name = "condtionOperator" >
						...
						<br>
						<select class="form-control input-sm firstBracesSelectBox"  name = "firstBracesSelectBox" > 
					...
					<td width="22%" >
						<select class="form-control input-sm leftObject" onchange="getLeftObjectColumns(this)" style="margin-bottom:22px;">
					...
					<td width="8%">
						<select class="form-control" name = "secondBracesSelectBox" >
							<option>Braces</option>
							<option value = ")">)</option>
							<option value = "(">(</option>
						</select>
		...
		<div class="card-footer clearfix">
			<div class="pull-${dirR}">
				...
				<button type="button" class="btn btn-icon" id = "closeNewRuleCreationWindow" onClick="window.close();"  title="Close Window">
	...
	<div class="card card-default card-primary" style="display:none;" id = "createdRulePanel"> 
	...
	<!-- modal for saving final rules... -->
		...
		<div class="card card-default card-primary ">
			<div class="card-header ">
				...
				<h4 class="modal-title" id="mysaveRuleModalLabel">Save Rule</h4>
			</div>
				 	...
				 	<tbody>
				 		<tr>
				 			...
				 			<td width="25%">
				 				...
				 				<input type="text" class='form-control input-sm' id="ruleCode" name="ruleCode" disabled/>
				 			 </td>
				 			 <td width="10%"></td>
				 		...
				 		<tr>
				 			<td>Comments</td>
				 			<td colspan="4">
				 				<textarea rows="4" id="makerComments" class="form-control input-sm textareaResize"></textarea>
				 			</td>
				 		</tr>
				 	</tbody>
				 </table>
...
<table style="display:none" id="cloningTable">
	...
	<tr>
		...
		<td width="30%">
			<div class="leftObjectFiledValueDiv"></div>
			<div class="rightObjectAndFFieldsDIv" style="display:none">
				<select class="form-control rightObject" onchange="getRightObjectColumns(this)" style="margin-bottom:22px;">
				 	<option>Select Object</option>
						<c:forEach var = "objects" items = "${BUSINESSOBJECTS}">
							<option value = "${objects.TABLENAME}">${objects.DISPLAYNAME} </option>
						</c:forEach>
...
<script type = "text/javascript">
	...
	function getCRPTableColumnValue(elm){
		...
		var str = "<select class='form-control selectedColumnValues selectpicker' name='columnValues[]' id='columnValues' multiple='multiple' data-size='5' style='height:80px; overflow: auto;'>";
		//str += "<option value=''>Select</option>";
		$.ajax({
	...
	function viewRuleFun(hideSetRulePanelDiv,hideCreatedRulePanelDiv){
		var tr = $("table#ConditionDetailsTable > tbody > tr");
		 ...
		 $(tr).each(function(indexTr){
			if(indexTr % 2 != 0){
				...
				$(td).each(function(indexTd){
					if(indexTd == 0){	
						...
						if(conditionOp != "" && conditionOp != null && conditionOp != "Condition"){
							rule += "  "+conditionOp;
						}
						bracesValue = $(td).find('select[name=firstBracesSelectBox] option:selected').val();
						if(bracesValue != "" && bracesValue != null && bracesValue != "Braces"){
							rule += "  "+bracesValue;
						}
					}else if(indexTd == 1){
						if($(this).children().eq(1).val() != "" && $(this).children().eq(1).val() != null){
							...
							let operatorSelectBoxVal = $(this).children().eq(0).val();
							if(operatorSelectBoxVal != "Operators" && operatorSelectBoxVal != "" ){
						...
						let secondBraceSelectBox =  $(this).children().eq(0).val();
						if(secondBraceSelectBox != "" && secondBraceSelectBox != "Braces"){
							secondBracesValue = secondBraceSelectBox;
						}else{
							secondBracesValue = "";
						}
					 }
			...
			rule += "  "+fieldName+" "+ operator+" "+rightObjFieldOrValue+" "+secondBracesValue;
			//alert(rule);
			}
		});
		 if(!check){
	...
	$(document).on("click",".deleteRows",function() {
		...
		countNoumerOfRulesRows("DELETE");
	});
	
	function countNoumerOfRulesRows(operation){
		var count = $("#ruleCountRows").text();
		if(operation == "DELETE"){
			count--;
			$("#ruleCountRows").text(count);
		}
		if(operation == "ADD"){
			count++;
			$("#ruleCountRows").text(count);
	...
	/* //for collapsing 
	$(document).on("click","#ConditionDetailsTable tbody tr:nth-child(1) ",function(e) {
		var mainTr = $(this);
		//alert(parseInt($(this).css('borderTopWidth'))+"   "+e.offsetY);
		if(e.offsetY <= parseInt($(this).css('borderTopWidth'))){
		    $(mainTr).next("tr").slideToggle(800);
		  //  alert($(mainTr).next("tr"));
		      // $(mainTr).next("tr").animate({
		     //       height: "1px"
		     //   }); 
		  // }
	}); */ 
	...
	function closeThisWindow(){
		window.opener.document.getElementById("flagFromNewRuleWindow").value = 'closingWindowAfterSavingRule';
		window.close();
	};
