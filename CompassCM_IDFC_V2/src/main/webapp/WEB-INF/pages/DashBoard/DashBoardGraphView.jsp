...
//for constructing data set for amluser wise buckets
	...
	$.each(constructDataForAmlUserWiseRecordStats(), function(key,value) {
		...
		labelString += "<tr>";
		labelString += "<td>";
		labelString += "<div class='col-sm-12' style='padding: 0; display:flex'>";
...
//Data Total Summary	
	...
	<c:forEach var="totalSummaryPieGraphHeader" items="${DashBoardData['TotalSummary']['listResultHeader'][0]}">
		totalSummary['name'].push("${f:substring(totalSummaryPieGraphHeader,11,totalSummaryPieGraphHeader.length())}");
	</c:forEach>
	<c:forEach var="totalSummaryPieGraphData" items="${DashBoardData['TotalSummary']['listResultData'][0]}">
		totalSummary['value'].push("${totalSummaryPieGraphData}");
	</c:forEach>
	
	/* //Modified the array to remove totalAssigned Element
	totalSummary['name'].shift();
	totalSummary['value'].shift(); */
	
	var totalSummaryArray = [totalSummary['name'], totalSummary['value']];
//console.log(totalSummary['name'],totalSummary['value']);
...
<div class="col-sm-12" style="margin-top:10px; padding: 0 0;">
	<div class="col-sm-12" id="graphPanelDiv" style="display: flex">
		<div class="col-sm-6 totalSummaryGraphDiv graphDiv" style="float: left; display: flex;">
			<div class="col-sm-11" >
		...
		<div class="col-sm-6 userwiseGraphDiv graphDiv" style="float: right; height: 310px; overflow-y: auto; display: flex;" >
	...
	<div class="col-sm-12" id="graphPanelDiv2" style="display: flex">
		<div class="col-sm-6 amlUserSummaryBarGraphDiv graphDiv" style="float: left; display: flex; ">
			<div class="col-sm-11" >
		...
		<div class="col-sm-6 amlUserSummarySubBarGraphDiv graphDiv" style="float: right; height: max-content; display: flex;">
			<div class="col-sm-12" id="textDiv" style="text-align: center; font-family: Helvetica; display: flex;">
	...
	<div class="col-sm-12" id="graphPanelDiv">
		<div class="col-sm-6 totalSummaryGraphDiv graphDiv" style="float: left; display: flex; width: inherit;">
			<div class="col-sm-11" >
				<canvas class="canvasDiv" id ="totalSummaryPieGraph"></canvas>
			</div>
			<div class="col-sm-1 " onmouseleave="downloadButtonLeaved(this)" style="padding:0px;">
				<button type="button" class="btn btn-sm dropdownBtn" data-toggle="dropdown" aria-expanded="false" 
						onmouseenter="downloadButtonHovered(this, 'totalSummaryPieGraph', 'totalSummaryArray', 'Total Summary')">
					<i class="fa fa-download"></i>
				</button>
			</div>
		</div>
...
<div class="col-sm-12" style="margin-top:300px; padding: 0;">
					...
					<ul class="nav nav-pills modalNav" role="tablist" id="dashBoardTab">
						...
						%>
							<li role="presentation">
								<a class="subTab nav-link <c:if test='${tabIndex.index == 0}'>active</c:if>" href="#dashBoardTab<%=tabIndex%>" aria-controls="dashBoardTab<%=tabIndex%>" role="tab" data-toggle="tab"><%=tabNameItr.next()%></a>
							</li>
						<%
							tabIndex++;
						}
...
var totalSummaryPieGraph = document.getElementById("totalSummaryPieGraph");
...
var myTrayDoughnutChart = new Chart(myTrayDoughnutGraph, {
	    ...
	    options: {
	     ...
	     cutoutPercentage : 60, //for doughnut
	     legend: {
	        display: true,
	       	usePointStyle: true,
	        position: 'right',
...
//TotalSummary starts
var totalSummaryPieChart = new Chart(totalSummaryPieGraph, {
		type: 'pie',
	    data: {
	      labels: totalSummary['name'],
	      datasets: [{
	        label: "Value",
	        labelColor : 'yellow',
	        backgroundColor: ["#A52A2A", "#D2B48C", "#90EE90", "#E6E6FA", "#008B8B", "#E05958",   "#FA8072", "#A9C54D", "#87CEFA", "#00CED1", "#8A2BE2", "#FF6347", "#B0E0E6", "#FF69B4"],
	        data: totalSummary['value']
	      }]
	    },
	    options: {
	      responsive: true,
...
totalSummaryPieGraph.onclick = function(evt){
    var activePoints = totalSummaryPieChart.getElementAtEvent(evt);
    if (activePoints && activePoints.length) {
    	var title = "Details for Total Summary = "+ totalSummary['name'][activePoints[0]["_index"]];
		searchInputParam["columnName"] = "TOTALSUMMARY" ;
		searchInputParam["columnValue"] = totalSummary['name'][activePoints[0]["_index"]];
		searchInputParam["moduleType"] = "dynamicResult";
		getGenericGraphDetails(title,searchInputParam); 
    }
};
