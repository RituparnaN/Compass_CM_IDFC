...
<script type="text/javascript">
	$(document).ready(function() {
		var id = '${UNQID}';
		compassTopFrame.init(id, 'swiftAdminPendingCaseTable'+id, 'dd/mm/yy');
		
		$('.panelSlidingSwiftAdminPendingCase'+id).on("click", function (e) {
			var mainRow = $(this).parents(".compassrow"+id);
			compassTopFrame.searchPanelSliding(id, mainRow, 'swiftAdminPendingCaseSerachResultPanel');
	    });
		
		$("#searchMasterForm"+id).submit(function(e){
			var submitButton = $("#searchSwiftAdminPendingCase"+id);
			compassTopFrame.submitForm(id, e, submitButton, 'swiftAdminPendingCaseSerachResultPanel', 
					'swiftAdminPendingCaseSerachResult', '${pageContext.request.contextPath}/swiftcommon/swiftWorkFlow/searchSwiftMessages',
		...
		$("#attachEvedence"+id).click(function(){
			var caseNo = "";
			var count = 0;
			$("#swiftAdminPendingCaseSerachResult"+id).find("table").children("tbody").children("tr").each(function(){
			...
			compassFileUpload.init("attachEvedence","${pageContext.request.contextPath}","5678","swiftAdminPendingCaseSerachResult"+id,"Y","Y");
			}
		});
...
<div class="row compassrow${UNQID}">
	<div class="col-sm-12">
		<div class="card card-primary panel_swiftAdminPendingCase">
			<div class="card-header panelSlidingSwiftAdminPendingCase${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}"><spring:message code="app.common.swiftAdminPutOnholdCaseSearchHeader"/></h6>
			...
			<input type="hidden" name="bottomPageUrl" value="SwiftModules/SwiftWorkFlow/SwiftAdmin/PutOnholdMessages/SearchBottomPage">
			<div class="card-search-card" >
				<table class="table table-striped formSearchTable swiftAdminPendingCaseTable${UNQID}" style="margin-bottom: 0px;">
					<tbody>
			...
			<div class="card-footer clearfix">
				<div class="pull-${dirR}">
					<button  type="submit" id="searchSwiftAdminPendingCase${UNQID}" class="btn btn-success btn-sm"><spring:message code="app.common.searchButton"/></button>
				</div>
			</div>
			</form>
			</div>
		</div>
		<div class="card card-primary" id="swiftAdminPendingCaseSerachResultPanel${UNQID}" style="display: none;">
			<div class="card-header panelSlidingSwiftAdminPendingCase${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}"><spring:message code="app.common.swiftAdminPutOnholdCaseResultHeader"/></h6>
				<div class="btn-group pull-${dirR} clearfix">
					<span class="pull-right"><i class="collapsable fa fa-chevron-up"></i></span>
				</div>
			</div>
			<div id="swiftAdminPendingCaseSerachResult${UNQID}"></div>
			
			<div class="card-footer action-footer clearfix">
				<div class="pull-${dirR} clearfix">
					
					<!--
					<button type="button" class="btn btn-primary btn-sm" onclick="caseWorkFlowActions.swiftAdminPutOnHold('swiftAdminPendingCaseSerachResult${UNQID}','${UNQID}')">Put OnHold</button>
                    -->
					<button type="button" class="btn btn-primary btn-sm" onclick="caseWorkFlowActions.swiftAdminCleanMessage('swiftAdminPendingCaseSerachResult${UNQID}','${UNQID}')">Clean Message</button>

					<button type="button" class="btn btn-primary btn-sm" onclick="caseWorkFlowActions.swiftAdminRejectMessage('swiftAdminPendingCaseSerachResult${UNQID}','${UNQID}')">Reject Message</button>

					<button type="button" class="btn btn-primary btn-sm" onclick="caseWorkFlowActions.swiftAdminViewComments('swiftAdminPendingCaseSerachResult${UNQID}','${UNQID}')">View Comment</button>
