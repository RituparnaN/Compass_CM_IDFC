...
<script type="text/javascript">
	$(document).ready(function() {
		var id = '${UNQID}';
		compassTopFrame.init(id, 'mostActiveAcctsTable'+id, 'dd/mm/yy');
		
		$('.panelSlidingMostActiveAccts'+id).on("click", function (e) {
			var mainRow = $(this).parents(".compassrow"+id);
			compassTopFrame.searchPanelSliding(id, mainRow, 'mostActiveAcctsSerachResultPanel');
	    });
		
		$("#searchMasterForm"+id).submit(function(e){
			var submitButton = $("#searchMostActiveAcct"+id);
			compassTopFrame.submitForm(id, e, submitButton, 'mostActiveAcctsSerachResultPanel', 
					'mostActiveAcctsSerachResult', '${pageContext.request.contextPath}/common/searchGenericMaster',
...
<div class="row compassrow${UNQID}">
	<div class="col-sm-12">
		<div class="card card-primary panel_mostActiveAccts">
			<div class="card-header panelSlidingMostActiveAccts${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}"><spring:message code="app.common.mostActiveAcctSearchHeader"/></h6>
			...
			<input type="hidden" name="bottomPageUrl" value="MostActiveAccounts/SearchBottomPage">
			<div class="card-search-card" >
				<table class="table table-striped formSearchTable mostActiveAcctsTable${UNQID}" style="margin-bottom: 0px;">
					<tbody>
			...
			<div class="card-footer clearfix">
				<div class="pull-${dirR}">
					<button  type="submit" id="searchMostActiveAcct${UNQID}" class="btn btn-success btn-sm"><spring:message code="app.common.searchButton"/></button>
				</div>
			</div>
			</form>
			</div>
		</div>
		<div class="card card-primary" id="mostActiveAcctsSerachResultPanel${UNQID}" style="display: none;">
			<div class="card-header panelSlidingMostActiveAccts${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}"><spring:message code="app.common.mostActiveAcctResultHeader"/></h6>
				<div class="btn-group pull-${dirR} clearfix">
					<span class="pull-right"><i class="collapsable fa fa-chevron-up"></i></span>
				</div>
			</div>
			<div id="mostActiveAcctsSerachResult${UNQID}"></div>
		</div>
	</div>
</div>