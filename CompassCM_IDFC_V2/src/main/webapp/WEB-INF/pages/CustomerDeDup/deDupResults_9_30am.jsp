		...
		<div class="card card-primary">
			...
			if(linkedHashMap != null && linkedHashMap.size()>=4){
				...
				%>
					<table width="100%" border="0">
						<tr>
							<td align="left"><font face="verdana" size="2"><b> 
				  				 <a href=" openSelectedListDetails('Customer','<%=strMapKey%>',' ')" title="View customer details"><%=strCustomerName%></a></font>
					...
					<table border="0" width="100%" cellspacing="1" cellpadding="1" bgcolor="#606062">
					<TH><font color="white" size="2">.</font></TH>
					<% for(int j=2;j<strColumnNameArray.length-6;j++){  %>  
						<TH><font color="white" size="2"><%=strColumnNameArray[j]%></font></TH>
					<%		
					}
			...
			%>	
				<tr <% if(!strActionFlag.equals("N")){ %>style="cursor: hand;" onClick=openPreviousComments('<%=alMatchedList.get(i+9)%>')<%}%>>
					 ...
					 <td bgcolor="<%=strTempColumn%>" width='15%' align='center'><img src="${pageContext.request.contextPath}/includes/images/RemoveUser24X24.gif" onClick="AddToPartyList('<%=strMapKey%>','<%=alMatchedList.get(i+2).toString()%>','ToRemove')" title="Remove From Party" style="cursor: hand">
					 <font size="2"><a href= "showMatches('<%=strMapKey%>','<%=alMatchedList.get(i+2).toString()%>')" title="View matches"><%=strBlackListName%></font></a>
					 <% } else { %>
					 <td bgcolor="<%=strTempColumn%>" width='15%' align='center'><font size="2"><a href= " showMatches('<%=strMapKey%>','<%=alMatchedList.get(i+2).toString()%>')" title="View matches"><%=strBlackListName%></font></a>
					 ...
					 <font size="2">
					 <a href= " openSelectedListDetails('Customer','<%=alMatchedList.get(i+2).toString()%>','<%=strMapKey+"^"+alMatchedList.get(i+2).toString()%>')" title="View customer details"> <%=alMatchedList.get(i+2).toString()%>
					 </font>
					 </a>
					 </td>
...
<%		
      ...
      %>
		...
		<INPUT TYPE='button' NAME= 'AttachEvidence' value ='VIEW/ATTACH EVIDENCE' class="btn btn-sm btn-warning"  onclick="attachFileEvidence();" > 
