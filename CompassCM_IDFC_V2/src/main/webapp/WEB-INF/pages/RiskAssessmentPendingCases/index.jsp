...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$("#searchMasterForm"+id).submit(function(e){
			...
			compassTopFrame.submitForm(id, e, submitButton, 'amlUserPendingCaseSerachResultPanel', 
					'amlUserPendingCaseSerachResult', '${pageContext.request.contextPath}/common/riskAssessmentSearchPendingCases',
		...
		$("#viewEDDQuestionRecords"+id).click(function(){
			var caseNo = "";
			var count = 0;
			$("#amlUserPendingCaseSerachResult"+id).find("table").children("tbody").children("tr").each(function(){
				if($(this).children("td").children("input").prop("checked")){
			...
			if(count == 0){
				...
				$("#compassMediumGenericModal-title").html("EDD Questions : "+caseNo);
				...
				$.ajax({
					url: "${pageContext.request.contextPath}/common/viewEDDQuestionRecords",
					cache: false,
					type: "POST",
					data: "caseNo="+caseNo,
					success: function(res) {
	...
	/* function openDetails(elm,a,b,c,d){
		$("#compassMediumGenericModal").modal("show");
		$("#compassMediumGenericModal-title").html("Case Details");
		$("#compassMediumGenericModal-body").html("<br/><center><img alt='Loading...' src='${pageContext.request.contextPath}/includes/images/qde-loadder.gif'></center><br/>");
		 $.ajax({
			url: "${pageContext.request.contextPath}/common/caseDetails",
			cache: false,
			type: "POST",
			/* data: "caseNo="+caseNo+"&caseId="+caseId.split(".")[0]+"&caseVersion="+caseId.split(".")[1], */
			/*success: function(res) {
				$("#compassMediumGenericModal-body").html(res);
			},
			error: function(a,b,c) {
	...
	function handleEscalateCase(){
		...
		$("#amlUserPendingCaseSerachResult"+id).find("table").children("tbody").children("tr").each(function(){
			if($(this).children("td").children("input").prop("checked")){
				caseNo = $(this).children("td").children("input").val().split("||")[0];
				caseId = $(this).children("td").children("input").val().split("||")[1];
				crn = $(this).children("td").children("input").val().split("||")[2];
				count++;
			}
		});
		...
		if(count == 0){
			...
			$("#compassMediumGenericModal").modal("show");
			$("#compassMediumGenericModal-title").html("Escalate Case : "+caseNo);
			$("#compassMediumGenericModal-body").html("<br/><center><img alt='Loading...' src='${pageContext.request.contextPath}/includes/images/qde-loadder.gif'></center><br/>");
			$.ajax({
				url: "${pageContext.request.contextPath}/common/riskAssessmentPendingCases/getCaseWorkflowModuleDetails",
				cache: false,
				type: "GET",
				data: "moduleValue="+caseNo+"&moduleCode="+"riskAssessmentViewComments"+"&detailPage="+escape("RiskAssessmentPendingCases/AddViewCommentsWithHistory")+"&actionType="+"escalateCase"+"&caseId="+caseId.split(".")[0]+"&caseVersion="+caseId.split(".")[1]+"&compassRefNo="+crn,
				success: function(res) {
					$("#compassMediumGenericModal-body").html(res);
				},
				error: function(a,b,c) {
	...
	function handleViewCaseComments(){
		var id = '${UNQID}';
		var caseNo = "";
		var caseId = "";
		var crn = "";
		...
		if(count == 0){
			...
			$("#compassMediumGenericModal").modal("show");
			$("#compassMediumGenericModal-title").html("Comment Logs for case : "+caseNo);
			$("#compassMediumGenericModal-body").html("<br/><center><img alt='Loading...' src='${pageContext.request.contextPath}/includes/images/qde-loadder.gif'></center><br/>");
			...
			$.ajax({
				...
				cache: false,
				type: "GET",
				data: "moduleValue="+caseNo+"&moduleCode="+"riskAssessmentViewComments"+"&detailPage="+escape("RiskAssessmentPendingCases/AddViewCommentsWithHistory")+"&actionType="+"viewComments"+"&caseId="+caseId.split(".")[0]+"&caseVersion="+caseId.split(".")[1]+"&compassRefNo="+crn,
				success: function(res) {
					$("#compassMediumGenericModal-body").html(res);
				},
				error: function(a,b,c) {
	...
	function handleAction(){
		...
		if(count == 0){
			...
			$.ajax({
				...
				cache: false,
				type: "GET",
				data: "moduleValue="+caseNo+"&moduleCode="+"riskAssessmentViewComments"+"&detailPage="+escape("RiskAssessmentPendingCases/AddViewCommentsWithHistory")+"&actionType="+"checkerAction"+"&caseId="+caseId.split(".")[0]+"&caseVersion="+caseId.split(".")[1]+"&compassRefNo="+crn,
...
<div class="row compassrow${UNQID}">
	<div class="col-sm-12">
		<div class="card card-primary panel_amlUserPendingCase">
			...
			<input type="hidden" name="bottomPageUrl" value="RiskAssessmentPendingCases/SearchBottomPage">
			...
			<div class="card-footer action-footer clearfix">
				<div class="pull-${dirR} clearfix">
					...
					<%-- <button type="button" class="btn btn-primary btn-sm" onclick="caseWorkFlowActions.escalateCaseByBranchUser('amlUserPendingCaseSerachResult${UNQID}','${UNQID}')">Escalate Case</button>
					 --%>
					<c:if test = "${USERROLE == 'ROLE_CM_MAKER'}">
						<button type="button" class="btn btn-primary btn-sm" onclick="handleEscalateCase()">Escalate Case</button>
						
					</c:if>
					<c:if test = "${USERROLE == 'ROLE_CM_CHECKER'}">
						
						<!-- <button type="button" class="btn btn-success btn-sm" onclick="approveCase()">Approve</button>
						<button type="button" class="btn btn-danger btn-sm" onclick="rejectCase()">Reject</button> -->
						<button type="button" class="btn btn-primary btn-sm" onclick="handleAction()">Action</button>
					</c:if>
					...
					<button type="button" class="btn btn-primary btn-sm" onclick="handleViewCaseComments()">View Comments</button>
					<!-- <button type="button" class="btn btn-primary btn-sm" onclick="handleViewCaseComments2()">View Comments (T)</button> -->
					<%-- <button type="button" class="btn btn-primary btn-sm" id="viewEDDQuestionRecords${UNQID}">View EDD Questions</button>
					 --%>
