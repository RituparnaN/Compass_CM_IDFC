...
<script type="text/javascript">
	...
	$("#submitRaiseSuspicionForm"+id).click(function(){
		...
		if(reportingOn == 'customer'){
			...
			if(reportingOn != "" && alertRating != "" && typeOfSuspicion != "" 
					&& reasonForSuspicion != ""){
				if(customerId != "" || accountNo != ""){
					if(customerId != "" && branchCode != "ALL"){
						$.ajax({
							url: "${pageContext.request.contextPath}/common/validationCheck",
							data : "customerId="+customerId+"&accountNo="+accountNo,
							cache:	false,
							type: "POST",
							success: function(response){
								//alert(response);
								if(response == 'CustomerId/AccountNo is valid'){
									$.ajax({
									...
									alert("Invalid CustomerId/AccountNo.");
								}
							}, 
							error: function(a,b,c){
								alert(a+b+c);
