		...
		<div class="card card-primary panel_falsePositive">
			...
			<form action="javascript:void(0)" method="POST" id="searchMasterForm${UNQID}">
				<table class="table compassModuleDetailsSearchTable${UNQID}" style="margin-bottom: 0px;">
					...
					<tr>	
						...
						<td width="30%">
							...
							<c:if test="${!f:containsIgnoreCase(ROLE,'ROLE_AMLUSER')}">disabled="disabled"</c:if>/>						
						</td>
					</tr>
					<tr>	
						...
						<td width="30%">
							<select class="form-control input-sm" name="isEnabled" id="isEnabled${UNQID}" <c:if test="${!f:containsIgnoreCase(ROLE,'ROLE_AMLUSER')}">disabled="disabled"</c:if>>
					...
					<tr>	
						...
						<td width="30%">
							<textarea class="form-control input-sm" name="reason" id="reason${UNQID}" <c:if test="${!f:containsIgnoreCase(ROLE,'ROLE_AMLUSER')}">disabled="disabled"</c:if>>${resultData['REASON']}</textarea>
						</td>
						<td width="10%">&nbsp;</td>
						<td width="15%">To Be Deleted</td>
						<td width="30%">
							<select class="form-control input-sm" name="isToBeDeleted" id="isToBeDeleted${UNQID}" <c:if test="${!f:containsIgnoreCase(ROLE,'ROLE_AMLUSER')}">disabled="disabled"</c:if>>
								<option value="Select"></option>
				...
				<div class="card-footer clearfix">
					...
					<c:if test="${f:containsIgnoreCase(ROLE,'ROLE_AMLUSER')}">
