...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#pendingWith"+id).on("change", function(){
			if($(this).val() == "AMLO"){
		...
		$("#searchCaseReassignment"+id).click(function(){
			...
			var fullData = "fromDate="+fromDate+"&toDate="+toDate+"&reassignmentFor="+reassignmentFor+"&pendingWith="+pendingWith+"&pendingUsersCode="+pendingUsersCode;
			if(reassignmentFor != 'all' && pendingWith != 'all' ){
			$.ajax({
				...
				alert("Please select options for reassignment for and pending with.");
			}
		});
			...
			if(intSelectedCount == 0 ) {
			   ...
			   alert('Please select only one case to assign.');
				...
				$.ajax({
					...
					cache: false,
					type: "GET",
					data: "selectedCase="+selectedCase+"&CaseStatus=1&FlagType=Y&roleId="+roleId+"&reassignmentFor="+reassignmentFor+"&pendingWith="+pendingWith+"&pendingUsersCode="+pendingUsersCode+"&searchButton="+searchButton,
	...
	<div class="col-sm-12">
		<div class="card card-primary panel_caseReassignment">
			...
			<form action="javascript:void(0)" method="POST" id="searchMasterForm${UNQID}">
				<table class="table compassCaseReassignmentSearchTable${UNQID}" style="margin-bottom: 0px;">
					<tr>
					...
					<tr>
						...
						<td width="10%">&nbsp;</td>
						<td width="15%"><div class="hideDiv" style="display: none;"><spring:message code="app.common.pendingWith"/></div></td>
						<td width="30%">
			...
			<div id="caseReassignmentSerachResult${UNQID}"></div>
			 <div class="card-footer clearfix">
				<div class="pull-${dirR}">
					...
					<input type="button" class="btn btn-primary btn-sm" id="reassignCases${UNQID}" name="reassignCases" value="Reassign Cases" />
					<input type="button" class="btn btn-primary btn-sm" id="viewEvidence${UNQID}" name="viewEvidence" value="View Evidence"/>
				</div>
			</div>
		</div>
	</div>
