...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#updateCustomerCDDReviewDetails").click(function(){
			var customerId = '${moduleValue}';
			var fieldsData = "";
			$(this).parents("div.card-footer").siblings("table").find("input").each(function(){
				fieldsData = fieldsData + $(this).attr("name") +"="+ $(this).val() + ",";
			...
			$.ajax({
	    		url : "${pageContext.request.contextPath}/common/updateCustomerCDDReviewDetails",
	    		data : "fieldsData="+fieldsData+"&customerId="+customerId,
	    		cache : false,
	    		type : 'POST',
	    		success : function(resData){
		...
		$("#openRPAFolder"+id).click(function(){
			var caseNo = '${moduleValue}';
			$.fileDownload('${pageContext.request.contextPath}/fraudCaseWorkFlow/downloadRPAFolder?caseNo='+caseNo, {
				httpMethod : "GET",
					...
					<div class="col-sm-6">
						<div class="card-body" style="text-align: center; padding: 5px 0px;">
							...
							<table class="table compassModuleDetailsSearchTable" style="margin-bottom: 0px;">
								<tr>
									<td width="20%" style="padding-top: 10px;">Case No</td>
									...
									<!-- <td width="20%"><button type="button" class="btn btn-primary btn-sm" id="searchModuleDetails${UNQID}"><spring:message code="app.common.searchButton"/></button></td> -->
									<td width="20%"><button type="button" class="btn btn-warning btn-sm" id="openRPAFolder${UNQID}" name="openRPAFolder">Download Details</button></td>
									<!-- linkType="_blank" link="file:///C:\APPFOLDER\RPARepositoryDetails" -->
								</tr>
							</table>
				...
				<c:choose>
					...
					<c:otherwise>
							<c:choose>
								...
								<c:when test="${tab eq 'CDD Review Details'}">
									<c:forEach var="fieldDeatails" items="${tabDetail}">
										<table class="table table-striped">
											<tbody>
												<c:set var="LABELSCOUNT" value="${f:length(fieldDeatails)}"/>
										...
										<div class="card-footer">
											<div style="margin-left: 95%">
												<button type="button" class="btn btn-primary btn-sm" id="updateCustomerCDDReviewDetails">Update</button>
											</div>
										</div>
									</c:forEach>
								</c:when>
