...
<script type="text/javascript">
	...
	function saveComments()
	{
	   ...
	   document.form1.amlUserCode.value = $("#amlUserCode"+id).val();
	   document.form1.fraudIndicator.value = fraudIndicator;
	   
	   if($("#amlUserCode"+id).val() == "" || $("#amlUserCode"+id).val() == "Select" ){
		   alert('Please Select AML User.');
		   return false; 
	   }else if(Comments == ""){
		...
		<div class="card card-primary panel_addViewComments" id="panelAddViewCommentsAMLUser" style="margin-top: 2px; margin-bottom: 0px;">
			...
			<div class="panelSearchForm">
				<table class="table table-striped compassModuleDetailsSearchTable${UNQID}" style="margin-bottom: 0px;">
					<tr>
						<td width="20%">Reason Of Allocation</td>
						<td width="25%">
						...
						<td width="20%">AMLUser List</td>
							<td width="25%">
								<select class="form-control input-sm" name="amlUserCodes" id = "amlUserCode${UNQID}">
									...
									<c:forEach items="${ALLUSERSLIST}" var = "usercode">
										<%-- <option value = '${usercode.USERCODE1}'>${usercode.USERCODE2}</option> --%>
										<option value = '${usercode.USERCODE1}'>${usercode.USERNAME}</option>									
									</c:forEach>
								</select>
							</td>
					</tr>
...
<form name="form1" id="saveCommentForm" method='POST' action="${pageContext.request.contextPath}/amlCaseWorkFlow/saveCommentWhileReAllocatingToAMLUser">
<input type="hidden" name="CaseNo" value="<%= CaseNos %>" />
