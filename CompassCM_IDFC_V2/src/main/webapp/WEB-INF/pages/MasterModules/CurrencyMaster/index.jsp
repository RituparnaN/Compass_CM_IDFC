...
<script type="text/javascript">
	$(document).ready(function() {
		var id = '${UNQID}';
		compassTopFrame.init(id, 'currencyMasterTable', 'dd/mm/yy');
		
		$('.panelSlidingCurrencyMaster'+id).on("click", function(e){
			var mainRow = $(this).parents(".compassrow"+id);
			compassTopFrame.searchPanelSliding(id, mainRow, 'currencyMasterSearchResultPanel');	
		});
		
		$('#searchMasterForm'+id).submit(function(e){
			var submitButton = $("#searchCurrencyMaster"+id);
			compassTopFrame.submitForm(id, e, submitButton, 'currencyMasterSearchResultPanel',
					'currencyMasterSearchResult', '${pageContext.request.contextPath}/common/searchGenericMaster',
					'${pageContext.request.contextPath}/includes/images/qde-loadder.gif');
		});
		
		$("#uploadCurrencyExchangeDetails"+id).click(function(){
			compassFileUpload.init("uploadCurrencyExchangeDetails"+id,"${pageContext.request.contextPath}","currencyexchangeMasterData","0","Y","Y","");
		});
...
<div class="row compassrow${UNQID}">
	<div class="col-sm-12">
		<div class="card card-primary panel_currencyMaster">
			<div class="card-header panelSlidingCurrencyMaster${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}"><spring:message code="app.common.currencySearchHeader"/></h6>
			...
			<input type="hidden" name="bottomPageUrl" value="MasterModules/CurrencyMaster/SearchBottomPage">
			<div class="card-search-card" >
				<table class="table table-striped formSearchTable currencyMasterTable" style="margin-bottom: 0px;">
					<tbody>
			...
			<div class="card-footer clearfix">
				<div class="pull-${dirR}">
					<button  type="submit" id="searchCurrencyMaster${UNQID}" class="btn btn-success btn-sm"><spring:message code="app.common.searchButton"/></button>
					<input type="button" class="btn btn-warning btn-sm" id="uploadCurrencyExchangeDetails${UNQID}" name="UploadCurrencyExchangeDetails" value="Upload Currency Exchange Data"/>
					<!--  28.05.2019 -->
		...
		<div class="card card-primary" id="currencyMasterSearchResultPanel${UNQID}" style="display: none;">
			<div class="card-header panelSlidingCurrencyMaster${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}">Currency Master Result</h6>
				<div class="btn-group pull-${dirR} clearfix">
					<span class="pull-right"><i class="collapsable fa fa-chevron-up"></i></span>
				</div>
			</div>
			<div id="currencyMasterSearchResult${UNQID}"></div>
		</div>
	</div>
</div>
