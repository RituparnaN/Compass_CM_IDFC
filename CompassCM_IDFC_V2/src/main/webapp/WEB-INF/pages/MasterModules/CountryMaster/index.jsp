...
<script type="text/javascript">
	$(document).ready(function() {
		var id = '${UNQID}';
		compassTopFrame.init(id, 'countryMasterTable', 'dd/mm/yy');
		
		$('.panelSlidingCountryMaster'+id).on("click", function (e) {
			var mainRow = $(this).parents(".compassrow"+id);
			compassTopFrame.searchPanelSliding(id, mainRow, 'countryMasterSearchResultPanel');
	 	});
		
		$('#searchMasterForm'+id).submit(function(e){
			var submitButton = $('#searchCountryMaster'+id);
			compassTopFrame.submitForm(id, e, submitButton, 'countryMasterSearchResultPanel',
					'countryMasterSearchResult', '${pageContext.request.contextPath}/common/searchGenericMaster',
	...
	function uploadCountryRiskFile(elm){
		compassFileUpload.init("uploadCountryRiskFile","${pageContext.request.contextPath}","COUNTRYRISKUPLOAD","0","Y","N","");
	}
	
</script>
<div class="row compassrow${UNQID}">
	<div class="col-sm-12">
		<div class="card card-primary panel_countryMaster">
			<div class="card-header panelSlidingCountryMaster${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}"><spring:message code="app.common.countrySearchHeader"/></h6>
			...
			<input type="hidden" name="bottomPageUrl" value="MasterModules/CountryMaster/SearchBottomPage">
			<div class="card-search-card" >
				<table class="table table-striped formSearchTable countryMasterTable" style="margin-bottom: 0px;">
					<tbody>
			...
			<div class="card-footer clearfix">
				<div class="pull-${dirR}">
					<button  type="submit" id="searchCountryMaster${UNQID}" class="btn btn-success btn-sm"><spring:message code="app.common.searchButton"/></button>
					<c:if test="${f:containsIgnoreCase(ROLE,'AMLO')}">
						<button type="button" id="uploadCountryRiskFile${UNQID}" class="btn btn-warning btn-sm"  onclick="uploadCountryRiskFile(this)">Upload Country Risk File</button>
					</c:if>
		...
		<div class="card card-primary" id="countryMasterSearchResultPanel${UNQID}" style="display: none;">
			<div class="card-header panelSlidingCountryMaster${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}">Country Master Result</h6>
				<div class="btn-group pull-${dirR} clearfix">
					<span class="pull-right"><i class="collapsable fa fa-chevron-up"></i></span>
				</div>
			</div>
			<div id="countryMasterSearchResult${UNQID}"></div>
		</div>
	</div>
</div>
