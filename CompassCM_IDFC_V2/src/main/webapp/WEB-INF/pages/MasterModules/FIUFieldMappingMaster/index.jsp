...
<script type="text/javascript">
	$(document).ready(function() {
		var id = '${UNQID}';
		compassTopFrame.init(id, 'fiuFieldMappingMasterTable', 'dd/mm/yy');
		
		$('.panelSlidingFIUFieldMappingMaster'+id).on("click", function (e) {
			var mainRow = $(this).parents(".compassrow"+id);
			compassTopFrame.searchPanelSliding(id, mainRow, 'fiuFieldMappingMasterSearchResultPanel');
	    });
		
		$("#searchMasterForm"+id).submit(function(e){
			var submitButton = $("#searchFIUFieldMappingMaster"+id);
			compassTopFrame.submitForm(id, e, submitButton, 'fiuFieldMappingMasterSearchResultPanel', 
					'fiuFieldMappingMasterSearchResult', '${pageContext.request.contextPath}/common/searchGenericMaster',
	...
	function uploadFIUFieldMapping(elm){
		compassFileUpload.init("uploadFIUFieldMappingFile","${pageContext.request.contextPath}","FIUFIELDMAPPINGUPLOAD","0","Y","N","");
	}
	
</script>
<div class="row compassrow${UNQID}">
	<div class="col-sm-12">
		<div class="card card-primary panel_fiuFieldMappingMaster">
			<div class="card-header panelSlidingFIUFieldMappingMaster${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}"><spring:message code="app.common.fiuFieldMappingSearchHeader"/></h6>
			...
			<input type="hidden" name="bottomPageUrl" value="MasterModules/FIUFieldMappingMaster/SearchBottomPage">
			<div class="card-search-card" >
				<table class="table table-striped formSearchTable fiuFieldMappingMasterTable" style="margin-bottom: 0px;">
					<tbody>
			...
			<div class="card-footer clearfix">
				<div class="pull-${dirR}">
					<button type="submit" id="searchFIUFieldMappingMaster${UNQID}" class="btn btn-success btn-sm"><spring:message code="app.common.searchButton"/></button>
						<c:if test="${f:containsIgnoreCase(ROLE,'AMLO')}">
						<button type="button" id="uploadFIUFieldMapping${UNQID}" class="btn btn-warning btn-sm"  onclick="uploadFIUFieldMapping(this)">Upload FIU Field Mapping</button>
					</c:if>
		...
		<div class="card card-primary" id="fiuFieldMappingMasterSearchResultPanel${UNQID}" style="display: none;">
			<div class="card-header panelSlidingFIUFieldMappingMaster${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}"><spring:message code="app.common.fiuFieldMappingSearchResultHeader"/></h6>
				<div class="btn-group pull-${dirR} clearfix">
					<span class="pull-right"><i class="collapsable fa fa-chevron-up"></i></span>
				</div>
			</div>
			<div id="fiuFieldMappingMasterSearchResult${UNQID}"></div>
		</div>
	</div>
</div>
