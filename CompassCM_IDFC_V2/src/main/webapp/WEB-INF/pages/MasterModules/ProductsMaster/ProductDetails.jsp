...
<script type="text/javascript">
	$(document).ready(function(){
		...
		compassDatatable.construct(tableClass, "ProductDetails", false);
		...
		$("#updateProductExclusionDetails"+id).click(function(){
			var productCode = '${moduleValue}';
			var status = 'P';
			var fieldsData = "";
			var exclusionData = "";
			var commentsData = "";
			...
			$(this).parents("div.card-footer").siblings("table").find("select").each(function(){
				exclusionData = exclusionData + $(this).attr("name") +"="+ $(this).val() + ",";
				//alert(exclusionData);
			});
			...
			fieldsData = fieldsData+exclusionData+commentsData;
			
			//alert(fieldsData);
			//alert("&status="+status+"&productCode="+productCode);
			$.ajax({
	    		url : "${pageContext.request.contextPath}/common/updateProductExclusionDetails",
	    		data : "fieldsData="+fieldsData+"&status="+status+"&productCode="+productCode,
	    		cache : false,
	    		type : 'POST',
		...
		$("#approveProductExclusionDetails").click(function(){
			var productCode = '${moduleValue}';
			var status = 'A';
			var fieldsData = "";
			var exclusionData = "";
		...
		$("#rejectProductExclusionDetails").click(function(){
			var productCode = '${moduleValue}';
			var status = 'R';
			var fieldsData = "";
			var exclusionData = "";
					...
					<div class="col-sm-6">
						<div class="card-body" style="text-align: center; padding: 5px 0px;">
							...
							<table class="table compassModuleDetailsSearchTable" style="margin-bottom: 0px;">
								<tr>
									<td width="30%"><spring:message code="app.common.PRODUCTCODE"/></td>
						...
						<c:forEach var="fieldDeatails" items="${tabDetail}">
							...
							<c:if test="${(CURRENTROLE eq 'ROLE_AMLO')}">
								...
								<div style="margin-left: 95%">
									<button type="button" class="btn btn-primary btn-sm" id="updateProductExclusionDetails${UNQID}">Update</button>
								</div>
							</div>
							</c:if>
							-->
									...
									<c:if test="${f:contains(columnName, 'app.common.STATUS')}">
										<c:if test="${f:contains(columnValue, 'P')}">
											...
											<div class="pull-right">
												<button type="button" class="btn btn-success btn-sm" id="approveProductExclusionDetails">Approve</button>
												<button type="button" class="btn btn-danger btn-sm" id="rejectProductExclusionDetails">Reject</button>
											</div>
											</div>
										</c:if>
										<c:if test="${not f:contains(columnValue, 'P')}">
											<div class="card-footer clearfix">
											<div class="pull-right">
												<button type="button" class="btn btn-success btn-sm" id="approveProductExclusionDetails" disabled="disabled">Approve</button>
												<button type="button" class="btn btn-danger btn-sm" id="rejectProductExclusionDetails" disabled="disabled">Reject</button>
											</div>
											</div>
										</c:if>
									</c:if>
