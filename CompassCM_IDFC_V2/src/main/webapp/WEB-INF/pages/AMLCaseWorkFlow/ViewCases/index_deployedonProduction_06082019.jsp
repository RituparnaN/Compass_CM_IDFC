...
<script type="text/javascript">
	$(document).ready(function() {
		var id = '${UNQID}';
		//compassTopFrame.init(id, 'viewCasesTable'+id, 'dd/mm/yy');
		var isDefaultDateRequired = false;
		compassTopFrame.init(id, 'viewCasesTable'+id, 'dd/mm/yy', isDefaultDateRequired);


		$('#FROMDATE_'+id).val('');
		$('#TODATE_'+id).val('');

		$('.panelSlidingViewCases'+id).on("click", function (e) {
			var mainRow = $(this).parents(".compassrow"+id);
			compassTopFrame.searchPanelSliding(id, mainRow, 'viewCasesSerachResultPanel');
	    });
		
		$("#searchMasterForm"+id).submit(function(e){
			var submitButton = $("#searchViewCases"+id);

			if($('#FROMDATE_'+id).val() == '' && $('#TODATE_'+id).val() == '' && $('#ALERTCODE_'+id).val() == 'ALL' &&
			   $('#BRANCHCODE_'+id).val() == 'ALL' && $('#ACCOUNTNO_'+id).val() == '' && $('#CUSTOMERID_'+id).val() == '' &&
			   $('#CURRENTSTATUS_'+id).val() == 'ALL' && $('#CASERATING_'+id).val() == 'ALL' && $('#FROMCASENO_'+id).val() == '' &&
			   $('#TOCASENO_'+id).val() == '' && $('#CASESTATUS_'+id).val() == 'ALL' && $('#CASESUBSTATUS_'+id).val() == 'ALL' &&
			   $('#ASSIGNEDUSER_'+id).val() == 'ALL'){
						alert('Select atleast one of the filter to search.');
			}else{
				var fromDate = $('#FROMDATE_'+id).val();
				var toDate = $('#TODATE_'+id).val();
				var fromCase = $('#FROMCASENO_'+id).val();
				var toCase = $('#TOCASENO_'+id).val();
				...
				if(days < '0'){
					...
					alert("The date range should be within 180 days.");
				}else if(fromCase != '' && toCase == ''){
					alert('To Case is mandatory.');
				}else if(fromCase == '' && toCase != ''){
					alert('From Case is mandatory.');	
				}
				else{
					compassTopFrame.submitForm(id, e, submitButton, 'viewCasesSerachResultPanel', 
							'viewCasesSerachResult', '${pageContext.request.contextPath}/amlCaseWorkFlow/searchViewClosedCases',
		...
		$("#attachEvedence"+id).click(function(){
			var caseNo = "";
			var count = 0;
			$("#viewCasesSerachResult"+id).find("table").children("tbody").children("tr").each(function(){
			...
			compassFileUpload.init("attachEvedence","${pageContext.request.contextPath}","5678","viewCasesSerachResult"+id,"N","N");
			}
		});
...
<div class="row compassrow${UNQID}">
	<div class="col-sm-12">
		<div class="card card-primary panel_viewCases">
			<div class="card-header panelSlidingViewCases${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}"><spring:message code="app.common.viewCasesSearchHeader"/></h6>
			...
			<input type="hidden" name="bottomPageUrl" value="AMLCaseWorkFlow/ViewCases/SearchBottomPage">
			<div class="card-search-card" >
				<table class="table table-striped formSearchTable viewCasesTable${UNQID}" style="margin-bottom: 0px;">
					<tbody>
			...
			<div class="card-footer clearfix">
				<div class="pull-${dirR}">
					<button  type="submit" id="searchViewCases${UNQID}" class="btn btn-success btn-sm"><spring:message code="app.common.searchButton"/></button>
				</div>
			</div>
			</form>
			</div>
		</div>
		<div class="card card-primary" id="viewCasesSerachResultPanel${UNQID}" style="display: none;">
			<div class="card-header panelSlidingViewCases${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}"><spring:message code="app.common.viewCasesResultHeader"/></h6>
				<div class="btn-group pull-${dirR} clearfix">
					<span class="pull-right"><i class="collapsable fa fa-chevron-up"></i></span>
				</div>
			</div>
			<div id="viewCasesSerachResult${UNQID}"></div>
			
			<div class="card-footer action-footer clearfix">
				<div class="pull-${dirR} clearfix">
					<!--
					<button type="button" class="btn btn-primary btn-sm" onclick="caseWorkFlowActions.desktopCloser('viewCasesSerachResult${UNQID}','${UNQID}')">Desktop Closure</button>
					...
					<div class="btn-group dropup" role="group">
						...
						<ul class="dropdown-menu" role="menu">
							<li><a href="javascript:void(0)" onclick="caseWorkFlowActions.closeWithSTR('viewCasesSerachResult${UNQID}','${UNQID}')">With STR</a></li>
							<li><a href="javascript:void(0)" onclick="caseWorkFlowActions.closeWithoutSTR('viewCasesSerachResult${UNQID}','${UNQID}')">Without STR</a></li>
						</ul>
					</div>
					-->
					<button type="button" class="btn btn-primary btn-sm" onclick="caseWorkFlowActions.viewComments('viewCasesSerachResult${UNQID}','${UNQID}')">View Comment</button>
					...
					<!-- <div class="btn-group dropup" role="group">
						...
						<ul class="dropdown-menu" role="menu">
							<li><a href="javascript:void(0)" onclick="caseWorkFlowActions.viewIndianSTR('viewCasesSerachResult${UNQID}','${UNQID}')">Indian STR</a></li>
							<li><a href="javascript:void(0)" onclick="caseWorkFlowActions.viewSriLankanSTR('viewCasesSerachResult${UNQID}','${UNQID}')">SL STR</a></li>
							<li><a href="javascript:void(0)" onclick="caseWorkFlowActions.viewUKSTR('viewCasesSerachResult${UNQID}','${UNQID}')">UK SAR</a></li>
							<li><a href="javascript:void(0)" onclick="caseWorkFlowActions.viewMaldivesSTR('viewCasesSerachResult${UNQID}','${UNQID}')">Maldives STR</a></li>
							<li><a href="javascript:void(0)" onclick="caseWorkFlowActions.viewNepalSTR('viewCasesSerachResult${UNQID}','${UNQID}')">Nepal STR</a></li>
							<li><a href="javascript:void(0)" onclick="caseWorkFlowActions.viewSriLankanSTR('viewCasesSerachResult${UNQID}','${UNQID}')">SL STR</a></li>
						</ul>
					</div>-->
					...
					<!--<button type="button" class="btn btn-primary btn-sm" onclick="caseWorkFlowActions.assignToBranchUserByAMLuser('viewCasesSerachResult${UNQID}','${UNQID}')">Assign To BranchUser</button>-->
