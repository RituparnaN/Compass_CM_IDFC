...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var existingFromDate = '${resultData["ACTIVEFROMDATE"]}';
		var existingToDate  = '${resultData["ACTIVETODATE"]}';

		var existingDiff = Math.round(parseDate(existingToDate) - parseDate(existingFromDate));
		var existingDays = Math.round(existingDiff/1000/60/60/24);
		$("#suppressionDuration"+id).val(existingDays); 
		...
		$("#activeFrom"+id).change(function(){
			activeFromDate = $(this).val();
			var diff = Math.round(parseDate(activeTo) - parseDate(activeFromDate));
			var days = Math.round(diff/1000/60/60/24);
			$("#suppressionDuration"+id).val(days); 
		});
		
		$("#activeTo"+id).change(function(){
			activeTo = $(this).val();
			var diff = Math.round(parseDate(activeTo) - parseDate(activeFromDate));
			var days = Math.round(diff/1000/60/60/24);
			$("#suppressionDuration"+id).val(days); 
		...
		$("#saveToFalsePositive"+id).click(function(){
			...
			var suppressionDuration = $("#suppressionDuration"+id).val();
			
			let diff = Math.round(parseDate(activeTo) - parseDate(activeFromDate));
			let days = Math.round(diff/1000/60/60/24);

			if(activeFromDate.length && activeTo.length){
				if(diff < 0){
					alert("From Date should not be less than To Date.");
				}else if(suppressionDuration > 180){
					alert("Duration of supression should be within 180 days.");	
				}else{
				...
				alert("FromDate and ToDate cannot be blank.");
			}	
		});
			...
			<form action="javascript:void(0)" method="POST" id="searchMasterForm${UNQID}">
				<table class="table compassModuleDetailsSearchTable${UNQID}" style="margin-bottom: 0px;">
					...
					<tr>	
						...
						<td width="30%">
							<input type="text" class="form-control input-sm datepicker" name="activeFrom" id="activeFrom${UNQID}"
								value="${resultData['ACTIVEFROMDATE']}" /> 
					...
					<tr>
						<td width="15%">Duration of Supression</td>	
						<td width="30%">
							<input type="text" class="form-control input-sm" name="suppressionDuration" id="suppressionDuration${UNQID}" readonly="readonly"/>
						</td>
						<td width="10%">&nbsp;</td>
					...
					<tr>	
						...
						<td width="30%">
							...
							onkeyup="$(this).val($(this).val().replace(/[^\d]/ig, ''))" value="${resultData['TOLERANCELEVEL']}" />
							<%-- <c:if test="${ROLE ne 'ROLE_AMLUSER'}">disabled="disabled"</c:if>/> --%>
							<%-- <c:if test="${ROLE ne 'ROLE_AMLO'}">disabled="disabled"</c:if>/> --%>
						</td>
						<td width="10%">&nbsp;</td>
					...
					<tr>
						...
						<td colspan="4">
							<textarea class="form-control input-sm" name="reason" id="reason${UNQID}">${resultData['REASON']}</textarea>
							<%-- <c:if test="${ROLE ne 'ROLE_AMLUSER'}">disabled="disabled"</c:if>> --%>
				...
				<div class="card-footer clearfix">
					...
					<c:if test="${f:contains(ROLE,'AMLO') || f:contains(ROLE,'MLRO')}">
