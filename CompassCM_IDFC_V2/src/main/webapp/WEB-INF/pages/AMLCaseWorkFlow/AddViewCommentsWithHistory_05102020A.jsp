	...
	$(document).ready(function(){		
	 ...
	 $("#addAlertComments"+id).click(function(){
		...
		$("#saveAndCloseComments"+id).click(function(){
			...
			alert(assignedBranchCode+", "+caseNos+", "+comments+", "+lastReviewedDate+", "+reassignToUserCode+", "+fraudIndicator+", "+flagType+", "+caseStatus+", "+removalReason+", "+addedToFalsePositive);
			var fullData ="CaseNos="+caseNos+"&LastReviewedDate="+lastReviewedDate+"&Comments="+comments+"&FlagType="+flagType+
						  "&CaseStatus="+caseStatus+"&OutcomeIndicator="+outcomeIndicator+"&RemovalReason="+removalReason+"&amlUserAddToMarkAll="+amlUserAddToMarkAll+
						  "&AddedToFalsePositive="+addedToFalsePositive+"&HighRiskReasonCode="+highRiskReasonCode+"&FraudIndicator="+fraudIndicator+
...
<div class="container commentsHistoryContainer" style="width: 100%;">
	...
	<ul class="nav nav-pills modalNav" role="tablist">
		...
		<%-- console.log(${f:startsWith(ROLE,tab)}); --%>
			<li role="presentation" <c:if test="${f:startsWith(ROLE,tab)}">class="active"</c:if>>
							...
							<div id="${tabIndex}addCommentsDiv${varStatus.index}" >
								<table class="table table-striped">
									...
									<tr>
										...
										<select class="form-control input-sm" name="amluserList" id="amluserList${UNQID}" style="width:100%">
										    <c:forEach var="AllUsersListMappings" items="${ALLUSERSLIST}">
												<option value="${AllUsersListMappings.USERCODE2}" <c:if test="${PREVIOUS_USERCODE eq userCode}">selected="selected"</c:if>>${AllUsersListMappings.USERNAME}</option>
											</c:forEach>
										</select>
										</td>
										<td colspan="3">&nbsp;</td>
											...
											<td width="20%">
												<select class="form-control input-sm" name="assignedBranch" id="assignedBranch${UNQID}" style="width:100%">
													<c:forEach var="AllBranchListMappings" items="${ALLBRANCHESLIST}">
														<option value="${AllBranchListMappings.BRANCHCODE}">${AllBranchListMappings.BRANCHCODE} - ${AllBranchListMappings.BRANCHNAME}</option>
													</c:forEach>
												</select>
											</td>
											<td colspan="3">&nbsp;</td>
									...
									<tr>
										...
										<c:if test="${ROLE eq 'AMLUSER'}">
											<td colspan="4">
												<textarea class="form-control input-sm validateComments autoCommentsSuggestion" name="amluserComments" id="amluserComments${UNQID}" ></textarea>
											</td>
										</c:if>
										<c:if test="${ROLE eq 'AMLO'}">
											<td colspan="4">
