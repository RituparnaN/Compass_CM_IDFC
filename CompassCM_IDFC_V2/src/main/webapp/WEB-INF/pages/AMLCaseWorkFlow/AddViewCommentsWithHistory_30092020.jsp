		...
		*/
		...
		$("input[name='amlUserAddToMarkAll']").change(function(){
			...
			// alert($(this).prop("checked"))
 			 if($(this).prop("checked")){
 				//if(customerId == ''){
                if(customerId == 'customerIdXYZABC'){
		...
		<c:forEach var="tab" items="${TABNAMES}" varStatus="tabIndex">
			<div role="tabpanel" class="tab-pane <c:if test="${f:startsWith(ROLE,tab)}">active</c:if>" id="${moduleCode}${UNQID}${tabIndex.index}">
				...
				<c:choose>
					...
					<c:otherwise>
						<c:if test="${f:contains(ROLE,tab) && action ne 'viewComments'}">
							...
							<div id="${tabIndex}addCommentsDiv${varStatus.index}" >
								<table class="table table-striped">
									<tr>
										...
										<c:if test="${ROLE eq 'AMLUSER' || ROLE eq 'AMLUSERL1' || ROLE eq 'AMLUSERL2' || ROLE eq 'AMLUSERL3'}">
										<td width="25%">
										...
										<c:if test="${ROLE eq 'MLRO' || ROLE eq 'MLROL1' || ROLE eq 'MLROL2'}">
										<td width="25%">
									...
									<c:if test="${ROLE eq 'AMLO' || ROLE eq 'MLRO' || ROLE eq 'MLROL1' || ROLE eq 'MLROL2'}"> 
									...
									<tr>
										...
										<c:if test="${ROLE eq 'USER'}">
											<td colspan="4">
												<textarea class="form-control input-sm validateComments" name="userComments" id="userComments${UNQID}" ></textarea>
											</td>
										</c:if>
										<c:if test="${ROLE eq 'AMLUSER' || ROLE eq 'AMLUSERL1' || ROLE eq 'AMLUSERL2' || ROLE eq 'AMLUSERL3'}">
											<td colspan="4">
												<textarea class="form-control input-sm validateComments" name="amluserComments" id="amluserComments${UNQID}" ></textarea>
											</td>
										</c:if>
										<c:if test="${ROLE eq 'AMLO'}">
											<td colspan="4">
												<textarea class="form-control input-sm validateComments" name="amloComments" id="amloComments${UNQID}" ></textarea>
											</td>
										</c:if>
										<c:if test="${ROLE eq 'MLRO' || ROLE eq 'MLROL1' || ROLE eq 'MLROL2'}">
											<td colspan="4">
												<textarea class="form-control input-sm validateComments" name="mlroComments" id="mlroComments${UNQID}" ></textarea>
											</td>
										</c:if>
									</tr>
								</table>
