...
<%-- <c:if test="${(currentCaseStatus eq '4' || currentCaseStatus eq '5' || currentCaseStatus eq '6') && ROLE eq 'AMLUSERL3'}">
	<c:set var="ROLE" value="AMLO"/>
</c:if>
 --%>
<style type="text/css">
	.modalNav li a{
...
<div class="container commentsHistoryContainer" style="width: 100%;">
						...
						<div class="card-body" style="text-align: center; padding: 5px 0px;">
							...
							<table class="table compassModuleDetailsSearchTable" style="margin-bottom: 0px;">
								...
								<tr>
									...
									<c:if test="${action eq 'viewComments'}">
										<td width="30%"><button type="button" class="btn btn-primary btn-sm" id="viewEvidence${UNQID}"><spring:message code="app.common.viewEvidence"/></button></td>
									</c:if>
									<!--
			...
			<li role="presentation">
			    <c:choose>
					<c:when test="${tab eq 'USER'}">
					   <a class="subTab nav-link <c:if test='${tabIndex.index == 0}'>active</c:if>" href="#${moduleCode}${UNQID}${tabIndex.index}" aria-controls="tab" role="tab" data-toggle="tab">BRANCHUSER</a>
					</c:when>
					<c:otherwise>
	...
	<div class="tab-content">
		<c:forEach var="tab" items="${TABNAMES}" varStatus="tabIndex">
			...
			<div role="tabpanel" class="tab-pane <c:if test="${f:startsWith(ROLE,tab)}">active</c:if>" id="${moduleCode}${UNQID}${tabIndex.index}">
				...
				<c:choose>
					...
					<c:otherwise>
						<c:if test="${f:startsWith(ROLE,tab) && action ne 'viewComments'}">
							...
							<div id="${tabIndex}addCommentsDiv${varStatus.index}" >
								<table class="table table-striped">
										    ...
										    <c:if test="${ROLE eq 'USER'}">
												<td width="25%">
													<input type="text" class="form-control input-sm datepicker" name="userAcctReviewedDate" 
													       id="userAcctReviewedDate${UNQID}" disabled style="width:100%" value="${CASECOMMENTDETAILS['CASEACCTREVIEWEDDATE']}"/>
												</td>
											</c:if>
											<c:if test="${ROLE eq 'AMLUSER'}">
												<td width="25%">
													<input type="text" class="form-control input-sm datepicker" name="amluserAcctReviewedDate" 
													       id="amluserAcctReviewedDate${UNQID}" disabled style="width:100%" value="${CASECOMMENTDETAILS['CASEACCTREVIEWEDDATE']}"/>
												</td>
											</c:if>
											<c:if test="${ROLE eq 'AMLO'}">
												<td width="25%">
													<input type="text" class="form-control input-sm datepicker" name="amloAcctReviewedDate" 
													       id="amloAcctReviewedDate${UNQID}" disabled style="width:100%" value="${CASECOMMENTDETAILS['CASEACCTREVIEWEDDATE']}"/>
												</td>
											</c:if>
											<c:if test="${ROLE eq 'MLRO'}">
												<td width="25%">
													<input type="text" class="form-control input-sm datepicker" name="mlroAcctReviewedDate" 
													       id="mlroAcctReviewedDate${UNQID}" disabled style="width:100%"  value="${CASECOMMENTDETAILS['CASEACCTREVIEWEDDATE']}"/>
												</td>
											</c:if>
												<td width="10%">&nbsp;</td>
										</c:if>
									...
									<tr>
										...
										<td width="25%">
										<select class="form-control input-sm" name="amluserList" id="amluserList${UNQID}" style="width:100%">
										    <c:forEach var="AllUsersListMappings" items="${ALLUSERSLIST}">
												<option value="${AllUsersListMappings.USERCODE2}" <c:if test="${AllUsersListMappings.USERCODE1 eq userCode}">selected="selected"</c:if>>${AllUsersListMappings.USERNAME}</option>
											</c:forEach>
										</select>
										</td>
										<td colspan="3">&nbsp;</td>
									...
									<tr>
											<c:if test="${action eq 'bonafideOkay' || action eq 'mergeCase'}">
												...
												<td colspan="2">
													<input type="checkbox" class="form-control input-sm" name="addToMarkAll" id="amlUserAddToMarkAll${UNQID}" style="width: 20px; margin-top: -4px;" >
												</td>
											</c:if>
										<c:if test="${action ne 'mergeCase' && action ne 'assignToBranchUserByAMLuser'}">
												...
												<td width="20%">
												  	<c:if test="${action eq 'bonafideOkay'}">
														<select class="form-control input-sm" name="amloList" id="amloList${UNQID}" style="width:100%" disabled>
												  	</c:if>
												  	<c:if test="${action ne 'bonafideOkay'}">
														<select class="form-control input-sm" name="amloList" id="amloList${UNQID}" style="width:100%">
												  	</c:if>	
													    <c:forEach var="AllUsersListMappings" items="${ALLUSERSLIST}">
										...
										<c:if test="${action eq 'assignToBranchUserByAMLuser'}">
											...
											<td width="20%">
												<select class="form-control input-sm" name="assignedBranch" id="assignedBranch${UNQID}" style="width:100%">
													<c:forEach var="AllBranchListMappings" items="${ALLBRANCHESLIST}">
														<option value="${AllBranchListMappings.BRANCHCODE}" <c:if test="${AllBranchListMappings.BRANCHCODE eq AllBranchListMappings.CASEBRANCHCODE}">selected="selected"</c:if>>${AllBranchListMappings.BRANCHCODE} - ${AllBranchListMappings.BRANCHNAME}</option>
													</c:forEach>
												</select>
											</td>
											<td colspan="3">&nbsp;</td>
									...
									<tr>		<%-- <td width="20%">MLRO List </td>
											...
											<c:if test="${action eq 'bonafideOkayByAMLO'}">
												<td width="20%">Mark All</td>
												<td colspan="2">
													<input type="checkbox" class="form-control input-sm" name="addToMarkAll" id="amloAddToMarkAll${UNQID}" style="width: 20px; margin-top: -4px;" >
												</td>
											</c:if>
											...
											<c:if test="${action eq 'bonafideOkayByMLRO'}">
												<td width="20%">Mark All</td>
												<td colspan="2">
													<input type="checkbox" class="form-control input-sm" name="addToMarkAll" id="mlroAddToMarkAll${UNQID}" style="width: 20px; margin-top: -4px;" >
												</td>
											</c:if>
											
										  	<c:if test="${action ne 'bonafideOkayByAMLO' && action ne 'bonafideOkayByMLRO'}">
											<td width="20%">List Of Users</td>
											<td width="25%">
											<select class="form-control input-sm" name="mlroList" id="mlroList${UNQID}" style="width:100%">
											    <c:forEach var="AllUsersListMappings" items="${ALLUSERSLIST}">
									...
									<tr>
										...
										<c:if test="${ROLE eq 'USER'}">
											<td colspan="4">
												<textarea class="form-control input-sm validateComments autoCommentsSuggestion" name="userComments" id="userComments${UNQID}" ></textarea>
											</td>
										</c:if>
										<c:if test="${ROLE eq 'AMLUSER'}">
											<td colspan="4">
												<textarea class="form-control input-sm validateComments autoCommentsSuggestion" name="amluserComments" id="amluserComments${UNQID}" >${CASECOMMENTDETAILS['NONSUSPICIOUSCOMMENTS']}</textarea>
											</td>
										</c:if>
										<c:if test="${ROLE eq 'AMLO'}">
											<td colspan="4">
												<textarea class="form-control input-sm validateComments autoCommentsSuggestion" name="amloComments" id="amloComments${UNQID}" ></textarea>
											</td>
										</c:if>
										<c:if test="${ROLE eq 'MLRO'}">
											<td colspan="4">
												<textarea class="form-control input-sm validateComments autoCommentsSuggestion" name="mlroComments" id="mlroComments${UNQID}" ></textarea>
											</td>
										</c:if>
									</tr>
								</table>
							...
							<div class="card-footer clearfix">
								<div class="pull-${dirR}">
									<c:if test="${flagType eq 'Y'}">
										<c:if test="${(f:contains(ROLE,'AMLO') || f:contains(ROLE,'MLRO')) && (currentCaseStatus eq '5' || currentCaseStatus eq '8') 
											...
											<c:choose>
												<c:when test="${LABELSITRCOUNT % 2 == 0}">
													<tr>
														...
														<c:choose>
														<c:when test="${f:contains(ALLLABELSMAP.key,'COMMENTS')}">
															<td width="25%">
																<textarea class="form-control input-sm" readonly="readonly">${ALLLABELSMAP.value}</textarea>
															</td>
														</c:when>
														<c:otherwise>
															<td width="25%">
																<input type="text" class="form-control input-sm viewCommentsTextarea" value="${ALLLABELSMAP.value}" readonly="readonly"/>
															</td>
														</c:otherwise>
														</c:choose>
														<td width="10%">&nbsp;</td>
												...
												<c:otherwise>
														...
														<c:when test="${f:contains(ALLLABELSMAP.key,'COMMENTS')}">
															<td width="25%">
																<textarea class="form-control input-sm viewCommentsTextarea" readonly="readonly">${ALLLABELSMAP.value}</textarea>
															</td>
														</c:when>
														<c:otherwise>
															<td width="25%">
