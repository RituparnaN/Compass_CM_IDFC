...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$(document).on("click", ".templateScreeningId"+id, function(){	
		...
		$(document).on("click", ".templateScan"+id, function(){	
			...
			if(isFreezedStatus == 'N'){
				...
				screenedNameMatches(isScreeningReferenceNo, templateSeqNo);
			}
		...
		//ajax call for Create and freeze 
		$(document).on("click", ".createAndFreeze"+id, function(){
		...
		$(document).on("click", ".createNewVersion"+id, function(){
			var templateSeqNo = $(this).closest('tr').find("td:eq(1)").text();
			var templateId = $(this).closest('tr').find("td:eq(2)").text();
			var subModuleCode = '${subModuleCode}';
			$.ajax({
	...
	function screenedNameMatches(screenedReferenceNo, templateSeqNo){
		$("#compassRTScanningModal").modal("show");
		$("#compassRTScanningModal-title").html("Real Time Scanning");
		$("#compassRTScanningModal-body").html("<br/><center><img alt='Loading...' src='${pageContext.request.contextPath}/includes/images/qde-loadder.gif'><center><br/>");
		var userRole = '${userRole}';
		...
		var fullData = "counter=0&filename="+screenedReferenceNo+"&FileImport=N&UserCode=ALL&RecordStatus=ALL&ScanningFromDate=&ScanningToDate=&ProcessingFromDate=&ProcessingToDate=&isActionAllowed="+isActionAllowed+"&templateSeqNo="+templateSeqNo;
		//alert(fullData);
