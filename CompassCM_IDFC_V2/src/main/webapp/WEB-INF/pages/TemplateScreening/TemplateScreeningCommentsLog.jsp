...
<c:set var="templateScreeningStatus" value="${templateScreeningStatus}"/>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		compassDatatable.construct(tableClass, "templateScreeningCommentsLog", true);
		...
		$("#saveComments"+id).click(function(){
		    var userComments = $("#comments"+id).val();
		    var templateScreeningStatus = '${templateScreeningStatus}';
		    var searchButton = "searchFalsePositive${UNQID}";
		    /* 
		    $("#searchResultGenericDiv").find("table").children("tbody").children("tr").each(function(){
				if($(this).children("td").children("input").prop("checked")){
					templateSeqNo = $(this).children("td").children("input").val();
				}
			});
		    */
		    var templateSeqNo = '${templateSeqNo}';
		    if(userComments == ''){
				alert('Please enter comments to proceed.');
			}else if(userComments.length > 4000){
				alert('Comments cannot exceed 4000 characters.');
			}else if(userComments != ""){
				$.ajax({
			 		url : '${pageContext.request.contextPath}/common/updateTemplateScreeningStatus',
			 		cache : true,
			 		type : 'POST',
			 		data : {templateSeqNo:templateSeqNo,templateScreeningStatus:templateScreeningStatus,userComments:userComments},
...
<div class="container" style="width: 100%;">
					...
					<div class="col-sm-6">
						<div class="card-body" style="text-align: center; padding: 5px 0px;">
							...
							<table class="table compassModuleDetailsSearchTable" style="margin-bottom: 0px;">
								<tr>
									<td width="30%"><spring:message code="app.common.TEMPLATESEQUENCENO"/></td>
									<td width="40%"><input type="text" class="form-control input-sm" name="moduleValue" id="moduleValue${UNQID}" value="${moduleValue}" readOnly/></td>
	...
	<c:if test="${moduleType ne 'addComments'}">
	<div class="row">
		<div class="col-sm-12">
			<div class="card card-primary">
										...
										<c:choose>
											<c:when test="${LABELSITRCOUNT % 2 == 0}">
												<tr>
													...
													<c:if test="${!f:containsIgnoreCase(ALLLABELSMAP.key, 'comments')}">
														<td width="30%">
...
<c:if test="${moduleType eq 'addComments'}">
...
<div class="col-sm-12">
	<div class="card card-primary">
		...
		<form action="javascript:void(0)" method="POST" id="AddCommentsForm${UNQID}" >
			...
			<div class="card-footer clearfix">
				<div class="pull-${dirR}">
					<button type="button" class="btn btn-success" name="saveComments" id="saveComments${UNQID}" >Save</button>
