...
$(document).ready(function() {
	...
	$('.panelSlidingTemplateScanning'+id).on("click", function (e) {
			...
			chkDuplicateTemplateId(templateId, function() {
				if(templateName == "")
				{
					...
					$.ajax({
						...
						cache: false,
						type: "POST",
						data: {templateId:templateId,templateType:templateType,templateName:templateName,searchButtonId:searchButtonId,subModuleCode:subModuleCode},
						success: function(res) {
							/* console.log(res);
							alert(res); */
							
							alert('Template added successfully')
							$("#screeningSearchParameter"+id).css("display", "block");
							$("#templateScreeningCreateResultPanel"+id).css("display", "block");
							$("#templateScreeningCreateResult"+id).html(res);
							$("#checklistForm"+id).html(res);
							$(panelBody).slideUp();
...
<div class="col-sm-12">
		...
		<div class="card card-primary">
			<div class="card-header panelSlidingTemplateScanning${UNQID} clearfix">
				<c:choose>
				<c:when test="${subModuleCode eq 'N.A.'}">
				    <h6 class="card-title pull-${dirL}"><spring:message code="app.common.createTemplateScreeningHeader"/></h6>
				</c:when>
				<c:otherwise>
					<h6 class="card-title pull-${dirL}"><spring:message code="app.common.searchTemplateScreeningHeader"/></h6>
				</c:otherwise>
				</c:choose>
				...
				<table class="table table-striped compassModuleDetailsSearchTable" style="margin-bottom: 0px;">
					<tbody>
						...
						<tr>
							<td>Template Type:</td>
							<td>
								<label class="btn btn-default btn-sm"><input type="radio" name="templateType"  value="e" >Export</label>
								<label class="btn btn-default btn-sm"><input type="radio" name="templateType"  value="i" >Import</label>
								...
								<label class="btn btn-default btn-sm"><input type="radio" name="templateType"  value="o" >Others</label>
	...
	<%-- <div class="card card-primary" id="screeningSearchParameter${UNQID}" style="display: none;">
			...
			<div id="checklistForm${UNQID}"></div>
	</div> --%>
	<div class="card card-primary" id="templateScreeningCreateResultPanel${UNQID}" style="display: none;">
			...
			<c:choose>
			   <c:when test="${subModuleCode eq 'N.A.'}">
				...
				<c:otherwise>
					<h6 class="card-title pull-${dirL}"><spring:message code="app.common.searchTemplateScreeningResult"/></h6>
				</c:otherwise>
			 </c:choose>	 
				<div class="btn-group pull-${dirR} clearfix">
					<span class="pull-right"><i class="collapsable fa fa-chevron-up"></i></span>
