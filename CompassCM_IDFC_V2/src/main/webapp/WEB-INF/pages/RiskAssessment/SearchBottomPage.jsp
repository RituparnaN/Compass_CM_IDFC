					...
					<td style="vertical-align: middle;">
						<c:choose>
							<c:when test="${(RISKASSESSMENTDATA['STATUSCODE'] eq 'CMO-P') && (CURRENTROLE eq 'ROLE_CM_OFFICER')}">
								<button compassRefNo="${RISKASSESSMENTDATA['COMPASSREFNO']}" onclick="continueRA(this)" class="btn btn-warning btn-xs">Continue Editing</button>
							</c:when>
							<c:when test="${(RISKASSESSMENTDATA['STATUSCODE'] eq 'CMM-P') && (CURRENTROLE eq 'ROLE_CM_MANAGER')}">
								<button compassRefNo="${RISKASSESSMENTDATA['COMPASSREFNO']}" onclick="continueRA(this)" class="btn btn-success btn-xs">Authorize</button>
							</c:when>
							<c:otherwise>
								<button compassRefNo="${RISKASSESSMENTDATA['COMPASSREFNO']}" onclick="continueRA(this)" class="btn btn-primary btn-xs">View</button>
							</c:otherwise>
						</c:choose>
						<button compassRefNo="${RISKASSESSMENTDATA['COMPASSREFNO']}" onclick="generateRAReport(this)" class="btn btn-info btn-xs">Generate Report</button>
					</td>
