...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var STATUS = "${PRODUCTRISKTABDATA['STATUS']}";
		var PRODUCTSSERVICES_WEIGHTAGE = '${PRODUCTSSERVICES_WEIGHTAGE}';

		if((CURRENTROLE == 'ROLE_CM_MANAGER') || (CURRENTROLE == 'ROLE_CM_OFFICER' && STATUS == 'CMM-A')){
			$("#productsServicesTabDiv"+id).find("input, textarea, select").prop("disabled", true);
		}
		 ...
		 $(".optionSelection").each(function(){
			if($(this).val() != ''){
				...
				setSelectedProductRiskRating($(this).val(), impactRiskRatingId, likelihoodRiskRatingId);
			} 
		}); 
		$("#calculateProductsServicesRisk"+id).click(function(){
			var productsServicesRiskRating = "High"; //dummy for testing
			...
			var provisionalRiskRating = (averageRiskRating * (PRODUCTSSERVICES_WEIGHTAGE/100)).toFixed(2);
			...
			$("#PRODUCTSSERVICES_SYSTEMRISKRATING"+id).val(averageRiskRating);		
			$("#PRODUCTSSERVICES_PROVISIONALRISKRATING"+id).val(provisionalRiskRating);	
			
			$("#prodServSysRiskRating").val(averageRiskRating);
			$("#prodServProvRiskRating").val(provisionalRiskRating);
	...
	function getProductsServicesRiskRating(elm, impactRiskRatingId, likelihoodRiskRatingId){
		//alert($(elm).val());
	...
	function setSelectedProductRiskRating(selectedRiskVal, impactRiskRatingId, likelihoodRiskRatingId){
	...
	function getProdServFinalRiskRating(elm, finalRiskRatingId1, finalRiskRatingId2){
		...
		if(finalRisk <= 9){
			...
			$("#prodServFinalRiskRating").val(finalRiskRatingVal);
			//$("#prodServFinalRiskRating").html(finalRiskRatingVal);
...
<div class="row compassrow${UNQID}" id="productsServicesTabDiv${UNQID}">
	<div class="col-sm-12">
						...
						<%-- <form action="javascript:void(0)" method="POST" id="searchMasterForm${UNQID}_${subGroupCode}"> --%>
							<div class="card-search-card" >
								<table class="table table-bordered table-striped formSearchTable riskAssessmentForm${UNQID}" style="margin-bottom: 0px;border-collapse:collapse;">
									...
									<c:set var="productFinalRiskVal" value="${f:split(PRODUCTRISKTABDATA['PRODUCTSFINALRISK'], ' - ')}"></c:set>
									<c:set var="productsFinalRiskRating" value="${f:split(productFinalRiskVal[1], '.')}"></c:set>
										...
										<tbody>
											...
											<tr>
												...
												<td style="width: 40%">
													...
													<td style="width: 15%">
														<select class="form-control input-sm optionSelection" id="OPTION_${questionId}_${UNQID}" name="PRODUCTSSERVICES_OPTION_${questionId}.${questionVersion}" 
																onchange="getProductsServicesRiskRating(this,'IMPACTRISKRATING_${questionId}_${UNQID}', 'LIKELIHOODRISKRATING_${questionId}_${UNQID}')">
													...
													<td style="width: 10%">
														<textarea class="form-control input-sm" id="REMARKS_${questionId}_${UNQID}" name="PRODUCTSSERVICES_REMARKS_${questionId}">${remarksData}</textarea>
													</td>
													<td style="width: 10%; text-align: center;">
													<select class="form-control input-sm risk1" id="IMPACTRISKRATING_${questionId}_${UNQID}" name="PRODUCTSSERVICES_IRR_${questionId}" 
																disabled="disabled"	>
													...
													<td style="width: 10%; text-align: center;">
													<select class="form-control input-sm risk2" id="LIKELIHOODRISKRATING_${questionId}_${UNQID}" name="PRODUCTSSERVICES_LRR_${questionId}" disabled="disabled">
											...
											<tr>
												...
												<td colspan="2">
													<select class="form-control input-sm optionSelection" id="OPTION_${questionId}_${UNQID}" name="PRODUCTSSERVICES_OPTION_${questionId}.${questionVersion}" 
														...
														<%-- 	<select class="form-control input-sm" id="OPTION_${questionId}_${UNQID}" name="PRODUCTSSERVICES_${questionId}" 
																onchange="getProductsServicesRiskRating(this,'IMPACTRISKRATING_${questionId}_${UNQID}', 'LIKELIHOODRISKRATING_${questionId}_${UNQID}')">
															<option value="">Select Option</option>
															<c:forEach var="optionsList" items="${OPTIONSLIST}">
																<c:set var="optionsRiskRatingList" value="${optionsList[questionId]}"></c:set>
							...
							<tbody>
									<tr>
										...
										<td width="20%" style="text-align: center;">	
											<button type="button" class="btn btn-primary btn-sm" id="calculateProductsServicesRisk${UNQID}">Calculate</button>
										</td>
										<td width="5%">&nbsp;</td>
										<td colspan="2">
											<input type="text" class="form-control input-sm"  readonly="readonly" 
											       name="PRODUCTSSERVICES_SYSTEMRISKRATING" id="PRODUCTSSERVICES_SYSTEMRISKRATING${UNQID}" 
											       value="${PRODUCTRISKTABDATA['PRODUCTSSYSTEMGENRISK']}"/>
										</td>
									</tr>
									<tr>
										<td width="25%">
										...
										<td width="20%">
											<input type="text" readonly="readonly" class="form-control input-sm" name="PRODUCTSSERVICES_PROVISIONALRISKRATING" 
											       id="PRODUCTSSERVICES_PROVISIONALRISKRATING${UNQID}" value="${PRODUCTRISKTABDATA['PRODUCTSPROVISRISK']}">
										</td>
										<td width="5%">&nbsp;</td>
										<td width="25%">
											Final Risk Rating
										...
										<td width="10%">
											<%--<select class="form-control input-sm" name="PRODUCTSSERVICES_FINALRISKRATING" id="PRODUCTSSERVICES_FINALRISKRATING${UNQID}"
													onchange="getProdServFinalRiskRating(this, 'PRODUCTSSERVICES_FINALRISKRATING${UNQID}')">
												<option value="">Select Risk Rating</option>
												<option value="3" <c:if test="${PRODUCTRISKTABDATA['PRODUCTSFINALRISK'] eq 'High - 3'}">selected="selected"</c:if>>High</option>
												<option value="2" <c:if test="${PRODUCTRISKTABDATA['PRODUCTSFINALRISK'] eq 'Medium - 2'}">selected="selected"</c:if>>Medium</option>
												<option value="1" <c:if test="${PRODUCTRISKTABDATA['PRODUCTSFINALRISK'] eq 'Low - 1'}">selected="selected"</c:if>>Low</option>
											</select>--%>
											<select class="form-control input-sm" name="PRODUCTSSERVICES_FINALRISKRATING1" id="PRODUCTSSERVICES_FINALRISKRATING1${UNQID}"
												onchange="getProdServFinalRiskRating(this, 'PRODUCTSSERVICES_FINALRISKRATING1${UNQID}', 'PRODUCTSSERVICES_FINALRISKRATING2${UNQID}')">
												<c:forEach var="i" begin="0" end="9">
													<option value="${i}" <c:if test="${productsFinalRiskRating[0] eq i}">selected="selected"</c:if>>${i}</option>
												</c:forEach>
											</select>
										</td>
										<td width="10%">
											 <select class="form-control input-sm" name="PRODUCTSSERVICES_FINALRISKRATING2" id="PRODUCTSSERVICES_FINALRISKRATING2${UNQID}"
													onchange="getProdServFinalRiskRating(this, 'PRODUCTSSERVICES_FINALRISKRATING1${UNQID}', 'PRODUCTSSERVICES_FINALRISKRATING2${UNQID}')">
												<c:forEach var="i" begin="00" end="99">
													<option value=".${i}" <c:if test="${productsFinalRiskRating[1] eq i}">selected="selected"</c:if>>.${i}</option>
												</c:forEach>
										</td>
									</tr>
									<tr>
										...
										<td colspan="5">
											<textarea rows="2" cols="2" class="form-control" name="PRODUCTSSERVICES_RISKRATINGREASON" id="PRODUCTSSERVICES_RISKRATINGREASON${UNQID}">${PRODUCTRISKTABDATA['PRODUCTSREMARKS']}</textarea>
										</td>
									</tr>
							</tbody>
						</table>
