...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var STATUS = "${TXNRISKTABDATA['STATUS']}";
		var TRANSACTIONS_WEIGHTAGE = '${TRANSACTIONS_WEIGHTAGE}';

		if((CURRENTROLE == 'ROLE_CM_MANAGER') || (CURRENTROLE == 'ROLE_CM_OFFICER' && STATUS == 'CMM-A')){
			$("#transactionsTabDiv"+id).find("input, textarea, select").prop("disabled", true);
		}
		 ...
		 $(".optionSelection").each(function(){
			if($(this).val() != ''){
				...
				setSelectedTxnRiskRating($(this).val(), impactRiskRatingId, likelihoodRiskRatingId);
			} 
		}); 
		
		$("#calculateTransactionsRisk"+id).click(function(){
			var transactionsRiskRating = "High"; //dummy for testing
			...
			var provisionalRiskRating = (averageRiskRating * (TRANSACTIONS_WEIGHTAGE/100)).toFixed(2);
			...
			$("#TRANSACTIONS_SYSTEMRISKRATING"+id).val(averageRiskRating);		
			$("#TRANSACTIONS_PROVISIONALRISKRATING"+id).val(provisionalRiskRating);	
			
			$("#txnSysRiskRating").val(averageRiskRating);
			$("#txnProvRiskRating").val(provisionalRiskRating);
	...
	function getTransactionsRiskRating(elm, impactRiskRatingId, likelihoodRiskRatingId){
		//alert($(elm).val());
	...
	function setSelectedTxnRiskRating(selectedRiskVal, impactRiskRatingId, likelihoodRiskRatingId){
	...
	function getTxnFinalRiskRating(elm, finalRiskRatingId1, finalRiskRatingId2){
		...
		if(finalRisk <= 9){
			...
			$("#txnFinalRiskRating").val(finalRiskRatingVal);
			//$("#txnFinalRiskRating").html(finalRiskRatingVal);
...
<div class="row compassrow${UNQID}" id="transactionsTabDiv${UNQID}">
	<div class="col-sm-12">
	<%-- <c:forEach var="subGroupcode" items="${SUBGROUPSLIST}"> --%>
						...
						<%-- <form action="javascript:void(0)" method="POST" id="searchMasterForm${UNQID}_${subGroupCode}"> --%>
							<div class="card-search-card" >
								<table class="table table-bordered table-striped formSearchTable riskAssessmentForm${UNQID}" style="margin-bottom: 0px;border-collapse:collapse;">
									...
									<c:set var="transactionsFinalRiskVal" value="${f:split(TXNRISKTABDATA['TRANSACTIONSFINALRISK'], ' - ')}"></c:set>
									<c:set var="transactionsFinalRiskRating" value="${f:split(transactionsFinalRiskVal[1], '.')}"></c:set>
										...
										<tbody>
											...
											<tr>
												...
												<td style="width: 40%">
													...
													<td style="width: 15%">
														<select class="form-control input-sm optionSelection" id="OPTION_${questionId}_${UNQID}" name="TRANSACTIONS_OPTION_${questionId}.${questionVersion}" 
																onchange="getTransactionsRiskRating(this,'IMPACTRISKRATING_${questionId}_${UNQID}', 'LIKELIHOODRISKRATING_${questionId}_${UNQID}')">
													...
													<td style="width: 10%">
														<textarea class="form-control input-sm" id="REMARKS_${questionId}_${UNQID}" name="TRANSACTIONS_REMARKS_${questionId}">${remarksData}</textarea>
													</td>
													<td style="width: 10%; text-align: center;">
													<select class="form-control input-sm risk1" id="IMPACTRISKRATING_${questionId}_${UNQID}" name="TRANSACTIONS_IRR_${questionId}" 
																disabled="disabled"	>
													...
													<td style="width: 10%; text-align: center;">
													<select class="form-control input-sm risk2" id="LIKELIHOODRISKRATING_${questionId}_${UNQID}" name="TRANSACTIONS_LRR_${questionId}" disabled="disabled">
											...
											<tr>
												...
												<td colspan="2">
													...
													<%--	<select class="form-control input-sm" id="OPTION_${questionId}_${UNQID}" name="TRANSACTIONS_${questionId}" 
																onchange="getTransactionsRiskRating(this,'IMPACTRISKRATING_${questionId}_${UNQID}', 'LIKELIHOODRISKRATING_${questionId}_${UNQID}')">
															<option value="">Select Option</option>
															<c:forEach var="optionsList" items="${OPTIONSLIST}">
																<c:set var="optionsRiskRatingList" value="${optionsList[questionId]}"></c:set>
		...
		<div class="card card-primary panel_RiskCalculationForm" style="margin-top: 20px;  margin-bottom: 0;">
			...
			<div class="panelSearchForm">
				<%-- <form action="javascript:void(0)" method="POST" id="searchMasterForm_TransactionsRiskCal_${UNQID}"> --%>
					<div class="card-search-card" >
							...
							<tbody>
									<tr>
										...
										<td width="20%" style="text-align: center;">	
											<button type="button" class="btn btn-primary btn-sm" id="calculateTransactionsRisk${UNQID}">Calculate</button>
										</td>
										<td width="5%">&nbsp;</td>
										<td colspan="2">
											<input type="text" class="form-control input-sm"  readonly="readonly" 
											       name="TRANSACTIONS_SYSTEMRISKRATING" id="TRANSACTIONS_SYSTEMRISKRATING${UNQID}" 
											       value="${TXNRISKTABDATA['TRANSACTIONSSYSTEMGENRISK']}"/>
										</td>
									</tr>
									<tr>
										<td width="25%">
										...
										<td width="20%">
											<input type="text" readonly="readonly" class="form-control input-sm" name="TRANSACTIONS_PROVISIONALRISKRATING" 
											       id="TRANSACTIONS_PROVISIONALRISKRATING${UNQID}" value="${TXNRISKTABDATA['TRANSACTIONSPROVISRISK']}">
										</td>
										<td width="5%">&nbsp;</td>
										<td width="25%">
											Final Risk Rating
										...
										<td width="10%">
											<%--<select class="form-control input-sm" name="TRANSACTIONS_FINALRISKRATING" id="TRANSACTIONS_FINALRISKRATING${UNQID}"
													onchange="getTxnFinalRiskRating(this, 'TRANSACTIONS_FINALRISKRATING${UNQID}')">
												<option value="">Select Risk Rating</option>
												<option value="3" <c:if test="${TXNRISKTABDATA['TRANSACTIONSFINALRISK'] eq 'High - 3'}">selected="selected"</c:if>>High</option>
												<option value="2" <c:if test="${TXNRISKTABDATA['TRANSACTIONSFINALRISK'] eq 'Medium - 2'}">selected="selected"</c:if>>Medium</option>
												<option value="1" <c:if test="${TXNRISKTABDATA['TRANSACTIONSFINALRISK'] eq 'Low - 1'}">selected="selected"</c:if>>Low</option>
											</select>--%>
											<select class="form-control input-sm" name="TRANSACTIONS_FINALRISKRATING1" id="TRANSACTIONS_FINALRISKRATING1${UNQID}"
												onchange="getTxnFinalRiskRating(this, 'TRANSACTIONS_FINALRISKRATING1${UNQID}', 'TRANSACTIONS_FINALRISKRATING2${UNQID}')">
												<c:forEach var="i" begin="0" end="9">
													<option value="${i}" <c:if test="${transactionsFinalRiskRating[0] eq i}">selected="selected"</c:if>>${i}</option>
												</c:forEach>
											</select>
										</td>
										<td width="10%">
											 <select class="form-control input-sm" name="TRANSACTIONS_FINALRISKRATING2" id="TRANSACTIONS_FINALRISKRATING2${UNQID}"
													onchange="getTxnFinalRiskRating(this, 'TRANSACTIONS_FINALRISKRATING1${UNQID}', 'TRANSACTIONS_FINALRISKRATING2${UNQID}')">
												<c:forEach var="i" begin="00" end="99">
													<option value=".${i}" <c:if test="${transactionsFinalRiskRating[1] eq i}">selected="selected"</c:if>>.${i}</option>
												</c:forEach>
											 </select>
										</td>
									</tr>
									<tr>
										...
										<td colspan="5">
											<textarea rows="2" cols="2" class="form-control" name="TRANSACTIONS_RISKRATINGREASON" id="TRANSACTIONS_RISKRATINGREASON${UNQID}">${TXNRISKTABDATA['TRANSACTIONSREMARKS']}</textarea>
										</td>
									</tr>
							</tbody>
						</table>
