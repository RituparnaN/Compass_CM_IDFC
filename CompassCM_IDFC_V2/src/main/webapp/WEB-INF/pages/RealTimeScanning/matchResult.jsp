...
String isActionAllowed = request.getParameter("isActionAllowed") == null ? "Y":(String)request.getParameter("isActionAllowed");
String uniqueNumber = (String)request.getAttribute("StrUniqueNumber");
String FromBulkScreening = (String)request.getAttribute("FromBulkScreening");
//System.out.println(FromBulkScreening);
...
if(alInputParametersName != null && alInputParametersName.size() > 0 && alInputParametersValue != null && alInputParametersValue.size() > 0 )
{
  ...
  { 
    ...
    if(alInputParametersName.get(i).equals("DRIVINGLICENSENO"))
    {
    ...
    {
     arrayParamsPart1Name[5] = alInputParametersName.get(i).toString(); 
     arrayParamsPart1Value[5] = alInputParametersValue.get(i).toString();
    } 
    ...
    if(alInputParametersName.get(i).equals("BIRTHCERTIFICATENO"))
    {
    ...
    {
     arrayParamsPart2Name[5] = alInputParametersName.get(i).toString(); 
     arrayParamsPart2Value[5] = alInputParametersValue.get(i).toString();
    } 
}
}
	...
	$(document).ready(function(){
		...
		$(".markMatchAction").click(function(){
			...
			var url = "${pageContext.request.contextPath}/common/AddRTComments?RecordStatus=<%=recordStatus%>"+
					  ...
					  "&isActionAllowed=<%=isActionAllowed%>&selected="+checkboxes+"&action="+action+"&counter="+counter+"&FileName="+fileName+"&FileImport="+FileImport;
			// alert(url);			
			$.ajax({
		 		url : url,
		 		cache : true,
		...
		$("#viewMatchEvedence").click(function(){
			$('[data-toggle="tooltip"]').tooltip("hide");
			compassFileUpload.init("attachEvedence","${pageContext.request.contextPath}","realTimScreening","compassRTScanningModal","N","N","");
		});
		...
		$("#compassFileUploadModal").on('hidden.bs.modal', function () {
			...
			*/
			...
			"&ScanningFromDate=<%=scanningFromDate%>&ScanningToDate=<%=scanningToDate%>"+
			"&ProcessingFromDate=<%=processingFromDate%>&ProcessingToDate=<%=processingToDate%>&isActionAllowed=<%=isActionAllowed%>";
...
<!--
	...
	function openSelectedListDetails(listname,listid,checkboxval)
	{
		...
		if(listname.substr(0, 11) == 'DOWJONELIST'){
			...
			//window.open('<%=contextPath%>/common/RTlistDetails?listname='+listname+'&listid='+listid,'','width=550,height=635,left=0,top=0,screenX=250,screenY=300,OuterWidth=370,OuterHeight=150,toolbar=yes,location=no,resizable=yes, scrollbars=yes,menubar=yes');
			window.open('${pageContext.request.contextPath}/common/realTimeScanningListIdDetails?listCode='+listname+'&listId='+listid,'','width=550,height=635,left=0,top=0,screenX=250,screenY=300,OuterWidth=370,OuterHeight=150,toolbar=yes,location=no,resizable=yes, scrollbars=yes,menubar=yes');
		} 
        }
        else
        {
         ...
         window.open('${pageContext.request.contextPath}/common/realTimeScanningListIdDetails?listCode='+listname+'&listId='+listid,'','width=550,height=635,left=525,top=0,screenX=250,screenY=300,OuterWidth=370,OuterHeight=150,toolbar=yes,location=no,resizable=yes, scrollbars=yes,menubar=yes');
        }
	}
	...
	function addEntityToAcceptList(key,AcctCustID,ActTaken) {
		...
		var url = "${pageContext.request.contextPath}/common/AddRTComments?UserCode=<%=userCode%>&RecordStatus=<%=recordStatus%>"+
				  ...
				  "&ProcessingToDate=<%=processingToDate%>&isActionAllowed=<%=isActionAllowed%>&selected="+checkboxes+"&action="+action+
	...
	*/
	...
	{	
		if('${FromBulkScreening}' == 'N'){
			window.open('<%=contextPath%>/common/printPage?uniqueNumber=<%=uniqueNumber%>&UserCode=<%=userCode%>&RecordStatus=<%=recordStatus%>&ScanningFromDate=<%=scanningFromDate%>&ScanningToDate=<%=scanningToDate%>&ProcessingFromDate=<%=processingFromDate%>&ProcessingToDate=<%=processingToDate%>&isActionAllowed=<%=isActionAllowed%>&FromBulkScreening=<%=FromBulkScreening%>&filename='+key+'&key='+key,'','width=1000,height=680,left=0,top=0,screenX=250,screenY=300,OuterWidth=370,OuterHeight=150,toolbar=yes,location=no,resizable=yes, scrollbars=yes,menubar=yes');
		}else{
			window.open('<%=contextPath%>/common/printPage?UserCode=<%=userCode%>&RecordStatus=<%=recordStatus%>&ScanningFromDate=<%=scanningFromDate%>&ScanningToDate=<%=scanningToDate%>&ProcessingFromDate=<%=processingFromDate%>&ProcessingToDate=<%=processingToDate%>&isActionAllowed=<%=isActionAllowed%>&FromBulkScreening=Y&filename='+key+'&key='+key,'','width=1000,height=680,left=0,top=0,screenX=250,screenY=300,OuterWidth=370,OuterHeight=150,toolbar=yes,location=no,resizable=yes, scrollbars=yes,menubar=yes');
		}
	}
	...
	function resetAllSelections()
	{
       ...
       var eleLen = document.viewmatches.elements.length;
	   var ele = eval("document.viewmatches.elements");
	   if(document.viewmatches.mainCheckBox.checked){
	   for(i=0;i<eleLen;i++)
		{
		  if(ele[i].type == "checkbox")
		  { 
...
<div class="row">
	<div class="col-sm-12">
			...
			<table class="table table-bordered table-striped ">
				<tr>
					...
					<th class="no-sort" style="text-align: center;">
						<!-- <input type="checkbox" name="mainCheckBox" id="mainCheckBox" onClick="resetAllSelections()"> -->
						<input type="checkbox" class="checkbox-check-all" name="mainCheckBox" id="mainCheckBox">
					</th>
       ...
       if((!(rtMatchResultVO.getCustomerName().trim()).equals(strNextCustName+"")) || l_intSize == resultedRecords.size()-1) {
        ...
        -->
        <%
        if(isActionAllowed.equalsIgnoreCase("Y")){
        %>
        ...
        <% } else { %>
        <button type="button" class="btn btn-primary btn-sm" id="viewMatchEvedence">View Evidence</button>
        <% }  %>
	    </TD>
	    </TR> 
		<table>
...
%>
		...
		<table border=0 width="100%">
			<tr>
				<td align="center" colspan="7">
					<button type="button" class="btn btn-warning btn-sm" action="openPrintFormatPage" onclick="openPrintFormatPage('')" >PRINT MATCH</button>
				</td>
			</tr>
		</table>
<%
