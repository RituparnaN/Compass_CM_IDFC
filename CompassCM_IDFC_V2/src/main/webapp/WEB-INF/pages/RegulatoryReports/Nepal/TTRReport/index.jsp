...
<script type="text/javascript">
	$(document).ready(function() {
		var id = '${UNQID}';
		compassTopFrame.init(id, 'NPLTTRReportTable', 'dd/mm/yy');
		
		$(".panelSlidingNPLTTReport"+id).on("click", function(e){
			var mainRow = $(this).parents(".compassrow"+id);
			compassTopFrame.searchPanelSliding(id, mainRow, 'NPLTTRReportSerachResultPanel');
		});
		
		$("#searchMasterForm"+id).submit(function(e){
			var submitButton = $("#searchNPLTTReport"+id);
			compassTopFrame.submitForm(id, e, submitButton, 'NPLTTRReportSerachResultPanel',
					'NPLTTRReportSerachResult', '${pageContext.request.contextPath}/common/searchGenericMaster',
					'${pageContext.request.contextPath}/includes/images/qde-loadder.gif');
		});

		$("#exportReportingNPLTTRFile").click(function(){
			var reportType = "NPLTTR";
			...
			if(confirm("Are you sure you want to Generate Text Reporting File?")){
				$.fileDownload("${pageContext.request.contextPath}/common/generateNPLTTRReportingFile?"+formData+"&reportType="+reportType+"&reportingFileType="+reportingFileType, {
				    httpMethod : "GET",
		...
		$("#exportReportingNPLTTRXMLFile").click(function(){
			var reportType = "NPLTTR";
...
<div class="row compassrow${UNQID}">
	<div class="col-sm-12">
		<div class="card card-primary panel_NPLTTRReport">
			<div class="card-header panelSlidingNPLTTReport${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}"><spring:message code="app.common.NPLTTRReportHeader"/></h6>
			...
			<input type="hidden" name="bottomPageUrl" value="RegulatoryReports/Nepal/TTRReport/SearchBottomPage">
			<div class="card-search-card" >
				<table class="table table-striped formSearchTable NPLTTRReportTable" style="margin-bottom: 0px;">
					<tbody>
			...
			<div class="card-footer clearfix">
				<div class="pull-${dirR}">
					<button type="submit" id="searchNPLTTReport${UNQID}" class="btn btn-success btn-sm"><spring:message code="app.common.searchButton"/></button>
					<input type="reset" id="clearNPLTTReport${UNQID}" class="btn btn-danger btn-sm" name="clearNPLTTReport" value="Clear"/>
		...
		<div class="card card-primary" id="NPLTTRReportSerachResultPanel${UNQID}" style="display: none;">
			<div class="card-header panelSlidingNPLTTReport${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}"><spring:message code="app.common.NPLTTRReportResultHeader"/></h6>
				<div class="btn-group pull-${dirR} clearfix">
					<span class="pull-right"><i class="collapsable fa fa-chevron-up"></i></span>
				</div>
			</div>
			<div id="NPLTTRReportSerachResult${UNQID}"></div>
			
			<div class="card-footer action-footer clearfix">
				<div class="pull-${dirR} clearfix">
					<!-- <button type="button" class="btn btn-primary" id="exportReportingNPLTTRFile">Export Reporting File</button>
					<button type="button" class="btn btn-primary" id="exportReportingNPLTTRXMLFile">Export XML File</button> -->
