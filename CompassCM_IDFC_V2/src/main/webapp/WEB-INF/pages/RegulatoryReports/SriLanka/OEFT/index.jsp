...
<script type="text/javascript">
	$(document).ready(function() {
		var id = '${UNQID}';
		compassTopFrame.init(id, 'oeftTable', 'dd/mm/yy');
		
		$(".panelSlidingSLOEFTeport"+id).on("click", function(e){
			var mainRow = $(this).parents(".compassrow"+id);
			compassTopFrame.searchPanelSliding(id, mainRow, 'SLOEFTReportSerachResultPanel');
		});
		
		$("#searchMasterForm"+id).submit(function(e){
			var submitButton = $("#searchSLOEFTeport"+id);
			compassTopFrame.submitForm(id, e, submitButton, 'SLOEFTReportSerachResultPanel',
					'SLOEFTReportSerachResult', '${pageContext.request.contextPath}/common/searchGenericMaster',
					'${pageContext.request.contextPath}/includes/images/qde-loadder.gif');
		});

		$("#exportReportingSLOEFTFile").click(function(){
			var reportType = "SLOEFT";
			...
			if(confirm("Are you sure you want to Generate Text Reporting File?")){
				$.fileDownload("${pageContext.request.contextPath}/common/generateSLOEFTReportingFile?"+formData+"&reportType="+reportType+"&reportingFileType="+reportingFileType, {
				    httpMethod : "GET",
		...
		$("#exportReportingSLOEFTXMLFile").click(function(){
			var reportType = "SLOEFT";
...
<div class="row compassrow${UNQID}">
	<div class="col-sm-12">
		<div class="card card-primary panel_SLOEFTReport">
			<div class="card-header panelSlidingSLOEFTeport${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}"><spring:message code="app.common.SLOEFTReportHeader"/></h6>
			...
			<input type="hidden" name="bottomPageUrl" value="RegulatoryReports/SriLanka/OEFT/SearchBottomPage">
			<div class="card-search-card" >
				<table class="table table-striped formSearchTable SLOEFTReportTable" style="margin-bottom: 0px;">
					<tbody>
			...
			<div class="card-footer clearfix">
				<div class="pull-${dirR}">
					<button type="submit" id="searchSLOEFTeport${UNQID}" class="btn btn-success btn-sm"><spring:message code="app.common.searchButton"/></button>
					<input type="reset" id="clearSLOEFTeport${UNQID}" class="btn btn-danger btn-sm" name="clearSLOEFTeport" value="Clear"/>
		...
		<div class="card card-primary" id="SLOEFTReportSerachResultPanel${UNQID}" style="display: none;">
			<div class="card-header panelSlidingSLOEFTeport${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}"><spring:message code="app.common.SLOEFTReportResultHeader"/></h6>
				<div class="btn-group pull-${dirR} clearfix">
					<span class="pull-right"><i class="collapsable fa fa-chevron-up"></i></span>
				</div>
			</div>
			<div id="SLOEFTReportSerachResult${UNQID}"></div>
			
			<div class="card-footer action-footer clearfix">
				<div class="pull-${dirR} clearfix">
					<button type="button" class="btn btn-primary" id="exportReportingSLOEFTFile">Export Reporting File</button>
					<!-- <button type="button" class="btn btn-primary" id="exportReportingSLOEFTXMLFile">Export XML File</button> -->
