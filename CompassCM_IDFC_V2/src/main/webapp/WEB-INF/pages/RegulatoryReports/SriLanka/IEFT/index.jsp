...
<script type="text/javascript">
	$(document).ready(function() {
		var id = '${UNQID}';
		compassTopFrame.init(id, 'ieftTable', 'dd/mm/yy');
		
		$(".panelSlidingSLIEFTeport"+id).on("click", function(e){
			var mainRow = $(this).parents(".compassrow"+id);
			compassTopFrame.searchPanelSliding(id, mainRow, 'SLIEFTReportSerachResultPanel');
		});
		
		$("#searchMasterForm"+id).submit(function(e){
			var submitButton = $("#searchSLIEFTeport"+id);
			compassTopFrame.submitForm(id, e, submitButton, 'SLIEFTReportSerachResultPanel',
					'SLIEFTReportSerachResult', '${pageContext.request.contextPath}/common/searchGenericMaster',
					'${pageContext.request.contextPath}/includes/images/qde-loadder.gif');
		});

		$("#exportReportingSLIEFTFile").click(function(){
			var reportType = "SLIEFT";
			...
			if(confirm("Are you sure you want to Generate Text Reporting File?")){
				$.fileDownload("${pageContext.request.contextPath}/common/generateSLIEFTReportingFile?"+formData+"&reportType="+reportType+"&reportingFileType="+reportingFileType, {
				    httpMethod : "GET",
		...
		$("#exportReportingSLIEFTXMLFile").click(function(){
			var reportType = "SLIEFT";
...
<div class="row compassrow${UNQID}">
	<div class="col-sm-12">
		<div class="card card-primary panel_SLIEFTReport">
			<div class="card-header panelSlidingSLIEFTeport${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}"><spring:message code="app.common.SLIEFTReportHeader"/></h6>
			...
			<input type="hidden" name="bottomPageUrl" value="RegulatoryReports/SriLanka/IEFT/SearchBottomPage">
			<div class="card-search-card" >
				<table class="table table-striped formSearchTable SLIEFTReportTable" style="margin-bottom: 0px;">
					<tbody>
			...
			<div class="card-footer clearfix">
				<div class="pull-${dirR}">
					<button type="submit" id="searchSLIEFTeport${UNQID}" class="btn btn-success btn-sm"><spring:message code="app.common.searchButton"/></button>
					<input type="reset" id="clearSLIEFTeport${UNQID}" class="btn btn-danger btn-sm" name="clearSLIEFTeport" value="Clear"/>
		...
		<div class="card card-primary" id="SLIEFTReportSerachResultPanel${UNQID}" style="display: none;">
			<div class="card-header panelSlidingSLIEFTeport${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}"><spring:message code="app.common.SLIEFTReportResultHeader"/></h6>
				<div class="btn-group pull-${dirR} clearfix">
					<span class="pull-right"><i class="collapsable fa fa-chevron-up"></i></span>
				</div>
			</div>
			<div id="SLIEFTReportSerachResult${UNQID}"></div>
			
			<div class="card-footer action-footer clearfix">
				<div class="pull-${dirR} clearfix">
					<button type="button" class="btn btn-primary" id="exportReportingSLIEFTFile">Export Reporting File</button>
					<!-- <button type="button" class="btn btn-primary" id="exportReportingSLIEFTXMLFile">Export XML File</button> -->
