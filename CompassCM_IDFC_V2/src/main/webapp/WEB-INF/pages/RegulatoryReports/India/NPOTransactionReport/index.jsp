...
<script type="text/javascript">
	$(document).ready(function() {
		var id = '${UNQID}';
		compassTopFrame.init(id, 'ntrTable', 'dd/mm/yy');
		
		$(".panelSlidingNTReport"+id).on("click", function(e){
			var mainRow = $(this).parents(".compassrow"+id);
			compassTopFrame.searchPanelSliding(id, mainRow, 'npoTransactionReportSerachResultPanel');
		});
		
		var buttonId = "";
		$("#viewGeneratedNtrFiles"+id).click(function(){
			buttonId = $(this)[0].id;
		});
		$("#searchNTReport"+id).click(function(){
			buttonId = $(this)[0].id;
		});
		
		$("#searchMasterForm"+id).submit(function(e){
			if(buttonId ==  "viewGeneratedNtrFiles"+id){
				var submitButton = $("#viewGeneratedNtrFiles"+id);
				
				let buttonTextDuringOperation = "Getting Generated Files..";
				compassTopFrame.submitFormInsteadOfSearch(id, e, submitButton, 'npoTransactionReportSerachResultPanel',
						'npoTransactionReportSerachResult', '${pageContext.request.contextPath}/common/viewGeneratedNTRFilesMaster?viewFilePageUrl=RegulatoryReports/India/NPOTransactionReport/ViewGeneratedFile'+'&viewNtrButton='+buttonId,
						'${pageContext.request.contextPath}/includes/images/qde-loadder.gif',buttonTextDuringOperation);
								
			}else if(buttonId ==  "searchNTReport"+id){
				var submitButton = $("#searchNTReport"+id);
				compassTopFrame.submitForm(id, e, submitButton, 'npoTransactionReportSerachResultPanel',
						'npoTransactionReportSerachResult', '${pageContext.request.contextPath}/common/searchGenericMaster',
...
<div class="row compassrow${UNQID}">
	<div class="col-sm-12">
		<div class="card card-primary panel_npoTransactionReport">
			<div class="card-header panelSlidingNTReport${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}"><spring:message code="app.common.npoTransactionReportHeader"/></h6>
			...
			<input type="hidden" name="bottomPageUrl" value="RegulatoryReports/India/NPOTransactionReport/SearchBottomPage">
			<div class="card-search-card" >
				<table class="table table-striped formSearchTable npoTransactionReportTable" style="margin-bottom: 0px;">
					<tbody>
			...
			<div class="card-footer clearfix">
				<div class="pull-${dirR}">
					<!-- DOWNLOAD GENERATED NTR FILES -->
					<button type="submit" id="viewGeneratedNtrFiles${UNQID}" class="btn btn-primary btn-sm">View Generated Files</button>
					<button type="submit" id="searchNTReport${UNQID}" class="btn btn-success btn-sm"><spring:message code="app.common.searchButton"/></button>
					<input type="reset" id="clearNTReport${UNQID}" class="btn btn-danger btn-sm" name="ClearNTReport" value="Clear"/>
		...
		<div class="card card-primary" id="npoTransactionReportSerachResultPanel${UNQID}" style="display: none;">
			<div class="card-header panelSlidingNTReport${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}"><spring:message code="app.common.npoTransactionReportResultHeader"/></h6>
				<div class="btn-group pull-${dirR} clearfix">
					<span class="pull-right"><i class="collapsable fa fa-chevron-up"></i></span>
				</div>
			</div>
			<div id="npoTransactionReportSerachResult${UNQID}"></div>
		</div>
	</div>
</div>
