...
function updateLegalDetails(strName,strID,strAnnexure)
{
...
var l_strAlertNo = '<%= a_strAlertNo%>';
window.open('<%=contextPath%>/common/getNewINDSTREntities?legalName='+custName+'&legalId='+custID+'&annexure='+annexure+'&CallFrom='+callfrom,'ANNEXUREB','top=150, height=600, width=750,scrollbars=yes,toolbar=yes,resizable=yes,location=no');
}

function deleteLegalDetails(strName,strID,strAnnexure)
{
callfrom = '';//document.form1.CallFrom.value;
custName = strName;
custID = strID;
...
if(conf)
{	
	this.parent.location.replace('<%=contextPath%>/common/deleteNewINDSTREntities?legalName='+custName+'&legalId='+custID+'&CallFrom='+callfrom);
}
}
-->
</script>
...
<div class="section">
	...
	<% for(int i=0; i < intNoOfEntities; i++){ 
		...
		<tr>
			...
			<td style="text-align : center;">
				<a href="#1" onclick="updateLegalDetails('<%=strEntitiesNameArray[i]%>','<%=strEntitiesIdArray[i]%>','<%=i+1%>')"><img alt="update details" title="Update Details" src="${pageContext.request.contextPath}/includes/images/edit.png"/></a>
				<a href="#1" onclick="deleteLegalDetails('<%=strEntitiesNameArray[i]%>','<%=strEntitiesIdArray[i]%>','<%=i+1%>')"><img alt="delete details" title="Delete Details" src="${pageContext.request.contextPath}/includes/images/delete.png"/></a>
			</td>
		</tr>
		<% }
		else
