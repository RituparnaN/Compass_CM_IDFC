...
//alert('l_strAlertNo:  '+l_strAlertNo);
//var win= window.open('<%=contextPath%>/RegulatoryReports/India/STR/addIndividualDetails.jsp?l_strAlertNo='+l_strAlertNo,'NEWANNEXUREA',"top=150, height=600, width=800,scrollbars=yes,toolbar=yes,resizable=yes,location=no");
var win= window.open('<%=contextPath%>/common/addNewIndividualDetails?l_strAlertNo='+l_strAlertNo,'NEWANNEXUREA',"top=150, height=600, width=800,scrollbars=yes,toolbar=yes,resizable=yes,location=no");
}
...
function deleteIndividualDetails(strName,strID,strAcct,strAnnexure)
{
callfrom = '';//document.form1.CallFrom.value;
custName = strName;
custID = strID;
...
if(conf)
{	this.parent.location.replace('<%=contextPath%>/common/deleteNewINDSTRIndividuals?indvName='+custName+'&indvId='+custID+'&acctNo='+accountNo+'&CallFrom='+callfrom);
}
}
-->
</script>
	...
	<table class="info-table">
		...
		<tr>
			...
			<td style="text-align : center;" <%if(canUpdated.equals("N")){%> hidden="hidden" <%}%> >
				...
				<a href="#1" onclick="deleteIndividualDetails('<%=strIndividualNameArray[i]%>','<%=strIndividualIdArray[i]%>','<%=strCounterAccountArray[i]%>','<%=i+1%>')"><img alt="delete details" title="Delete Details" src="${pageContext.request.contextPath}/includes/images/delete.png"/></a>
			</td>
		</tr>
		<% }
		else
