...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#generateRegulatoryException"+id).click(function(){
			var reportingMonth = $("#reportingMonth"+id).val();
			var reportingYear = $("#reportingYear"+id).val();
			var reportType = $("#reportType"+id).val();
			
			if(confirm("Do you want to generate exception file ?")){
				window.open("${pageContext.request.contextPath}/common/generateRegulatoryExceptionFile?reportingMonth="+reportingMonth+"&reportingYear="+reportingYear+"&reportType="+reportType);
			}
		});
		

		$("#downloadRegulatoryExcel"+id).click(function(){
			var reportingMonth = $("#reportingMonth"+id).val();
			var reportingYear = $("#reportingYear"+id).val();
			var reportType = $("#reportType"+id).val();
			
			if(confirm("Do you want to download excel ?")){
				window.open("${pageContext.request.contextPath}/common/downloadRegulatoryExcel?reportingMonth="+reportingMonth+"&reportingYear="+reportingYear+"&reportType="+reportType);
			}
		});
		
		$("#regulatoryExceptionUpload"+id).click(function(){
			compassFileUpload.init("uploadFile","${pageContext.request.contextPath}","regulatoryExceptionUpload","0","Y","N","");
		});
		...
		<div class="card card-primary panel_regulatoryReportGenetation">
							...
							<td>
								<div class="input-group" style="z-index: 1">
									<input type="text" class="form-control input-sm" aria-describedby="basic-addon-ela" name="reportFileName" id="reportFileName" readonly="readonly"/>
										<span class="input-group-addon formSearchIcon searchFileName${UNQID}" id="basic-addon-ela" style="cursor: pointer;" title="Search">
			...
			<div class="card-footer clearfix">
				<div class="pull-${dirR}">
					...
					<button type="button" id="downloadRegulatoryExcel${UNQID}" class="btn btn-warning btn-sm">Download Excel</button>
					<button type="button" id="generateRegulatoryException${UNQID}" class="btn btn-warning btn-sm">Generate Exception</button>
					<button type="button" id="regulatoryExceptionUpload${UNQID}" class="btn btn-warning btn-sm">Upload Corrected Exception</button>
				</div>
			</div>
			</form>
		</div>
