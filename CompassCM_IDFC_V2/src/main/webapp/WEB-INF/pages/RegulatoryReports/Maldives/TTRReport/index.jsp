...
<script type="text/javascript">
	$(document).ready(function() {
		var id = '${UNQID}';
		compassTopFrame.init(id, 'MVTTRReportTable', 'dd/mm/yy');
		
		$(".panelSlidingMVTTReport"+id).on("click", function(e){
			var mainRow = $(this).parents(".compassrow"+id);
			compassTopFrame.searchPanelSliding(id, mainRow, 'MVTTRReportSerachResultPanel');
		});
		
		$("#searchMasterForm"+id).submit(function(e){
			var submitButton = $("#searchMVTTReport"+id);
			compassTopFrame.submitForm(id, e, submitButton, 'MVTTRReportSerachResultPanel',
					'MVTTRReportSerachResult', '${pageContext.request.contextPath}/common/searchGenericMaster',
					'${pageContext.request.contextPath}/includes/images/qde-loadder.gif');
		});

		$("#exportReportingMVTTRFile").click(function(){
			var reportType = "MVTTR";
			...
			if(confirm("Are you sure you want to Generate Text Reporting File?")){
				$.fileDownload("${pageContext.request.contextPath}/common/generateMVTTRReportingFile?"+formData+"&reportType="+reportType+"&reportingFileType="+reportingFileType, {
				    httpMethod : "GET",
		...
		$("#exportReportingMVTTRXMLFile").click(function(){
			var reportType = "MVTTR";
...
<div class="row compassrow${UNQID}">
	<div class="col-sm-12">
		<div class="card card-primary panel_MVTTRReport">
			<div class="card-header panelSlidingMVTTReport${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}"><spring:message code="app.common.MVTTRReportHeader"/></h6>
			...
			<input type="hidden" name="bottomPageUrl" value="RegulatoryReports/Maldives/TTRReport/SearchBottomPage">
			<div class="card-search-card" >
				<table class="table table-striped formSearchTable MVTTRReportTable" style="margin-bottom: 0px;">
					<tbody>
			...
			<div class="card-footer clearfix">
				<div class="pull-${dirR}">
					<button type="submit" id="searchMVTTReport${UNQID}" class="btn btn-success btn-sm"><spring:message code="app.common.searchButton"/></button>
					<input type="reset" id="clearMVTTReport${UNQID}" class="btn btn-danger btn-sm" name="clearMVTTReport" value="Clear"/>
		...
		<div class="card card-primary" id="MVTTRReportSerachResultPanel${UNQID}" style="display: none;">
			<div class="card-header panelSlidingMVTTReport${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}"><spring:message code="app.common.MVTTRReportResultHeader"/></h6>
				<div class="btn-group pull-${dirR} clearfix">
					<span class="pull-right"><i class="collapsable fa fa-chevron-up"></i></span>
				</div>
			</div>
			<div id="MVTTRReportSerachResult${UNQID}"></div>
			
			<div class="card-footer action-footer clearfix">
				<div class="pull-${dirR} clearfix">
					<!-- <button type="button" class="btn btn-primary" id="exportReportingMVTTRFile">Export Reporting File</button>
					<button type="button" class="btn btn-primary" id="exportReportingMVTTRXMLFile">Export XML File</button> -->
