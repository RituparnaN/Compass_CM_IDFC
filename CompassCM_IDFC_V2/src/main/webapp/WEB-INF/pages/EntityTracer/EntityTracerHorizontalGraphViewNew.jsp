...
var tree = d3.layout.tree().nodeSize([80, 50]); 
...
var url = "${pageContext.request.contextPath}/common/getEntityLinkedDetailsGraphView?AccountNumber=<%=AccountNumber%>"+
	...
	function update(source) {
	 ...
	 //calculate svg width using node children
	  ...
	  var newHeight = d3.max(levelWidth) * 150; 
	  if(levelWidth.length*200 < 1400){
		  d3.select("svg").attr("width", "100%")
	  }else{
		  d3.select("svg").attr("width", levelWidth.length*200)
	  }
	  ...
	  nodes.forEach(function(d) {
		 nodeEnter.append("circle").attr("r", 20).attr("fill", "white")
      ...
      .style("stroke-dasharray", function(d) {
		let tempDetails = d.target;
           if (tempDetails.LINKEDTYPE == "TRANSACTION") {return "2.5"};
         })
      ...
      .style("stroke-width",
        function(d){
         ...
         // stroke with respect to transaction Risk
	   .style("stroke",function(d) {
			let tempDetails = d.target;
			let clr = "";
	  ...
	  window.setTimeout(function() {
		    ...
		    d3.select("svg").attr("height", max + 80)
		  }, 800)
	}
	...
	function findPosition(elm){
		...
		if($("#flareDataDialogBox > #SEARCHACCOUNTNO").val() != "")
			...
			//$("#flareDataDialogBox").css({"top" : mouseY, "left" : mouseX}).hide();
...
<!-- <div class="inlineCheckboxAndButtons sticky">
	<div class="linkCheckbox">
		<label id="staticLinkLabel"></label><br>
		<label id="transactionLinkLabel"></label><br>
	</div>
...
<div id="d3flare">
	<h1 id="staticLinkChecks">No data for static link</h1>
</div>
<style type="text/css">
	#flareDataDialogBox{
		padding: 10px;
