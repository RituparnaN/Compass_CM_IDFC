...
<script type="text/javascript">
	$(document).ready(function() {
		var id = '${UNQID}';
		compassTopFrame.init(id, 'fatcaUserPendingCaseTable'+id, 'dd/mm/yy');
		
		$('.panelSlidingFATCAUserPendingCase'+id).on("click", function (e) {
			var mainRow = $(this).parents(".compassrow"+id);
			compassTopFrame.searchPanelSliding(id, mainRow, 'fatcaUserPendingCaseSerachResultPanel');
	    });
		
		$("#searchMasterForm"+id).submit(function(e){
			var submitButton = $("#searchFATCAUserPendingCase"+id);
			compassTopFrame.submitForm(id, e, submitButton, 'fatcaUserPendingCaseSerachResultPanel', 
					'fatcaUserPendingCaseSerachResult', '${pageContext.request.contextPath}/fatcaCaseWorkFlow/searchPendingCases',
		...
		$("#attachEvedence").click(function(){
			var caseNo = "";
			var count = 0;
			$("#fatcaUserPendingCaseSerachResult"+id).find("table").children("tbody").children("tr").each(function(){
			...
			compassFileUpload.init("attachEvedence","${pageContext.request.contextPath}","5678","fatcaUserPendingCaseSerachResult"+id,"Y","Y");
			}
		});
		/*
		$("#fatca61B").click(function(){
			var caseNo = "";
			var count = 0;
			$("#fatcaUserPendingCaseSerachResult"+id).find("table").children("tbody").children("tr").each(function(){
				if($(this).children("td").children("input").prop("checked")){
			...
			if(count == 0){
				...
				$("#compassCaseWorkFlowGenericModal-title").html("FORM 61B : "+caseNo);
				...
				$.ajax({
					url: "${pageContext.request.contextPath}/common/show61BForm",
					cache: false,
					type: "POST",
					data: "caseNo="+caseNo,
		...
		*/
        $("#fatca61B").click(function(){
			...
			if(count == 0){
				...
				window.open("${pageContext.request.contextPath}/common/show61BForm?caseNo="+caseNo, "Form 61B", "");
			}
		});
        ...
        */
        ...
        $("#maldivesSTRIndiv").click(function(){
        ...
        $("#maldivesSTRLegal").click(function(){
...
<div class="row compassrow${UNQID}">
	<div class="col-sm-12">
		<div class="card card-primary panel_fatcaUserPendingCase">
			<div class="card-header panelSlidingFATCAUserPendingCase${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}"><spring:message code="app.common.fatcaUserPendingCaseSearchHeader"/></h6>
			...
			<input type="hidden" name="bottomPageUrl" value="FATCACaseWorkFlow/FATCAUser/PendingCases/SearchBottomPage">
			<div class="card-search-card" >
				<table class="table table-striped formSearchTable fatcaUserPendingCaseTable${UNQID}" style="margin-bottom: 0px;">
					<tbody>
			...
			<div class="card-footer clearfix">
				<div class="pull-${dirR}">
					<button  type="submit" id="searchFATCAUserPendingCase${UNQID}" class="btn btn-success btn-sm"><spring:message code="app.common.searchButton"/></button>
				</div>
			</div>
			</form>
			</div>
		</div>
		<div class="card card-primary" id="fatcaUserPendingCaseSerachResultPanel${UNQID}" style="display: none;">
			<div class="card-header panelSlidingFATCAUserPendingCase${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}"><spring:message code="app.common.fatcaUserPendingCaseResultHeader"/></h6>
				<div class="btn-group pull-${dirR} clearfix">
					<span class="pull-right"><i class="collapsable fa fa-chevron-up"></i></span>
				</div>
			</div>
			<div id="fatcaUserPendingCaseSerachResult${UNQID}"></div>
			
			<div class="card-footer action-footer clearfix">
				<div class="pull-${dirR} clearfix">
					
					<button type="button" class="btn btn-primary btn-sm" onclick="caseWorkFlowActions.fatcaDesktopCloser('fatcaUserPendingCaseSerachResult${UNQID}')">Desktop Closure</button>
					...
					<div class="btn-group dropup" role="group">
						...
						<ul class="dropdown-menu" role="menu">
							<li><a class="nav-link" href="javascript:void(0)" onclick="caseWorkFlowActions.closeWithEscalation('fatcaUserPendingCaseSerachResult${UNQID}')">With Escalation</a></li>
							<li><a class="nav-link" href="javascript:void(0)" onclick="caseWorkFlowActions.closeWithoutEscalation('fatcaUserPendingCaseSerachResult${UNQID}')">Without Escalation</a></li>
						</ul>
					</div>
					
					<button type="button" class="btn btn-primary btn-sm" onclick="caseWorkFlowActions.viewFatcaComments('fatcaUserPendingCaseSerachResult${UNQID}')">View Comment</button>
					...
					<div class="btn-group dropup" role="group">
						...
						<ul class="dropdown-menu" role="menu">
							<li><a href="javascript:void(0)" onclick="caseWorkFlowActions.viewIndianSTR('fatcaUserPendingCaseSerachResult${UNQID}')">Indian STR</a></li>
							<li><a href="javascript:void(0)" onclick="caseWorkFlowActions.viewSriLankanSTR('fatcaUserPendingCaseSerachResult${UNQID}')">SL STR</a></li>
							<li><a href="javascript:void(0)" onclick="caseWorkFlowActions.viewUKSTR('fatcaUserPendingCaseSerachResult${UNQID}')">UK SAR</a></li>
							<li><a href="javascript:void(0)" onclick="caseWorkFlowActions.viewMaldivesSTR('fatcaUserPendingCaseSerachResult${UNQID}')">Maldives STR</a></li>
						</ul>
					</div>
					 -->			
					<button type="button" class="btn btn-primary btn-sm" id="fatca61B">FORM 61B</button>
					...
					<button type="button" class="btn btn-primary btn-sm" onclick="caseWorkFlowActions.assignToRMUserByFATCAUser('fatcaUserPendingCaseSerachResult${UNQID}')">Assign To RM</button>
