...
<script type="text/javascript">
	$(document).ready(function() {
		var id = '${UNQID}';
		compassTopFrame.init(id, 'fatcaOfficerPendingCaseTable'+id, 'dd/mm/yy');
		
		$('.panelSlidingFATCAOfficerPendingCase'+id).on("click", function (e) {
			var mainRow = $(this).parents(".compassrow"+id);
			compassTopFrame.searchPanelSliding(id, mainRow, 'fatcaOfficerPendingCaseSerachResultPanel');
	    });
		
		$("#searchMasterForm"+id).submit(function(e){
			var submitButton = $("#searchFATCAOfficerPendingCase"+id);
			compassTopFrame.submitForm(id, e, submitButton, 'fatcaOfficerPendingCaseSerachResultPanel', 
					'fatcaOfficerPendingCaseSerachResult', '${pageContext.request.contextPath}/fatcaCaseWorkFlow/searchPendingCases',
		...
		$("#attachEvedence").click(function(){
			var caseNo = "";
			var count = 0;
			$("#fatcaOfficerPendingCaseSerachResult"+id).find("table").children("tbody").children("tr").each(function(){
			...
			compassFileUpload.init("attachEvedence","${pageContext.request.contextPath}","5678","fatcaOfficerPendingCaseSerachResult"+id,"Y","Y");
			}
		});
...
<div class="row compassrow${UNQID}">
	<div class="col-sm-12">
		<div class="card card-primary panel_fatcaOfficerPendingCase">
			<div class="card-header panelSlidingFATCAOfficerPendingCase${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}"><spring:message code="app.common.fatcaOfficerPendingCaseSearchHeader"/></h6>
			...
			<input type="hidden" name="bottomPageUrl" value="FATCACaseWorkFlow/FATCAOfficer/PendingCases/SearchBottomPage">
			<div class="card-search-card" >
				<table class="table table-striped formSearchTable fatcaOfficerPendingCaseTable${UNQID}" style="margin-bottom: 0px;">
					<tbody>
			...
			<div class="card-footer clearfix">
				<div class="pull-${dirR}">
					<button  type="submit" id="searchFATCAOfficerPendingCase${UNQID}" class="btn btn-success btn-sm"><spring:message code="app.common.searchButton"/></button>
				</div>
			</div>
			</form>
			</div>
		</div>
		<div class="card card-primary" id="fatcaOfficerPendingCaseSerachResultPanel${UNQID}" style="display: none;">
			<div class="card-header panelSlidingFATCAOfficerPendingCase${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}"><spring:message code="app.common.fatcaOfficerPendingCaseResultHeader"/></h6>
				<div class="btn-group pull-${dirR} clearfix">
					<span class="pull-right"><i class="collapsable fa fa-chevron-up"></i></span>
				</div>
			</div>
			<div id="fatcaOfficerPendingCaseSerachResult${UNQID}"></div>
			
			<div class="card-footer action-footer clearfix">
				<div class="pull-${dirR} clearfix">
					
					<button type="button" class="btn btn-primary btn-sm" onclick="caseWorkFlowActions.rejectCaseByFATCAOfficer('fatcaOfficerPendingCaseSerachResult${UNQID}')">Reject Case</button>
					
					<button type="button" class="btn btn-primary btn-sm" onclick="caseWorkFlowActions.approveCaseByFATCAOfficer('fatcaOfficerPendingCaseSerachResult${UNQID}')">Approve Case</button>
					<!-- <button type="button" class="btn btn-primary btn-sm" onclick="caseWorkFlowActions.closeWithHindSightingByAMLO('fatcaOfficerPendingCaseSerachResult${UNQID}')">Close With HindSighting</button>
					<button type="button" class="btn btn-primary btn-sm" onclick="caseWorkFlowActions.closeWithoutHindSightingByAMLO('fatcaOfficerPendingCaseSerachResult${UNQID}')">Close Without HindSighting</button> -->
					
					<button type="button" class="btn btn-primary btn-sm" onclick="caseWorkFlowActions.viewFatcaComments('fatcaOfficerPendingCaseSerachResult${UNQID}')">View Comment</button>
		            ...
		            <div class="btn-group dropup" role="group">
						...
						<ul class="dropdown-menu" role="menu">
							<li><a href="javascript:void(0)" onclick="caseWorkFlowActions.viewIndianSTR('fatcaOfficerPendingCaseSerachResult${UNQID}')">Indian STR</a></li>
							<li><a href="javascript:void(0)" onclick="caseWorkFlowActions.viewSriLankanSTR('fatcaOfficerPendingCaseSerachResult${UNQID}')">SL STR</a></li>
							<li><a href="javascript:void(0)" onclick="caseWorkFlowActions.viewUKSTR('fatcaOfficerPendingCaseSerachResult${UNQID}')">UK SAR</a></li>
						</ul>
					</div>
		            -->
					...
					<button type="button" class="btn btn-primary btn-sm" id="assignToBranchUser">ReAssign To FATCAUser</button>
