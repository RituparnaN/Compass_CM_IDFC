...
<script type="text/javascript">
	$(document).ready(function() {
		var id = '${UNQID}';
		compassTopFrame.init(id, 'fatcaOfficerClosedCaseTable'+id, 'dd/mm/yy');
		
		$('.panelSlidingFATCAOfficerClosedCase'+id).on("click", function (e) {
			var mainRow = $(this).parents(".compassrow"+id);
			compassTopFrame.searchPanelSliding(id, mainRow, 'fatcaOfficerClosedCaseSerachResultPanel');
	    });
		
		$("#searchMasterForm"+id).submit(function(e){
			var submitButton = $("#searchFATCAOfficerClosedCase"+id);
			compassTopFrame.submitForm(id, e, submitButton, 'fatcaOfficerClosedCaseSerachResultPanel', 
					'fatcaOfficerClosedCaseSerachResult', '${pageContext.request.contextPath}/fatcaCaseWorkFlow/searchClosedCases',
		...
		$("#attachEvedence").click(function(){
			var caseNo = "";
			var count = 0;
			$("#fatcaOfficerClosedCaseSerachResult"+id).find("table").children("tbody").children("tr").each(function(){
			...
			compassFileUpload.init("attachEvedence","${pageContext.request.contextPath}","5678","fatcaOfficerClosedCaseSerachResult"+id,"Y","Y");
			}
		});
...
<div class="row compassrow${UNQID}">
	<div class="col-sm-12">
		<div class="card card-primary panel_fatcaOfficerClosedCase">
			<div class="card-header panelSlidingFATCAOfficerClosedCase${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}"><spring:message code="app.common.fatcaOfficerClosedCaseSearchHeader"/></h6>
			...
			<input type="hidden" name="bottomPageUrl" value="FATCACaseWorkFlow/FATCAOfficer/ClosedCasesWithAMLUser/SearchBottomPage">
			<div class="card-search-card" >
				<table class="table table-striped formSearchTable fatcaOfficerClosedCaseTable${UNQID}" style="margin-bottom: 0px;">
					<tbody>
			...
			<div class="card-footer clearfix">
				<div class="pull-${dirR}">
					<button  type="submit" id="searchFATCAOfficerClosedCase${UNQID}" class="btn btn-success btn-sm"><spring:message code="app.common.searchButton"/></button>
				</div>
			</div>
			</form>
			</div>
		</div>
		<div class="card card-primary" id="fatcaOfficerClosedCaseSerachResultPanel${UNQID}" style="display: none;">
			<div class="card-header panelSlidingFATCAOfficerClosedCase${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}"><spring:message code="app.common.fatcaOfficerClosedCaseResultHeader"/></h6>
				<div class="btn-group pull-${dirR} clearfix">
					<span class="pull-right"><i class="collapsable fa fa-chevron-up"></i></span>
				</div>
			</div>
			<div id="fatcaOfficerClosedCaseSerachResult${UNQID}"></div>
			
			<div class="card-footer action-footer clearfix">
				<div class="pull-${dirR} clearfix">
					
					<button type="button" class="btn btn-primary btn-sm" onclick="caseWorkFlowActions.rejectSTRByAMLO('fatcaOfficerClosedCaseSerachResult${UNQID}')">Reject STR</button>
					
					<button type="button" class="btn btn-primary btn-sm" onclick="caseWorkFlowActions.approveSTRByAMLO('fatcaOfficerClosedCaseSerachResult${UNQID}')">Approve STR</button>
					<button type="button" class="btn btn-primary btn-sm" onclick="caseWorkFlowActions.closeWithHindSightingByAMLO('fatcaOfficerClosedCaseSerachResult${UNQID}')">Close With HindSighting</button>
					<button type="button" class="btn btn-primary btn-sm" onclick="caseWorkFlowActions.closeWithoutHindSightingByAMLO('fatcaOfficerClosedCaseSerachResult${UNQID}')">Close Without HindSighting</button>
					
					<button type="button" class="btn btn-primary btn-sm" onclick="caseWorkFlowActions.viewComments('fatcaOfficerClosedCaseSerachResult${UNQID}')">View Comment</button>
		            ...
		            <div class="btn-group dropup" role="group">
						...
						<ul class="dropdown-menu" role="menu">
							<li><a class="nav-link" href="javascript:void(0)" onclick="caseWorkFlowActions.viewIndianSTR('fatcaOfficerPendingCaseSerachResult${UNQID}')">Indian STR</a></li>
							<li><a class="nav-link" href="javascript:void(0)" onclick="caseWorkFlowActions.viewSriLankanSTR('fatcaOfficerPendingCaseSerachResult${UNQID}')">SL STR</a></li>
							<li><a class="nav-link" href="javascript:void(0)" onclick="caseWorkFlowActions.viewUKSTR('fatcaOfficerPendingCaseSerachResult${UNQID}')">UK SAR</a></li>
						</ul>
					</div>
					...
					<button type="button" class="btn btn-primary btn-sm" id="assignToBranchUser">ReAssign To AMLUser</button>
