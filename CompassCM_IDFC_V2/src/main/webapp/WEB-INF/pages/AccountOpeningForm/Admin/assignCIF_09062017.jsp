...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$("#calculateCIF").click(function(){
			...
			$.ajax({
				type : "POST",
				url : "<%=contextPath%>/calculateCIFForAssign?${_csrf.parameterName}=${_csrf.token}&BRANCH_CODE="+branchCode+"&CIF_TYPE="+cifType+"&CIF_NUMBER="+cifNumber+"&ACCOUNT_NO="+accountNo+"&EXCLUDE_APPROVED="+excludeApproved,
				cache : false,
		...
		$("#fetchStatus").click(function(){
			$.ajax({
				type : "POST",
				url : "<%=contextPath%>/getAccountOpeningMandateFetchStatus?${_csrf.parameterName}=${_csrf.token}",
				cache : false,
				success : function(response){
					var resData = "Status: ";
	...
	<div class="row">
		<div class="col-lg-12">
			<div class="card card-primary">
				...
				<form action="${pageContext.request.contextPath}/assignCIFToCheckerMaker?${_csrf.parameterName}=${_csrf.token}" method="POST" onsubmit="return checkForm()">
					<table class="table table-bordered" style="margin-bottom: 0px;">
						...
						<tr id="assignTR" <% if(count == 0){%>style="display:none;"<%}%>>
							<td colspan="5" style="text-align:center">
								<button type="submit" class="btn btn-primary btn-small" value="Assign"></button>
							</td>
						</tr>
					</table>
				</form>
