...
<script type="text/javascript">
	$(document).ready(function() {
		var id = '${UNQID}';
		compassTopFrame.init(id, 'pendingFilesTable'+id, 'dd/mm/yy');

		$('.panelSlidingPendingFiles'+id).on("click", function (e) {
			var mainRow = $(this).parents(".compassrow"+id);
			compassTopFrame.searchPanelSliding(id, mainRow, 'pendingFilesSerachResultPanel');
	    });
		
		$("#searchMasterForm"+id).submit(function(e){
			var submitButton = $("#searchPendingFiles"+id);
			compassTopFrame.submitForm(id, e, submitButton, 'pendingFilesSerachResultPanel', 
					'pendingFilesSerachResult', '${pageContext.request.contextPath}/common/searchGenericMaster',
...
<div class="row compassrow${UNQID}">
	<div class="col-sm-12">
		<div class="card card-primary panel_pendingFiles">
			<div class="card-header panelSlidingPendingFiles${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}"><spring:message code="app.common.pendingFilesSearchHeader"/></h6>
			...
			<input type="hidden" name="bottomPageUrl" value="ManualFileUpload/PendingFiles/SearchBottomPage">
			<div class="card-search-card" >
				<table class="table table-striped formSearchTable pendingFilesTable${UNQID}" style="margin-bottom: 0px;">
					<tbody>
			...
			<div class="card-footer clearfix">
				<div class="pull-${dirR}">
					<button  type="submit" id="searchPendingFiles${UNQID}" class="btn btn-primary btn-sm"><spring:message code="app.common.searchButton"/></button>
				</div>
			</div>
			</form>
			</div>
		</div>
		<div class="card card-primary" id="pendingFilesSerachResultPanel${UNQID}" style="display: none;">
			<div class="card-header panelSlidingPendingFiles${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}"><spring:message code="app.common.pendingFilesResultHeader"/></h6>
				<div class="btn-group pull-${dirR} clearfix">
					<span class="pull-right"><i class="collapsable fa fa-chevron-up"></i></span>
				</div>
			</div>
			<div id="pendingFilesSerachResult${UNQID}"></div>
		</div>
	</div>
</div>	