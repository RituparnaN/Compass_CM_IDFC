	 ...
	 $("#addAlertComments"+id).click(function(){
		...
		$("#escalateCaseButton").click(function() {
			 ...
			 if(remarks != undefined){
				 ...
				 $.ajax({
					url : "${pageContext.request.contextPath}/common/escalateCaseNew",
					cache : false,
					type : "POST",
					data : data,
					success : function(res) {
	...
	function checkerAction(action) {
		...
		if (comments.length == 0) {
			alert("Please provide comments")
			 ...
			 $.ajax({
				url : "${pageContext.request.contextPath}/common/checkerActionNew",
				cache : false,
				type : "POST",
										...
										<div class='card card-primary'
											...
											<table class="table table-striped">
												...
												<tr>
													...
													<td width="30%">
														<c:choose>
															<c:when test = "${f:contains(RESPONSE.OPTIONS,'^') }">
															 	<input type="text"
																	class="form-control input-sm"
																	name="userAcctReviewedDate" disabled
																	value="${(f:split(RESPONSE.OPTIONS,'^'))[0]}" />
															</c:when>
															<c:otherwise>
																 <input type="text"
																class="form-control input-sm"
														...
														<tr>
															<c:if
																...
																<tr>
																	...
																	<td width="30%"><input type='text' readonly
																		...
																		class='form-control input-sm'
																		value="${CASEMETADETAILS.QUESTIONID}" />
																	</td>
																	<td width="10%"></td>
																...
																<tr>
																	...
																	<td colspan="4"><textarea name="question"
																			...
																			class='form-control' disabled>${CASEMETADETAILS.QUESTION}</textarea>
																...
																<tr>
																		<td width="15%">Value</td>
																		<c:if test = "${CASEMETADETAILS.INPUTTYPE  == 'text'}">
																			<td width="30%"><textarea name="options"
																				id="options"
																				class='form-control input-sm'/>
																					
																			</td>
																		</c:if>
																		<c:if test = "${CASEMETADETAILS.INPUTTYPE  == 'numeric'}">
																			<td width="30%"><input name="options"
																				id="options" type = 'number'
																				class='form-control input-sm'/>
																					
																			</td>
																		</c:if>
																		<c:if test = "${CASEMETADETAILS.INPUTTYPE  == 'select'}">
																			<td width="30%"><select name="options"
																				id="options"
																				class='form-control input-sm selectpicker'>
																					<c:forEach var="option"
																						items="${f:split(CASEMETADETAILS.OPTIONSLIST,'||')}">
																						<c:if test = "${(f:split(option,'^'))[0] ne 'null' }">
																							<option value="${option}">${(f:split(option,"^"))[0]}</option>
																						</c:if>
																					</c:forEach>
																			</select></td>
																		</c:if>
															...
															<c:if
																...
																<tr>
																	...
																	<td width="30%">
																		<c:choose>
																			<c:when test = "${f:contains(CASEMETADETAILS.ESCALATIONOPTION,'^') }">
																			 	<input type="text"
																					class="form-control input-sm"
																					name="userAcctReviewedDate" disabled
																					value="${(f:split(CASEMETADETAILS.ESCALATIONOPTION,'^'))[0]}" />
																			</c:when>
																			<c:otherwise>
																				 <input type="text"
																				class="form-control input-sm"
