...
$(document).ready(function() {
	...
	$("#processFalsePositiveMappingFile"+id).click(function(){
		var fullData = "uploadRefNo="+uploadRefNo+"&moduleRefId="+moduleRefId;
		$.ajax({
			url: "${pageContext.request.contextPath}/admin/processFalsePositiveUploadedFile",
			cache: false,
			type: "POST",
			data: fullData,
	...
	<div class="col-sm-12">
		 <div class="card card-primary panel_processAlertRatingsMapping">
			<div class="card-header panelSlidingAlertRatingsMappingFile${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}">Customer/Account False Positive Mapping File Upload Process</h6>
			</div>
			<div class="card-footer clearfix">
				<p style="color: red;">Click the Process button to process the uploaded file.</p>
				<div style="float: right;">
					<input type="button" class="btn btn-success btn-sm" id="processFalsePositiveMappingFile${UNQID}" name="Process" value="Process"/>
				</div>
			</div>
		 </div>
	</div>
