	...
	$(document).ready(function(){
		...
		//	 "bFilter" : false,
		 ...
		 $("#searchBO").keyup(function(){
	 	 ...
	 	 $("#startCreateRule").click(function(){
			 ...
			 if(count > 0){
				 ...
				 $.ajax({
					 url : "${pageContext.request.contextPath}/admin/getAllObjectColumns",
					 data : "value="+values,
					 type : "GET",
					 cache : false,	
	 		...
	 		$("#setJoinButton").click(function(){
				 ...
				 $.ajax({
					 url : "${pageContext.request.contextPath}/admin/createJoinForm",
		 ...
		 $("#saveJoinRow").click(function(){
			 ...
			 $.ajax({
				url : "${pageContext.request.contextPath}/admin/buildJoin",
		 ...
		 $("#saveReportRow").click(function(){
			 ...
			 if(isValid){
				 $.ajax({
					 url : "${pageContext.request.contextPath}/admin/buildReportColumns",
					 data : "value="+escape(value),
					 type : "GET",
					 cache : false,	
					 success : function(resData){
		 ...
		 $("#setConditionButton").click(function(){
			 ...
			 if($("#builtConditionColumn").val() == ""){
				 $.ajax({
					 url : "${pageContext.request.contextPath}/admin/createSetConditionForm",
		 ...
		 $("#saveConditionRow").click(function(){
			 ...
			 $.ajax({
				 url : "${pageContext.request.contextPath}/admin/buildCondition",
				 data : "value="+escape(value),
				 type : "GET",
				 cache : false,	
				 success : function(resData){
		 ...
		 $("#setAggregateConditionButton").click(function(){
			 ...
			 if($("#builtAggregateConditionColumn").val() == ""){
				 $.ajax({
					 url : "${pageContext.request.contextPath}/admin/createSetAggregateConditionForm",
		 ...
		 $("#saveAggregateConditionRow").click(function(){
			 ...
			 if(isValid){
				 $.ajax({
					 url : "${pageContext.request.contextPath}/admin/buildAggregateCondition",
				...
				if(reportCode.trim().length > 0){
					if(reportName.trim().length > 0){
						if(reportHeader.trim().length > 0 && reportFooter.trim().length > 0){
							...
							$.ajax({
								url : "${pageContext.request.contextPath}/admin/saveReport",
								type : "POST",
								cache : false,
								data : fullData,
								success : function(resData){
...
<script type="text/javascript">
	function getReportColumns(){
		...
		if(count > 0){
			...
			if(newAdded){
				...
				$.ajax({
					url : "${pageContext.request.contextPath}/admin/builtReportColumnForm",
	...
	function getFields(elm, side){
		if(elm.value != ""){
			...
			$.ajax({
				 url : "${pageContext.request.contextPath}/admin/getObjectColumns",
	...
	function getColumnDatatype(elm){
		...
		var datepickerObj;
		$.ajax({
			url : "${pageContext.request.contextPath}/admin/getColumnsDatatype",
			data : "value="+inpVal,
			type : "GET",
			cache : false,
			success : function(resData){
