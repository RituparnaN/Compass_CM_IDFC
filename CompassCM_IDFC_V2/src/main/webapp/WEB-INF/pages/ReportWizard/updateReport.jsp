...
<style type="text/css">
	...
	.datePicker{
		background-image:url("${pageContext.request.contextPath}/includes/resources/images/calendar.png");
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#businessObjects > tbody > tr").each(function(){
			...
			$.ajax({
				 url : "${pageContext.request.contextPath}/admin/objectToSelect?${_csrf.parameterName}=${_csrf.token}",
				 data : "reportID="+reportId+"&objectName="+escape(elm.val()),
				 type : "POST",
				 cache : false,	
		...
		$.ajax({
			 url : "${pageContext.request.contextPath}/admin/getReportDetails?${_csrf.parameterName}=${_csrf.token}",
			 data : "reportID="+reportId+"&section=NOOFPARAMETERS",
			 type : "POST",
			 cache : false,	
			 success : function(resData){
				 $("#noOfParams").val(resData);
			 },
			 error : function(a,b,c){
				 alert(a+"\n"+b+"\n"+c);
			 }
		...
		//	 "bFilter" : false,
			 ...
			 if(count > 0){
				 ...
				 $.ajax({
					 ...
					 success : function(resData){
						 ...
						 $.ajax({
							 url : "${pageContext.request.contextPath}/admin/getReportDetails?${_csrf.parameterName}=${_csrf.token}",
							 data : "reportID="+reportId+"&section=REPORTJOINDETAILS",
							 type : "POST",
							 cache : false,	
							 success : function(resData){
								 $("#builtJoin").show();
						...
						$.ajax({
							 url : "${pageContext.request.contextPath}/admin/getReportDetails?${_csrf.parameterName}=${_csrf.token}",
							 data : "reportID="+reportId+"&section=JOINTABLEHTML",
							 type : "POST",
							 cache : false,	
							 success : function(resData){
						 ...
						 $.ajax({
							 url : "${pageContext.request.contextPath}/admin/getReportDetails?${_csrf.parameterName}=${_csrf.token}",
							 data : "reportID="+reportId+"&section=REPORTCOLUMNNAMES",
							 type : "POST",
							 cache : false,	
							 success : function(resData){
						...
						$.ajax({
							 url : "${pageContext.request.contextPath}/admin/getReportDetails?${_csrf.parameterName}=${_csrf.token}",
							 data : "reportID="+reportId+"&section=REPORTTABLEHTML",
							 type : "POST",
							 cache : false,	
							 success : function(resData){
						...
						$.ajax({
							 url : "${pageContext.request.contextPath}/admin/getReportDetails?${_csrf.parameterName}=${_csrf.token}",
							 data : "reportID="+reportId+"&section=REPORTCONDITIONDETAILS",
							 type : "POST",
							 cache : false,	
							 success : function(resData){
						...
						$.ajax({
							 url : "${pageContext.request.contextPath}/admin/getReportDetails?${_csrf.parameterName}=${_csrf.token}",
							 data : "reportID="+reportId+"&section=CONDITIONTABLEHTML",
							 type : "POST",
							 cache : false,	
							 success : function(resData){
						...
						$.ajax({
							 url : "${pageContext.request.contextPath}/admin/getReportDetails?${_csrf.parameterName}=${_csrf.token}",
							 data : "reportID="+reportId+"&section=AGGREGATEDCONDITIONDETAILS",
							 type : "POST",
							 cache : false,	
							 success : function(resData){
						...
						$.ajax({
							 url : "${pageContext.request.contextPath}/admin/getReportDetails?${_csrf.parameterName}=${_csrf.token}",
							 data : "reportID="+reportId+"&section=AGGREGATETABLEHTML",
							 type : "POST",
							 cache : false,	
							 success : function(resData){
							 ...
							 success : function(resData){
								 $("#objectDetailPanel > div > div > table").each(function(index){
										...
										$(elm).each(function(indexTd){
											...
											if(resData.indexOf(checkBox.val()) >= 0){
						...
						$.ajax({
							 url : "${pageContext.request.contextPath}/admin/getReportDetails?${_csrf.parameterName}=${_csrf.token}",
							 data : "reportID="+reportId+"&section=REPORTCODE",
							 type : "POST",
							 cache : false,	
							 success : function(resData){
								 $("#reportCode").val(resData);
							 },
							 error : function(a,b,c){
								 alert(a+"\n"+b+"\n"+c);
							 }
						...
						$.ajax({
							 url : "${pageContext.request.contextPath}/admin/getReportDetails?${_csrf.parameterName}=${_csrf.token}",
							 data : "reportID="+reportId+"&section=REPORTNAME",
							 type : "POST",
							 cache : false,	
							 success : function(resData){
								 $("#reportName").val(resData);
							 },
							 error : function(a,b,c){
								 alert(a+"\n"+b+"\n"+c);
							 }
						...
						$.ajax({
							 url : "${pageContext.request.contextPath}/admin/getReportDetails?${_csrf.parameterName}=${_csrf.token}",
							 data : "reportID="+reportId+"&section=REPORTHEADER",
							 type : "POST",
							 cache : false,	
							 success : function(resData){
								 $("#reportHeader").val(resData);
							 },
							 error : function(a,b,c){
								 alert(a+"\n"+b+"\n"+c);
							 }
						...
						$.ajax({
							 url : "${pageContext.request.contextPath}/admin/getReportDetails?${_csrf.parameterName}=${_csrf.token}",
							 data : "reportID="+reportId+"&section=REPORTFOOTER",
							 type : "POST",
							 cache : false,	
							 success : function(resData){
								 $("#reportFooter").val(resData);
							 },
							 error : function(a,b,c){
								 alert(a+"\n"+b+"\n"+c);
							 }
						...
						$.ajax({
							 url : "${pageContext.request.contextPath}/admin/getReportDetails?${_csrf.parameterName}=${_csrf.token}",
							 data : "reportID="+reportId+"&section=ISENABLED",
							 type : "POST",
							 cache : false,	
							 success : function(resData){
								 if(resData == "true"){
		 ...
		 $("#setParametersButton").click(function(){
			 var noOfParams = $("#noOfParams").val();
			 var reportId = $("#reportId").val();
			 // var mywin = window.open("${pageContext.request.contextPath}/ReportWidgets/SetReportParameters.jsp?l_strNoOfParameters="+noOfParams+"&l_strReportID="+reportId,'','height=500,width=1250,scrollbars=1,resizable=Yes');
...
<style type="text/css">
    ...
    <div class="card-body">
		<div class="row row-xs-height">
			<div class="col-sm-3" >
				<div class="card card-info">
					<div class="card-header reportWizHeader">
					    <h5 class="card-title">Report Widget</h5>
					</div>
					...
					<div class="card-footer" style="text-align: center;">
						<button type="button" class="btn btn-success xtrSmlButton" id="startCreateRule">Start</button>
					</div>
				</div>
			</div>
			<div class="col-sm-9">
		...
		<center><div id="initInfo">Initializing report : <%=reportID%>... Please wait</div></center>
		<div class="row row-xs-height" style="margin-top: 20px;">
			<div class="col-sm-12">
				<div class="card card-info" style="border-color: #bce8f1;">>
					<div role="tabpanel"  style="min-height:225px; max-height: 225px; overflow-y: auto;">
					  <ul class="nav nav-tabs" role="tablist" id="reportGenTab">
					    <li role="presentation" class="active">
					    	<a class="nav-link active" href="#joinDetails" aria-controls="joinDetails" role="tab" data-toggle="tab">Set Joins</a>
					    </li>
					    <li role="presentation">
					    	<a class="nav-link" href="#reportColumnDetails" aria-controls="reportColumnDetails" role="tab" data-toggle="tab">Report Columns</a>
					    </li>
					    <li role="presentation">
					    	<a class="nav-link" href="#parameterDetails" aria-controls="parameterDetails" role="tab" data-toggle="tab">Set Parameters</a>
					    </li>
					    <li role="presentation">
					    	<a class="nav-link" href="#conditionColumnDetails" aria-controls="conditionColumnDetails" role="tab" data-toggle="tab">Condition Columns</a>
					    </li>
					    <li role="presentation">
					    	<a class="nav-link" href="#aggregateConditionDetails" aria-controls="aggregateConditionDetails" role="tab" data-toggle="tab">Aggregate Conditions</a>
					    </li>
					  </ul>
					  ...
					  <div class="tab-content">
					    <div role="tabpanel" class="tab-pane active" id="joinDetails">
					    	<div class="col-xs-4 d-flex mb-3 ml-2" >
					    ...
					    <div role="tabpanel" class="tab-pane" id="conditionColumnDetails">
					    	<div class="col-xs-4 d-flex mb-3 ml-2 mt-2">
					    		<div style="vertical-align: middle; ">
					...
					<div class="card-footer" style="text-align: center;" id="reportSaveFooter">
						<button type="button" class="btn btn-success xtrSmlButton" data-toggle="modal" data-target="#saveReportModal" id="saveReportDummy">Save Report</button>
	...
	<div class="modal fade bs-example-modal-lg" id="setJoinModal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
		...
		<div class="modal-content">
			...
			<div class="modal-footer">
				<button type="button" class="btn btn-primary btn-sm" id="addJoinRow">Add Row</button>
				<button type="button" class="btn btn-success btn-sm" id="saveJoinRow">Save Joins</button>
	...
	<div class="modal fade bs-example-modal-lg" id="setReportModal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
		...
		<div class="modal-content">
			...
			<div class="modal-footer">
				<button type="button" class="btn btn-primary btn-sm" id="addReportRow">Add Row</button>
				<button type="button" class="btn btn-success btn-sm" id="saveReportRow">Save Report Columns</button>
				<button type="button" class="btn btn-danger btn-sm" data-dismiss="modal">Close</button>
			</div>
		</div>
		</div>
	...
	<div class="modal fade bs-example-modal-lg" id="setAggregateConditionModal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
		...
		<div class="modal-content">
			...
			<div class="modal-footer">
				<button type="button" class="btn btn-success btn-sm" id="saveAggregateConditionRow">Save Aggregate Conditions</button>
				<button type="button" class="btn btn-danger btn-sm" data-dismiss="modal">Close</button>
			</div>
		</div>
		</div>
	...
	<div class="modal fade bs-example-modal-lg" id="saveReportModal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
		...
		<div class="modal-content">
				 ...
				 <table id='SaveReportTable' class='table table-striped table-bordered' cellspacing='0' width='100%'>
				 	<tbody>
				 		...
				 		<tr>
				 			...
				 			<input type="checkbox" name="isReport" value="N" id="isReport" checked="checked"/>Is Report </td>
				 		</tr>
				 		 -->
				 		<tr>
				 			<td>Is Enable</td>
			...
			<div class="modal-footer">
				<button type="button" class="btn btn-success btn-sm" id="SaveReportFinal">Save Report</button>
				<button type="button" class="btn btn-danger btn-sm" data-dismiss="modal">Close</button>
			</div>
		</div>
		</div>
