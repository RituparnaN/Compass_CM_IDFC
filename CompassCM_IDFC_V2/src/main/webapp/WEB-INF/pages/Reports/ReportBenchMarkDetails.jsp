...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$("#savereportBenchMarkDtls"+id).click(function(e){
			...
			if($('#FromDate_'+id).length && $('#ToDate_'+id).length){
				...
				if(diff < '0'){
					...
					var configurationComments = prompt("Please enter your report configuration comments before saving."); 
					if(configurationComments != null){
						var formObj = $("#searchreportBenchMarkDtlsForm"+id);
						var formData = formatSelectValues($("#searchreportBenchMarkDtlsForm"+id).serializeArray());//(formObj).serialize();
						$.ajax({
							url: "${pageContext.request.contextPath}/common/saveReportBenchMarkParameters" ,
							cache: false,
							data: formData+"&reportId="+reportId+"&id="+id+"&viewType="+viewType+"&reportSerialNo="+reportSerialNo+"&group="+group+"&configurationComments="+configurationComments,
							type: 'POST',
							success: function(res){
								$("#compassMediumGenericModal").modal("hide");
								alert(res);
								$(".reportsGenericListTable"+id).children("tbody").children("tr").each(function(){
									//var reportIdTD = $(this).children("td:first-child");
									var reportNameTD = $(this).children("td:nth-child(2)");
									var reportIdVal = $(reportNameTD).find("#reportIdValue").val();
									if(reportIdVal == reportId){
										$(reportNameTD).click();	
									}
								});
							},
		...
		function  formatSelectValues(formDataObjArray){
			//console.log(formDataObjArray);
			let formDataStr = "";
			let fieldValueAndSeq = {};
			$.each(formDataObjArray,function(index,formFieldObj){
				let fieldName = formFieldObj.name;
				let fieldValue = formFieldObj.value;
				
				if(fieldName in fieldValueAndSeq){
					formDataStr += ","+fieldValue;
				}else{
					if(formDataStr != ""){
						formDataStr += "&"; 
					}
					formDataStr += fieldName+"="+fieldValue;
					fieldValueAndSeq[fieldName] = index;
				}
		...
		$("#showReport"+id).click(function(e){
			var mainRow = $(this).parents("div.compassmodalrow"+id);
			var slidingDiv = $(mainRow).children().children().children();
			var panelBody = $(mainRow).children().children().find(".panelSearchForm");
			var generationType = "showReport";
			var btn = $(this);
			//if(confirm("Are you sure you want to Generate Excel Report?")){
				btn.prop('disabled', true);
				var formObj = $("#searchreportBenchMarkDtlsForm"+id);
	...
	<div class="col-sm-12">
		<div class="card card-primary panel_reportBenchMarkDtls">
								...
								<c:when test="${LABELSITRCOUNT % 2 == 0}">
									<tr>
										...
										<td width="30%">
											...
											<c:if test="${ALLLABELSMAP['MODULEPARAMDATATYPE'] eq 'multiSelect'}">
												<c:set var="SELECTOPTIONNAMEVALUE" value="${ALLLABELSMAP['MODULEPARAMSELECTNAMEVALUES']}"/>
												<c:set var="selectedValueArray" value="${f:split(ALLLABELSMAP['MODULEPARAMDEFAULTVALUE'],',')}"/>
												<select multiple class="form-control input-sm" id="${ALLLABELSMAP['MODULEPARAMIDNAME']}_${UNQID}" name="${ALLLABELSMAP['MODULEPARAMINDEX']}_${ALLLABELSMAP['MODULEPARAMIDNAME']}">
													
													<c:forEach var="NAMEVALUE" items="${SELECTOPTIONNAMEVALUE}">
															<c:set var="contains" value="false" />
															<c:forEach var="item" items="${selectedValueArray}">
															  <c:if test="${item eq NAMEVALUE.key}">
															    <c:set var="contains" value="true" />
															  </c:if>
															</c:forEach>
														<option value="${NAMEVALUE.key}" <c:if test="${contains}">selected</c:if>>${NAMEVALUE.value}</option>
													</c:forEach>
												</select>
											</c:if>
			...
			<div class="card-footer clearfix">
				<div class="pull-${dirR}">
					<button type="button" id="showReport${UNQID}" class="btn btn-primary btn-sm">Show Report</button>
