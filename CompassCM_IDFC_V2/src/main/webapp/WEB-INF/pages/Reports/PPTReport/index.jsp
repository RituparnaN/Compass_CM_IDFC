...
<script type="text/javascript">
	$(document).ready(function() {
		var id = '${UNQID}';
		compassTopFrame.init(id, 'pptReportData', 'dd/mm/yy');
		
		$('.panelSlidingPPTReport'+id).on("click", function (e) {
			var mainRow = $(this).parents(".compassrow"+id);
			compassTopFrame.searchPanelSliding(id, mainRow, 'pptReportSearchResultPanel');
	    });
		...
		$("#searchMasterForm"+id).submit(function(e){
			if(buttonId == "searchPPTReportData"+id){
				var submitButton = $("#searchPPTReportData"+id);
				compassTopFrame.submitForm(id, e, submitButton, 'pptReportSearchResultPanel', 
						'pptReportSearchResult', '${pageContext.request.contextPath}/common/getPPTReportData',
						'${pageContext.request.contextPath}/includes/images/qde-loadder.gif');
			}else if(buttonId == "downloadPPTReportData"+id){
				/* $.ajax({
				    type:"GET",
				    xhrFields: {
	                    responseType: 'blob'
	                  },
				    url:"${pageContext.request.contextPath}/common/downloadPPTReport",
				...
				$.fileDownload("${pageContext.request.contextPath}/common/downloadPPTReport", {
				    httpMethod : "GET",
					successCallback: function (url) {
						coonsole.log("success");
				    	$(elm).html("Downloaded");
				    },
				...
				alert("Erro while submitting form");
			}
			
		});
		
		
		/* $("#downloadPPTReportData"+id).click(function(){
...
<div class="row compassrow${UNQID}">
	<div class="col-sm-12">
		<div class="card card-primary panel_pptReport">
			<div class="card-header panelSlidingPPTReport${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}">PPT Report</h6>
			...
			<input type="hidden" name="bottomPageUrl" value="Reports/PPTReport/searchBottomPage">
			<div class="card-search-card" >
				<table class="table table-striped formSearchTable pptReportData" style="margin-bottom: 0px;">
					<tbody>
			...
			<div class="card-footer clearfix">
				<div class="pull-${dirR}">
					<button type="submit" id="downloadPPTReportData${UNQID}" class="btn btn-success btn-sm submitBtn${UNQID}">Download PPT Report</button>
					<%-- <button type="submit" id="searchPPTReportData${UNQID}" class="btn btn-success btn-sm submitBtn${UNQID}">Get Report Data</button> --%>
				</div>
			</div>
			</form>
			</div>
		</div>
		<div class="card card-primary" id="pptReportSearchResultPanel${UNQID}" style="display: none;">
			<div class="card-header panelSlidingPPTReport${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}">Result</h6>
				<div class="btn-group pull-${dirR} clearfix">
					<span class="pull-right"><i class="collapsable fa fa-chevron-up"></i></span>
				</div>
			</div>
			<div id="pptReportSearchResult${UNQID}"></div>
		</div>
	</div>
</div>
