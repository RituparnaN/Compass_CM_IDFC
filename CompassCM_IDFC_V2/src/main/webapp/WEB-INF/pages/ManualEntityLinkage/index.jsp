...
<script type="text/javascript">
	$(document).ready(function() {
		var id = '${UNQID}';
		compassTopFrame.init(id, 'manualEntityLinkageTable', 'dd/mm/yy');
		
		$('.panelSlidingManualEntityLinkage'+id).on("click", function (e) {
			var mainRow = $(this).parents(".compassrow"+id);
			compassTopFrame.searchPanelSliding(id, mainRow, 'manualEntityLinkageResultPanel');
	    });
		...
		$("#searchMasterForm"+id).submit(function(e){
			if(buttonId == "createEntityRelationLinkage"+id){
				var submitButton = $("#createEntityRelationLinkag"+id);
				compassTopFrame.submitForm(id, e, submitButton, 'manualEntityLinkageResultPanel', 
						'manualEntityLinkageResult', '${pageContext.request.contextPath}/common/getEntityDetails',
						'${pageContext.request.contextPath}/includes/images/qde-loadder.gif');	
				$("#resultPannelHeading"+id).text("Create Manual Entity Linkage");
			}else if(buttonId == "search"+id){
				var submitButton = $("#search"+id);
				compassTopFrame.submitForm(id, e, submitButton, 'manualEntityLinkageResultPanel', 
						'manualEntityLinkageResult', '${pageContext.request.contextPath}/common/searchGenericMaster',
						'${pageContext.request.contextPath}/includes/images/qde-loadder.gif');
				$("#resultPannelHeading"+id).text("Manual Entity Linkage Result");
			}else{
				alert("error, not geeting submit button Id");
			}
...
<div class="row compassrow${UNQID}">
	<div class="col-sm-12">
		<div class="card card-primary panel_manualEntityLinkage">
			<div class="card-header panelSlidingManualEntityLinkage${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}"><spring:message code="app.common.manualEntityLinkageHeader"/></h6>
			...
			<input type="hidden" name="bottomPageUrl" value="ManualEntityLinkage/SearchBottomPage">
			<div class="card-search-card" >
				<table class="table table-striped formSearchTable " style="margin-bottom: 0px;">
					<tbody>
			...
			<div class="card-footer clearfix">
				<div class="pull-${dirR}">
					<button type="submit" id="createEntityRelationLinkage${UNQID}" class="btn btn-success btn-sm submitBtn${UNQID}">Create Entity Linkage</button>
					<button type="submit" id="search${UNQID}" class="btn btn-success btn-sm submitBtn${UNQID}">Search</button>
				</div>
			</div>
			</form>
			</div>
		...
		<div class="card card-primary" id="manualEntityLinkageResultPanel${UNQID}" style="display: none;">
			<div class="card-header panelSlidingManualEntityLinkage${UNQID} clearfix">
				<h6 class="card-title pull-${dirL}"  id="resultPannelHeading${UNQID}"></h6>
				<div class="btn-group pull-${dirR} clearfix">
					<span class="pull-right"><i class="collapsable fa fa-chevron-up"></i></span>
				</div>
			</div>
			<div id="manualEntityLinkageResult${UNQID}"></div>
		</div>
	</div>
</div>
