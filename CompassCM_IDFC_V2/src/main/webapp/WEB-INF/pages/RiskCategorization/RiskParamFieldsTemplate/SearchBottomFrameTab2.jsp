	...
	<thead>
		<tr>
			...
			<c:forEach var="colHeader" items="${resultData['HEADER']}">
				<th class="info" id="${colHeader}"><spring:message code="${colHeader}"/></th>
			</c:forEach>
		</tr>
	</thead>
	<tbody>
						...
						<c:when test="${tabIndex.index == 5}">
							<td>
								<select class = "form-control input-sm" id= "complianceScore">
									<option value="1" <c:if test="${field.value eq '1'}">selected="selected"</c:if>>1-Low</option>
									<option value="2" <c:if test="${field.value eq '2'}">selected="selected"</c:if>>2-Medium</option>
									<option value="3" <c:if test="${field.value eq '3'}">selected="selected"</c:if>>3-High</option>
								</select>
							</td>
						</c:when>
