package com.quantumdataengines.app.compass.dao.riskAssessmentPendingCasesNew;
...
@Repository
public class RiskAssessmentPendingCasesNewDAOImpl implements RiskAssessmentPendingCasesNewDAO{
...
//			System.out.println("chekersCount = "+String.valueOf(checkersArray.length));
	...
	public Map<String,Object> getRFICaseData(String caseNos, String caseId,String caseVersion, String caseStatus, String userCode, String userRole, String ipAddress,String compassRefNo){
		System.out.println("in get rfi case data");
		String procedureName = "{CALL STP_GETRISKASSESSQSDATANEW(:QUESTIONID, :STATUS ,:USERCODE, :ROLECODE, :IPADDRESS, :RESULTSET)}";
		...
		try{
			...
			String query =    "SELECT INPUTOPTIONSLIST"
							+ "  FROM " + schemaName+ "TB_RISKASSESSQUESTIONSCONFIG " 
							+ " WHERE QUESTIONID = ?";
			...
			resultSet = preparedStatement.executeQuery();
			if (resultSet.next()) {
				resultData.put("OPTIONSLIST", resultSet.getString("INPUTOPTIONSLIST"));
			}
			...
			query =    "SELECT ASSIGNEDCHECKERCODES, ESCALATIONOPTIONS "
			...
			preparedStatement = connection.prepareStatement(query);
			preparedStatement.setString(1, caseNos);
			preparedStatement.setString(2, compassRefNo);
			
			resultSet = preparedStatement.executeQuery();
			System.out.println("cmref:"+compassRefNo);
			if (resultSet.next()) {
					System.out.println("res:"+resultSet.getString("ASSIGNEDCHECKERCODES"));
					checkerList = resultSet.getString("ASSIGNEDCHECKERCODES").split(",");
					resultData.put("ESCALATIONOPTION", resultSet.getString("ESCALATIONOPTIONS"));
			}
